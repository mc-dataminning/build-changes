import com.mojang.serialization.Codec;
import com.mojang.serialization.codecs.RecordCodecBuilder;
import java.util.stream.Stream;

public class dae extends daa {
   public static final Codec<dae> b = RecordCodecBuilder.create(
      $$0 -> $$0.group(czw.d.fieldOf("biomes").forGetter($$0x -> $$0x.c), Codec.intRange(0, 62).fieldOf("scale").orElse(2).forGetter($$0x -> $$0x.e))
            .apply($$0, dae::new)
   );
   private final ip<czw> c;
   private final int d;
   private final int e;

   public dae(ip<czw> $$0, int $$1) {
      this.c = $$0;
      this.d = $$1 + 2;
      this.e = $$1;
   }

   @Override
   protected Stream<il<czw>> b() {
      return this.c.a();
   }

   @Override
   protected Codec<? extends daa> a() {
      return b;
   }

   @Override
   public il<czw> getNoiseBiome(int $$0, int $$1, int $$2, daf.f $$3) {
      return this.c.a(Math.floorMod(($$0 >> this.d) + ($$2 >> this.d), this.c.b()));
   }
}
