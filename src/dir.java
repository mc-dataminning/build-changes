import com.mojang.serialization.MapCodec;
import javax.annotation.Nullable;

public class dir extends daa {
   public static final MapCodec<dir> a = b(dir::new);
   public static final int b = 2;
   public static final int c = 1;
   public static final int d = 4;
   private static final eqk g = daa.a(3.0, 0.0, 3.0, 12.0, 7.0, 12.0);
   private static final eqk h = daa.a(1.0, 0.0, 1.0, 15.0, 7.0, 15.0);
   public static final dnz e = dnp.aE;
   public static final dnz f = dnp.aD;

   @Override
   public MapCodec<dir> a() {
      return a;
   }

   public dir(dmy.d $$0) {
      super($$0);
      this.k(this.E.b().a(e, Integer.valueOf(0)).a(f, Integer.valueOf(1)));
   }

   @Override
   public void a(cwz $$0, ib $$1, dmz $$2, bow $$3) {
      if (!$$3.bT()) {
         this.a($$0, $$2, $$1, $$3, 100);
      }

      super.a($$0, $$1, $$2, $$3);
   }

   @Override
   public void a(cwz $$0, dmz $$1, ib $$2, bow $$3, float $$4) {
      if (!($$3 instanceof cgr)) {
         this.a($$0, $$1, $$2, $$3, 3);
      }

      super.a($$0, $$1, $$2, $$3, $$4);
   }

   private void a(cwz $$0, dmz $$1, ib $$2, bow $$3, int $$4) {
      if (this.a($$0, $$3)) {
         if (!$$0.B && $$0.z.a($$4) == 0 && $$1.a(dac.mf)) {
            this.a($$0, $$2, $$1);
         }
      }
   }

   private void a(cwz $$0, ib $$1, dmz $$2) {
      $$0.a(null, $$1, aty.zE, atz.e, 0.7F, 0.9F + $$0.z.i() * 0.2F);
      int $$3 = $$2.c(f);
      if ($$3 <= 1) {
         $$0.b($$1, false);
      } else {
         $$0.a($$1, $$2.a(f, Integer.valueOf($$3 - 1)), 2);
         $$0.a(drn.f, $$1, drn.a.a($$2));
         $$0.c(2001, $$1, daa.i($$2));
      }
   }

   @Override
   protected void b(dmz $$0, apf $$1, ib $$2, axd $$3) {
      if (this.a($$1) && a($$1, $$2)) {
         int $$4 = $$0.c(e);
         if ($$4 < 2) {
            $$1.a(null, $$2, aty.zF, atz.e, 0.7F, 0.9F + $$3.i() * 0.2F);
            $$1.a($$2, $$0.a(e, Integer.valueOf($$4 + 1)), 2);
            $$1.a(drn.c, $$2, drn.a.a($$0));
         } else {
            $$1.a(null, $$2, aty.zG, atz.e, 0.7F, 0.9F + $$3.i() * 0.2F);
            $$1.a($$2, false);
            $$1.a(drn.f, $$2, drn.a.a($$0));

            for (int $$5 = 0; $$5 < $$0.c(f); $$5++) {
               $$1.c(2001, $$2, daa.i($$0));
               cce $$6 = bpc.bg.a((cwz)$$1);
               if ($$6 != null) {
                  $$6.c_(-24000);
                  $$6.i($$2);
                  $$6.b((double)$$2.u() + 0.3 + (double)$$5 * 0.2, (double)$$2.v(), (double)$$2.w() + 0.3, 0.0F, 0.0F);
                  $$1.b($$6);
               }
            }
         }
      }
   }

   public static boolean a(cwf $$0, ib $$1) {
      return b($$0, $$1.d());
   }

   public static boolean b(cwf $$0, ib $$1) {
      return $$0.a_($$1).a(aun.H);
   }

   @Override
   protected void b(dmz $$0, cwz $$1, ib $$2, dmz $$3, boolean $$4) {
      if (a($$1, $$2) && !$$1.B) {
         $$1.c(2012, $$2, 15);
      }
   }

   private boolean a(cwz $$0) {
      float $$1 = $$0.f(1.0F);
      return (double)$$1 < 0.69 && (double)$$1 > 0.65 ? true : $$0.z.a(500) == 0;
   }

   @Override
   public void a(cwz $$0, cis $$1, ib $$2, dmz $$3, @Nullable dkg $$4, cqk $$5) {
      super.a($$0, $$1, $$2, $$3, $$4, $$5);
      this.a($$0, $$2, $$3);
   }

   @Override
   protected boolean a(dmz $$0, css $$1) {
      return !$$1.h() && $$1.n().a(this.l()) && $$0.c(f) < 4 ? true : super.a($$0, $$1);
   }

   @Nullable
   @Override
   public dmz a(css $$0) {
      dmz $$1 = $$0.q().a_($$0.a());
      return $$1.a(this) ? $$1.a(f, Integer.valueOf(Math.min(4, $$1.c(f) + 1))) : super.a($$0);
   }

   @Override
   protected eqk a(dmz $$0, cwf $$1, ib $$2, epw $$3) {
      return $$0.c(f) > 1 ? h : g;
   }

   @Override
   protected void a(dna.a<daa, dmz> $$0) {
      $$0.a(e, f);
   }

   private boolean a(cwz $$0, bow $$1) {
      if ($$1 instanceof cce || $$1 instanceof cay) {
         return false;
      } else {
         return !($$1 instanceof bpo) ? false : $$1 instanceof cis || $$0.Z().b(cwv.c);
      }
   }
}
