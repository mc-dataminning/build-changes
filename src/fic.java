import com.mojang.logging.LogUtils;
import java.util.List;
import javax.annotation.Nullable;
import org.slf4j.Logger;

public class fic extends ffe {
   public static final int a = 308;
   public static final int b = 100;
   public static final int c = 74;
   public static final int k = 64;
   private static final Logger m = LogUtils.getLogger();
   private final fqk n = new fqk();
   private final ffe o;
   protected fig l;
   private fqj p;
   private ezh q;
   private ezh r;
   private ezh t;
   private fqi u;
   private gla.b v;
   @Nullable
   private gla.a w;
   private boolean x;

   public fic(ffe $$0) {
      super(vq.c("multiplayer.title"));
      this.o = $$0;
   }

   @Override
   protected void aQ_() {
      if (this.x) {
         this.l.a(this.g, this.h - 64 - 32, 0, 32);
      } else {
         this.x = true;
         this.p = new fqj(this.f);
         this.p.a();
         this.v = new gla.b();

         try {
            this.w = new gla.a(this.v);
            this.w.start();
         } catch (Exception var8) {
            m.warn("Unable to start LAN server detection: {}", var8.getMessage());
         }

         this.l = new fig(this, this.f, this.g, this.h - 64 - 32, 32, 36);
         this.l.a(this.p);
      }

      this.d(this.l);
      this.r = this.d(ezh.a(vq.c("selectServer.select"), $$0 -> this.o()).a(100).a());
      ezh $$1 = this.d(ezh.a(vq.c("selectServer.direct"), $$0 -> {
         this.u = new fqi(ghz.a("selectServer.defaultName"), "", fqi.c.c);
         this.f.a(new fef(this, this::g, this.u));
      }).a(100).a());
      ezh $$2 = this.d(ezh.a(vq.c("selectServer.add"), $$0 -> {
         this.u = new fqi(ghz.a("selectServer.defaultName"), "", fqi.c.c);
         this.f.a(new feh(this, this::f, this.u));
      }).a(100).a());
      this.q = this.d(ezh.a(vq.c("selectServer.edit"), $$0 -> {
         fig.a $$1x = this.l.i();
         if ($$1x instanceof fig.d) {
            fqi $$2x = ((fig.d)$$1x).c();
            this.u = new fqi($$2x.a, $$2x.b, fqi.c.c);
            this.u.b($$2x);
            this.f.a(new feh(this, this::e, this.u));
         }
      }).a(74).a());
      this.t = this.d(ezh.a(vq.c("selectServer.delete"), $$0 -> {
         fig.a $$1x = this.l.i();
         if ($$1x instanceof fig.d) {
            String $$2x = ((fig.d)$$1x).c().a;
            if ($$2x != null) {
               vq $$3x = vq.c("selectServer.deleteQuestion");
               vq $$4x = vq.a("selectServer.deleteWarning", $$2x);
               vq $$5x = vq.c("selectServer.deleteButton");
               vq $$6x = vp.e;
               this.f.a(new fdx(this::c, $$3x, $$4x, $$5x, $$6x));
            }
         }
      }).a(74).a());
      ezh $$3 = this.d(ezh.a(vq.c("selectServer.refresh"), $$0 -> this.J()).a(74).a());
      ezh $$4 = this.d(ezh.a(vp.k, $$0 -> this.d()).a(74).a());
      fcx $$5 = fcx.d();
      fcq $$6 = $$5.a(new fcq(308, 20, fcq.b.a));
      $$6.a(this.r);
      $$6.a($$1);
      $$6.a($$2);
      $$5.a(fcy.b(4));
      fcq $$7 = $$5.a(new fcq(308, 20, fcq.b.a));
      $$7.a(this.q);
      $$7.a(this.t);
      $$7.a($$3);
      $$7.a($$4);
      $$5.a();
      fcr.a($$5, 0, this.h - 64, this.g, 64);
      this.E();
   }

   @Override
   public void d() {
      this.f.a(this.o);
   }

   @Override
   public void e() {
      super.e();
      List<gkz> $$0 = this.v.a();
      if ($$0 != null) {
         this.l.a($$0);
      }

      this.n.a();
   }

   @Override
   public void k() {
      if (this.w != null) {
         this.w.interrupt();
         this.w = null;
      }

      this.n.b();
      this.l.d();
   }

   private void J() {
      this.f.a(new fic(this.o));
   }

   private void c(boolean $$0) {
      fig.a $$1 = this.l.i();
      if ($$0 && $$1 instanceof fig.d) {
         this.p.a(((fig.d)$$1).c());
         this.p.b();
         this.l.a(null);
         this.l.a(this.p);
      }

      this.f.a(this);
   }

   private void e(boolean $$0) {
      fig.a $$1 = this.l.i();
      if ($$0 && $$1 instanceof fig.d) {
         fqi $$2 = ((fig.d)$$1).c();
         $$2.a = this.u.a;
         $$2.b = this.u.b;
         $$2.b(this.u);
         this.p.b();
         this.l.a(this.p);
      }

      this.f.a(this);
   }

   private void f(boolean $$0) {
      if ($$0) {
         fqi $$1 = this.p.b(this.u.b);
         if ($$1 != null) {
            $$1.a(this.u);
            this.p.b();
         } else {
            this.p.a(this.u, false);
            this.p.b();
         }

         this.l.a(null);
         this.l.a(this.p);
      }

      this.f.a(this);
   }

   private void g(boolean $$0) {
      if ($$0) {
         fqi $$1 = this.p.a(this.u.b);
         if ($$1 == null) {
            this.p.a(this.u, true);
            this.p.b();
            this.a(this.u);
         } else {
            this.a($$1);
         }
      } else {
         this.f.a(this);
      }
   }

   @Override
   public boolean a(int $$0, int $$1, int $$2) {
      if (super.a($$0, $$1, $$2)) {
         return true;
      } else if ($$0 == 294) {
         this.J();
         return true;
      } else if (this.l.i() != null) {
         if (fdh.a($$0)) {
            this.o();
            return true;
         } else {
            return this.l.a($$0, $$1, $$2);
         }
      } else {
         return false;
      }
   }

   @Override
   public void a(eyu $$0, int $$1, int $$2, float $$3) {
      super.a($$0, $$1, $$2, $$3);
      $$0.a(this.i, this.e, this.g / 2, 20, 16777215);
   }

   public void o() {
      fig.a $$0 = this.l.i();
      if ($$0 instanceof fig.d) {
         this.a(((fig.d)$$0).c());
      } else if ($$0 instanceof fig.c) {
         gkz $$1 = ((fig.c)$$0).b();
         this.a(new fqi($$1.a(), $$1.b(), fqi.c.a));
      }
   }

   private void a(fqi $$0) {
      fdy.a(this, this.f, frk.a($$0.b), $$0, false, null);
   }

   public void a(fig.a $$0) {
      this.l.a($$0);
      this.E();
   }

   protected void E() {
      this.r.j = false;
      this.q.j = false;
      this.t.j = false;
      fig.a $$0 = this.l.i();
      if ($$0 != null && !($$0 instanceof fig.b)) {
         this.r.j = true;
         if ($$0 instanceof fig.d) {
            this.q.j = true;
            this.t.j = true;
         }
      }
   }

   public fqk H() {
      return this.n;
   }

   public fqj I() {
      return this.p;
   }
}
