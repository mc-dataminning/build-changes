...
public class mf implements ko {
   private final kq.a d;
   private final kq.a e;

   public mf(kq $$0) {
      this.d = $$0.a(kq.b.b, "blockstates");
      this.e = $$0.a(kq.b.b, "models");
   }

   @Override
   public CompletableFuture<?> a(km $$0) {
      Map<czf, mg> $$1 = Maps.newHashMap();
      Consumer<mg> $$2 = $$1x -> {
         czf $$2x = $$1x.a();
         mg $$3x = $$1.put($$2x, $$1x);
         if ($$3x != null) {
      ...
      Map<ajc, Supplier<JsonElement>> $$3 = Maps.newHashMap();
      Set<cpl> $$4 = Sets.newHashSet();
      BiConsumer<ajc, Supplier<JsonElement>> $$5 = ($$1x, $$2x) -> {
         Supplier<JsonElement> $$3x = $$3.put($$1x, $$2x);
         if ($$3x != null) {
            throw new IllegalStateException("Duplicate model definition for " + $$1x);
         }
      };
      Consumer<cpl> $$6 = $$4::add;
      new md($$2, $$5, $$6).a();
      new me($$5).a();
      List<czf> $$7 = kh.e.g().stream().filter($$0x -> true).map(Entry::getValue).filter($$1x -> !$$1.containsKey($$1x)).toList();
      if (!$$7.isEmpty()) {
   ...
   private <T> CompletableFuture<?> a(km $$0, Map<T, ? extends Supplier<JsonElement>> $$1, Function<T, Path> $$2) {
