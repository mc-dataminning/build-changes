import java.util.Map;
import javax.annotation.Nullable;

public class cjw extends ciu implements bxu {
   private static final int bG = 40;
   private static final akj<Byte> bH = akn.a(cjw.class, akl.a);
   private static final Map<cyb, Integer> bI = af.a(cyb.class, cjw::c);
   private int bK;
   private cds bL;

   private static int c(cyb $$0) {
      if ($$0 == cyb.a) {
         return -1644826;
      } else {
         int $$1 = $$0.d();
         float $$2 = 0.75F;
         return axw.a(255, azm.d((float)axw.b($$1) * 0.75F), azm.d((float)axw.c($$1) * 0.75F), azm.d((float)axw.d($$1) * 0.75F));
      }
   }

   public static int a(cyb $$0) {
      return bI.get($$0);
   }

   public cjw(bwo<? extends cjw> $$0, dja $$1) {
      super($$0, $$1);
   }

   @Override
   protected void D() {
      this.bL = new cds(this);
      this.bD.a(0, new cdu(this));
      this.bD.a(1, new cet(this, 1.25));
      this.bD.a(2, new cdm(this, 1.0));
      this.bD.a(3, new cfi(this, 1.1, $$0 -> $$0.a(axk.ag), false));
      this.bD.a(4, new cdz(this, 1.1));
      this.bD.a(5, this.bL);
      this.bD.a(6, new cfn(this, 1.0));
      this.bD.a(7, new cei(this, crc.class, 6.0F));
      this.bD.a(8, new cev(this));
   }

   @Override
   public boolean i(czd $$0) {
      return $$0.a(axk.ag);
   }

   @Override
   protected void a(arq $$0) {
      this.bK = this.bL.h();
      super.a($$0);
   }

   @Override
   public void k_() {
      if (this.dV().C) {
         this.bK = Math.max(0, this.bK - 1);
      }

      super.k_();
   }

   public static byj.a q() {
      return ciu.gw().a(byk.s, 8.0).a(byk.v, 0.23F);
   }

   @Override
   protected void a(akn.a $$0) {
      super.a($$0);
      $$0.a(bH, (byte)0);
   }

   @Override
   public void b(byte $$0) {
      if ($$0 == 10) {
         this.bK = 40;
      } else {
         super.b($$0);
      }
   }

   public float J(float $$0) {
      if (this.bK <= 0) {
         return 0.0F;
      } else if (this.bK >= 4 && this.bK <= 36) {
         return 1.0F;
      } else {
         return this.bK < 4 ? ((float)this.bK - $$0) / 4.0F : -((float)(this.bK - 40) - $$0) / 4.0F;
      }
   }

   public float K(float $$0) {
      if (this.bK > 4 && this.bK <= 36) {
         float $$1 = ((float)(this.bK - 4) - $$0) / 32.0F;
         return (float) (Math.PI / 5) + 0.21991149F * azm.a($$1 * 28.7F);
      } else {
         return this.bK > 0 ? (float) (Math.PI / 5) : this.k($$0) * (float) (Math.PI / 180.0);
      }
   }

   @Override
   public bud b(crc $$0, buc $$1) {
      czd $$2 = $$0.b($$1);
      if ($$2.a(czh.td)) {
         if (this.dV() instanceof arq $$3 && this.a()) {
            this.a($$3, awo.h, $$2);
            this.a(efh.M, $$0);
            $$2.a(1, $$0, d($$1));
            return bud.b;
         }

         return bud.c;
      } else {
         return super.b($$0, $$1);
      }
   }

   @Override
   public void a(arq $$0, awo $$1, czd $$2) {
      $$0.a(null, this, awn.xr, $$1, 1.0F, 1.0F);
      this.a($$0, ezd.aV, $$2, ($$0x, $$1x) -> {
         for (int $$2x = 0; $$2x < $$1x.M(); $$2x++) {
            cnh $$3 = this.a($$0x, $$1x.c(1), 1.0F);
            if ($$3 != null) {
               $$3.i($$3.dy().b((double)((this.ae.i() - this.ae.i()) * 0.1F), (double)(this.ae.i() * 0.05F), (double)((this.ae.i() - this.ae.i()) * 0.1F)));
            }
         }
      });
      this.w(true);
   }

   @Override
   public boolean a() {
      return this.bK() && !this.x() && !this.n_();
   }

   @Override
   public void b(tz $$0) {
      super.b($$0);
      $$0.a("Sheared", this.x());
      $$0.a("Color", (byte)this.t().a());
   }

   @Override
   public void a(tz $$0) {
      super.a($$0);
      this.w($$0.q("Sheared"));
      this.b(cyb.a($$0.f("Color")));
   }

   @Override
   protected awm u() {
      return awn.xo;
   }

   @Override
   protected awm e(buu $$0) {
      return awn.xq;
   }

   @Override
   protected awm l_() {
      return awn.xp;
   }

   @Override
   protected void b(iu $$0, eah $$1) {
      this.a(awn.xs, 0.15F, 1.0F);
   }

   public cyb t() {
      return cyb.a(this.al.a(bH) & 15);
   }

   public void b(cyb $$0) {
      byte $$1 = this.al.a(bH);
      this.al.a(bH, (byte)($$1 & 240 | $$0.a() & 15));
   }

   @Nullable
   @Override
   public <T> T a(ki<? extends T> $$0) {
      return $$0 == kj.aQ ? c((ki<T>)$$0, this.t()) : super.a($$0);
   }

   @Override
   protected void a(ke $$0) {
      this.a($$0, kj.aQ);
      super.a($$0);
   }

   @Override
   protected <T> boolean b(ki<T> $$0, T $$1) {
      if ($$0 == kj.aQ) {
         this.b(c(kj.aQ, $$1));
         return true;
      } else {
         return super.b($$0, $$1);
      }
   }

   public boolean x() {
      return (this.al.a(bH) & 16) != 0;
   }

   public void w(boolean $$0) {
      byte $$1 = this.al.a(bH);
      if ($$0) {
         this.al.a(bH, (byte)($$1 | 16));
      } else {
         this.al.a(bH, (byte)($$1 & -17));
      }
   }

   public static cyb a(azv $$0) {
      int $$1 = $$0.a(100);
      if ($$1 < 5) {
         return cyb.p;
      } else if ($$1 < 10) {
         return cyb.h;
      } else if ($$1 < 15) {
         return cyb.i;
      } else if ($$1 < 18) {
         return cyb.m;
      } else {
         return $$0.a(500) == 0 ? cyb.g : cyb.a;
      }
   }

   @Nullable
   public cjw b(arq $$0, bvv $$1) {
      cjw $$2 = bwo.bb.a($$0, bwn.e);
      if ($$2 != null) {
         cyb $$3 = this.t();
         cyb $$4 = ((cjw)$$1).t();
         $$2.b(cyb.a($$0, $$3, $$4));
      }

      return $$2;
   }

   @Override
   public void R() {
      super.R();
      this.w(false);
      if (this.n_()) {
         this.b_(60);
      }
   }

   @Nullable
   @Override
   public bxw a(djr $$0, bub $$1, bwn $$2, @Nullable bxw $$3) {
      this.b(a($$0.C_()));
      return super.a($$0, $$1, $$2, $$3);
   }
}
