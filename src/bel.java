...
public class bel extends DataFix {
   ...
   public TypeRewriteRule makeRule() {
      return this.a(this.getInputSchema().getTypeRaw(bjd.t), this.getOutputSchema().getTypeRaw(bjd.t));
   }

   private <ItemStackOld, ItemStackNew> TypeRewriteRule a(Type<ItemStackOld> $$0, Type<ItemStackNew> $$1) {
      Type<Pair<String, Either<List<ItemStackOld>, Unit>>> $$2 = DSL.named(bjd.A.typeName(), DSL.optional(DSL.field("Equipment", DSL.list($$0))));
      ...
      if (!$$2.equals(this.getInputSchema().getType(bjd.A))) {
         throw new IllegalStateException("Input entity_equipment type does not match expected");
      } else if (!$$3.equals(this.getOutputSchema().getType(bjd.A))) {
         throw new IllegalStateException("Output entity_equipment type does not match expected");
      } else {
         return TypeRewriteRule.seq(
            this.fixTypeEverywhereTyped(
               "EntityEquipmentToArmorAndHandFix - drop chances", this.getInputSchema().getType(bjd.D), $$0x -> $$0x.update(DSL.remainderFinder(), bel::a)
            ),
