import com.google.common.collect.Lists;
import com.google.common.collect.Maps;
import com.google.common.collect.Sets;
import com.mojang.authlib.GameProfile;
import com.mojang.logging.LogUtils;
import com.mojang.serialization.Dynamic;
import java.io.File;
import java.net.SocketAddress;
import java.nio.file.Path;
import java.text.SimpleDateFormat;
import java.time.Instant;
import java.util.EnumSet;
import java.util.List;
import java.util.Map;
import java.util.Optional;
import java.util.Set;
import java.util.UUID;
import java.util.function.Function;
import java.util.function.Predicate;
import javax.annotation.Nullable;
import net.minecraft.server.MinecraftServer;
import org.slf4j.Logger;

public abstract class avd {
   public static final File a = new File("banned-players.json");
   public static final File b = new File("banned-ips.json");
   public static final File c = new File("ops.json");
   public static final File d = new File("whitelist.json");
   public static final wp e = wp.c("chat.filtered_full");
   public static final wp f = wp.c("multiplayer.disconnect.duplicate_login");
   private static final Logger h = LogUtils.getLogger();
   private static final int i = 600;
   private static final SimpleDateFormat j = new SimpleDateFormat("yyyy-MM-dd 'at' HH:mm:ss z");
   private final MinecraftServer k;
   private final List<are> l = Lists.newArrayList();
   private final Map<UUID, are> m = Maps.newHashMap();
   private final avj n = new avj(a);
   private final ava o = new ava(b);
   private final ave p = new ave(c);
   private final avl q = new avl(d);
   private final Map<UUID, awg> r = Maps.newHashMap();
   private final Map<UUID, ald> s = Maps.newHashMap();
   private final evj t;
   private boolean u;
   private final jy<ale> v;
   protected final int g;
   private int w;
   private int x;
   private boolean y;
   private static final boolean z = false;
   private int A;

   public avd(MinecraftServer $$0, jy<ale> $$1, evj $$2, int $$3) {
      this.k = $$0;
      this.v = $$1;
      this.g = $$3;
      this.t = $$2;
   }

   public void a(vi $$0, are $$1, art $$2) {
      GameProfile $$3 = $$1.gh();
      auz $$4 = this.k.at();
      String $$6;
      if ($$4 != null) {
         Optional<GameProfile> $$5 = $$4.a($$3.getId());
         $$6 = $$5.<String>map(GameProfile::getName).orElse($$3.getName());
         $$4.a($$3);
      } else {
         $$6 = $$3.getName();
      }

      Optional<tq> $$8 = this.a($$1);
      aku<dgj> $$9 = $$8.<aku<dgj>>flatMap($$0x -> eat.a(new Dynamic(ue.a, $$0x.c("Dimension"))).resultOrPartial(h::error)).orElse(dgj.i);
      ard $$10 = this.k.a($$9);
      ard $$11;
      if ($$10 == null) {
         h.warn("Unknown respawn dimension {}, defaulting to overworld", $$9);
         $$11 = this.k.J();
      } else {
         $$11 = $$10;
      }

      $$1.a($$11);
      String $$13 = $$0.a(this.k.bl());
      h.info("{}[{}] logged in with entity id {} at ({}, {}, {})", new Object[]{$$1.al().getString(), $$13, $$1.ar(), $$1.dA(), $$1.dC(), $$1.dG()});
      evc $$14 = $$11.D_();
      $$1.c($$8.orElse(null));
      asg $$15 = new asg(this.k, $$0, $$1, $$2);
      $$0.a(agf.a.a(wa.a(this.k.ba())), $$15);
      dgf $$16 = $$11.O();
      boolean $$17 = $$16.b(dgf.E);
      boolean $$18 = $$16.b(dgf.q);
      boolean $$19 = $$16.b(dgf.x);
      $$15.b(new add($$1.ar(), $$14.l(), this.k.K(), this.n(), this.w, this.x, $$18, !$$17, $$19, $$1.b($$11), this.k.aA()));
      $$15.b(new aby($$14.q(), $$14.r()));
      $$15.b(new adn($$1.gj()));
      $$15.b(new afb($$1.gi().j));
      dbm $$20 = this.k.aI();
      $$15.b(new aga($$20.b(), $$20.d()));
      this.e($$1);
      $$1.I().c();
      $$1.J().a($$1);
      this.a($$11.g(), $$1);
      this.k.av();
      xd $$21;
      if ($$1.gh().getName().equalsIgnoreCase($$6)) {
         $$21 = wp.a("multiplayer.player.joined", $$1.p_());
      } else {
         $$21 = wp.a("multiplayer.player.joined.renamed", $$1.p_(), $$6);
      }

      this.a($$21.a(n.o), false);
      $$15.a($$1.dA(), $$1.dC(), $$1.dG(), $$1.dL(), $$1.dN());
      ajs $$23 = this.k.au();
      if ($$23 != null && !$$2.d()) {
         $$1.a($$23);
      }

      $$1.f.b(adt.a(this.l));
      this.l.add($$1);
      this.m.put($$1.cG(), $$1);
      this.a(adt.a(List.of($$1)));
      this.a($$1, $$11);
      $$11.a($$1);
      this.k.aM().a($$1);
      this.d($$1);
      $$1.b($$8);
      $$1.a($$8);
      $$1.d();
   }

   protected void a(alo $$0, are $$1) {
      Set<fby> $$2 = Sets.newHashSet();

      for (fcb $$3 : $$0.g()) {
         $$1.f.b(aff.a($$3, true));
      }

      for (fbx $$4 : fbx.values()) {
         fby $$5 = $$0.a($$4);
         if ($$5 != null && !$$2.contains($$5)) {
            for (yw<?> $$7 : $$0.d($$5)) {
               $$1.f.b($$7);
            }

            $$2.add($$5);
         }
      }
   }

   public void a(ard $$0) {
      $$0.F_().a(new dym() {
         @Override
         public void a(dyo $$0, double $$1) {
            avd.this.a(new aem($$0));
         }

         @Override
         public void a(dyo $$0, double $$1, double $$2, long $$3) {
            avd.this.a(new ael($$0));
         }

         @Override
         public void a(dyo $$0, double $$1, double $$2) {
            avd.this.a(new aek($$0));
         }

         @Override
         public void a(dyo $$0, int $$1) {
            avd.this.a(new aen($$0));
         }

         @Override
         public void b(dyo $$0, int $$1) {
            avd.this.a(new aeo($$0));
         }

         @Override
         public void b(dyo $$0, double $$1) {
         }

         @Override
         public void c(dyo $$0, double $$1) {
         }
      });
   }

   public Optional<tq> a(are $$0) {
      tq $$1 = this.k.aZ().w();
      Optional<tq> $$2;
      if (this.k.a($$0.gh()) && $$1 != null) {
         $$2 = Optional.of($$1);
         $$0.g($$1);
         h.debug("loading single player");
      } else {
         $$2 = this.t.b($$0);
      }

      return $$2;
   }

   protected void b(are $$0) {
      this.t.a($$0);
      awg $$1 = this.r.get($$0.cG());
      if ($$1 != null) {
         $$1.a();
      }

      ald $$2 = this.s.get($$0.cG());
      if ($$2 != null) {
         $$2.b();
      }
   }

   public void c(are $$0) {
      ard $$1 = $$0.y();
      $$0.a(awk.j);
      this.b($$0);
      if ($$0.bZ()) {
         bum $$2 = $$0.df();
         if ($$2.de()) {
            h.debug("Removing player mount");
            $$0.ae();
            $$2.db().forEach($$0x -> $$0x.c(bum.d.d));
         }
      }

      $$0.ao();

      for (cqb $$3 : $$0.ak()) {
         $$3.c(bum.d.d);
      }

      $$1.a($$0, bum.d.d);
      $$0.S().a();
      this.l.remove($$0);
      this.k.aM().b($$0);
      UUID $$4 = $$0.cG();
      are $$5 = this.m.get($$4);
      if ($$5 == $$0) {
         this.m.remove($$4);
         this.r.remove($$4);
         this.s.remove($$4);
      }

      this.a(new ads(List.of($$0.cG())));
   }

   @Nullable
   public wp a(SocketAddress $$0, GameProfile $$1) {
      if (this.n.a($$1)) {
         avk $$2 = this.n.b($$1);
         xd $$3 = wp.a("multiplayer.disconnect.banned.reason", $$2.d());
         if ($$2.c() != null) {
            $$3.b(wp.a("multiplayer.disconnect.banned.expiration", j.format($$2.c())));
         }

         return $$3;
      } else if (!this.c($$1)) {
         return wp.c("multiplayer.disconnect.not_whitelisted");
      } else if (this.o.a($$0)) {
         avb $$4 = this.o.b($$0);
         xd $$5 = wp.a("multiplayer.disconnect.banned_ip.reason", $$4.d());
         if ($$4.c() != null) {
            $$5.b(wp.a("multiplayer.disconnect.banned_ip.expiration", j.format($$4.c())));
         }

         return $$5;
      } else {
         return this.l.size() >= this.g && !this.d($$1) ? wp.c("multiplayer.disconnect.server_full") : null;
      }
   }

   public are a(GameProfile $$0, aqo $$1) {
      return new are(this.k, this.k.J(), $$0, $$1);
   }

   public boolean e(GameProfile $$0) {
      UUID $$1 = $$0.getId();
      Set<are> $$2 = Sets.newIdentityHashSet();

      for (are $$3 : this.l) {
         if ($$3.cG().equals($$1)) {
            $$2.add($$3);
         }
      }

      are $$4 = this.m.get($$0.getId());
      if ($$4 != null) {
         $$2.add($$4);
      }

      for (are $$5 : $$2) {
         $$5.f.a(f);
      }

      return !$$2.isEmpty();
   }

   public are a(are $$0, boolean $$1, bum.d $$2) {
      this.l.remove($$0);
      $$0.y().a($$0, $$2);
      eua $$3 = $$0.a(!$$1, eua.a);
      ard $$4 = $$3.b();
      are $$5 = new are(this.k, $$4, $$0.gh(), $$0.C());
      $$5.f = $$0.f;
      $$5.a($$0, $$1);
      $$5.e($$0.ar());
      $$5.b($$0.fy());
      if (!$$3.g()) {
         $$5.b($$0);
      }

      for (String $$6 : $$0.as()) {
         $$5.a($$6);
      }

      fbb $$7 = $$3.c();
      $$5.b($$7.d, $$7.e, $$7.f, $$3.e(), $$3.f());
      if ($$3.g()) {
         $$5.f.b(new act(act.b, 0.0F));
      }

      byte $$8 = (byte)($$1 ? 1 : 0);
      ard $$9 = $$5.y();
      evc $$10 = $$9.D_();
      $$5.f.b(new aee($$5.b($$9), $$8));
      $$5.f.a($$5.dA(), $$5.dC(), $$5.dG(), $$5.dL(), $$5.dN());
      $$5.f.b(new aet($$4.Z(), $$4.aa()));
      $$5.f.b(new aby($$10.q(), $$10.r()));
      $$5.f.b(new aez($$5.ct, $$5.cs, $$5.cr));
      this.d($$5);
      this.a($$5, $$4);
      this.e($$5);
      $$4.b($$5);
      this.l.add($$5);
      this.m.put($$5.cG(), $$5);
      $$5.d();
      $$5.x($$5.eD());
      ji $$11 = $$5.T();
      ard $$12 = this.k.a($$5.V());
      if (!$$1 && $$11 != null && $$12 != null) {
         dwy $$13 = $$12.a_($$11);
         if ($$13.a(djp.pN)) {
            $$5.f.b(new afn(awa.vY, awb.e, (double)$$11.u(), (double)$$11.v(), (double)$$11.w(), 1.0F, 1.0F, $$4.H_().g()));
         }
      }

      return $$5;
   }

   public void d(are $$0) {
      this.a($$0, $$0.f);
   }

   public void a(bvi $$0, asg $$1) {
      for (btr $$2 : $$0.eA()) {
         $$1.b(new afz($$0.ar(), $$2, false));
      }
   }

   public void e(are $$0) {
      GameProfile $$1 = $$0.gh();
      int $$2 = this.k.c($$1);
      this.a($$0, $$2);
   }

   public void d() {
      if (++this.A > 600) {
         this.a(new adt(EnumSet.of(adt.a.e), this.l));
         this.A = 0;
      }
   }

   public void a(yw<?> $$0) {
      for (are $$1 : this.l) {
         $$1.f.b($$0);
      }
   }

   public void a(yw<?> $$0, aku<dgj> $$1) {
      for (are $$2 : this.l) {
         if ($$2.dV().ai() == $$1) {
            $$2.f.b($$0);
         }
      }
   }

   public void a(coy $$0, wp $$1) {
      fci $$2 = $$0.cr();
      if ($$2 != null) {
         for (String $$4 : $$2.g()) {
            are $$5 = this.a($$4);
            if ($$5 != null && $$5 != $$0) {
               $$5.a($$1);
            }
         }
      }
   }

   public void b(coy $$0, wp $$1) {
      fci $$2 = $$0.cr();
      if ($$2 == null) {
         this.a($$1, false);
      } else {
         for (int $$3 = 0; $$3 < this.l.size(); $$3++) {
            are $$4 = this.l.get($$3);
            if ($$4.cr() != $$2) {
               $$4.a($$1);
            }
         }
      }
   }

   public String[] e() {
      String[] $$0 = new String[this.l.size()];

      for (int $$1 = 0; $$1 < this.l.size(); $$1++) {
         $$0[$$1] = this.l.get($$1).gh().getName();
      }

      return $$0;
   }

   public avj f() {
      return this.n;
   }

   public ava g() {
      return this.o;
   }

   public void a(GameProfile $$0) {
      this.p.a(new avf($$0, this.k.k(), this.p.a($$0)));
      are $$1 = this.a($$0.getId());
      if ($$1 != null) {
         this.e($$1);
      }
   }

   public void b(GameProfile $$0) {
      this.p.c($$0);
      are $$1 = this.a($$0.getId());
      if ($$1 != null) {
         this.e($$1);
      }
   }

   private void a(are $$0, int $$1) {
      if ($$0.f != null) {
         byte $$2;
         if ($$1 <= 0) {
            $$2 = 24;
         } else if ($$1 >= 4) {
            $$2 = 28;
         } else {
            $$2 = (byte)(24 + $$1);
         }

         $$0.f.b(new acp($$0, $$2));
      }

      this.k.aG().a($$0);
   }

   public boolean c(GameProfile $$0) {
      return !this.u || this.p.d($$0) || this.q.d($$0);
   }

   public boolean f(GameProfile $$0) {
      return this.p.d($$0) || this.k.a($$0) && this.k.aZ().m() || this.y;
   }

   @Nullable
   public are a(String $$0) {
      int $$1 = this.l.size();

      for (int $$2 = 0; $$2 < $$1; $$2++) {
         are $$3 = this.l.get($$2);
         if ($$3.gh().getName().equalsIgnoreCase($$0)) {
            return $$3;
         }
      }

      return null;
   }

   public void a(@Nullable coy $$0, double $$1, double $$2, double $$3, double $$4, aku<dgj> $$5, yw<?> $$6) {
      for (int $$7 = 0; $$7 < this.l.size(); $$7++) {
         are $$8 = this.l.get($$7);
         if ($$8 != $$0 && $$8.dV().ai() == $$5) {
            double $$9 = $$1 - $$8.dA();
            double $$10 = $$2 - $$8.dC();
            double $$11 = $$3 - $$8.dG();
            if ($$9 * $$9 + $$10 * $$10 + $$11 * $$11 < $$4 * $$4) {
               $$8.f.b($$6);
            }
         }
      }
   }

   public void h() {
      for (int $$0 = 0; $$0 < this.l.size(); $$0++) {
         this.b(this.l.get($$0));
      }
   }

   public avl i() {
      return this.q;
   }

   public String[] j() {
      return this.q.a();
   }

   public ave k() {
      return this.p;
   }

   public String[] l() {
      return this.p.a();
   }

   public void a() {
   }

   public void a(are $$0, ard $$1) {
      dyo $$2 = this.k.J().F_();
      $$0.f.b(new acw($$2));
      $$0.f.b(new afj($$1.ad(), $$1.ae(), $$1.O().b(dgf.l)));
      $$0.f.b(new aet($$1.Z(), $$1.aa()));
      if ($$1.ag()) {
         $$0.f.b(new act(act.c, 0.0F));
         $$0.f.b(new act(act.i, $$1.d(1.0F)));
         $$0.f.b(new act(act.j, $$1.b(1.0F)));
      }

      $$0.f.b(new act(act.o, 0.0F));
      this.k.aP().a($$0);
   }

   public void f(are $$0) {
      $$0.cc.b();
      $$0.v();
      $$0.f.b(new afb($$0.gi().j));
   }

   public int m() {
      return this.l.size();
   }

   public int n() {
      return this.g;
   }

   public boolean o() {
      return this.u;
   }

   public void a(boolean $$0) {
      this.u = $$0;
   }

   public List<are> b(String $$0) {
      List<are> $$1 = Lists.newArrayList();

      for (are $$2 : this.l) {
         if ($$2.B().equals($$0)) {
            $$1.add($$2);
         }
      }

      return $$1;
   }

   public int p() {
      return this.w;
   }

   public int q() {
      return this.x;
   }

   public MinecraftServer c() {
      return this.k;
   }

   @Nullable
   public tq r() {
      return null;
   }

   public void b(boolean $$0) {
      this.y = $$0;
   }

   public void s() {
      for (int $$0 = 0; $$0 < this.l.size(); $$0++) {
         this.l.get($$0).f.a(wp.c("multiplayer.disconnect.server_shutdown"));
      }
   }

   public void a(wp $$0, boolean $$1) {
      this.a($$0, $$1x -> $$0, $$1);
   }

   public void a(wp $$0, Function<are, wp> $$1, boolean $$2) {
      this.k.a($$0);

      for (are $$3 : this.l) {
         wp $$4 = $$1.apply($$3);
         if ($$4 != null) {
            $$3.b($$4, $$2);
         }
      }
   }

   public void a(xf $$0, ex $$1, wl.a $$2) {
      this.a($$0, $$1::a, $$1.i(), $$2);
   }

   public void a(xf $$0, are $$1, wl.a $$2) {
      this.a($$0, $$1::c, $$1, $$2);
   }

   private void a(xf $$0, Predicate<are> $$1, @Nullable are $$2, wl.a $$3) {
      boolean $$4 = this.a($$0);
      this.k.a($$0.d(), $$3, $$4 ? null : "Not Secure");
      xe $$5 = xe.a($$0);
      boolean $$6 = false;

      for (are $$7 : this.l) {
         boolean $$8 = $$1.test($$7);
         $$7.a($$5, $$8, $$3);
         $$6 |= $$8 && $$0.j();
      }

      if ($$6 && $$2 != null) {
         $$2.a(e);
      }
   }

   private boolean a(xf $$0) {
      return $$0.i() && !$$0.a(Instant.now());
   }

   public awg a(coy $$0) {
      UUID $$1 = $$0.cG();
      awg $$2 = this.r.get($$1);
      if ($$2 == null) {
         File $$3 = this.k.a(eve.b).toFile();
         File $$4 = new File($$3, $$1 + ".json");
         if (!$$4.exists()) {
            File $$5 = new File($$3, $$0.al().getString() + ".json");
            Path $$6 = $$5.toPath();
            if (v.a($$6) && v.b($$6) && $$6.startsWith($$3.getPath()) && $$5.isFile()) {
               $$5.renameTo($$4);
            }
         }

         $$2 = new awg(this.k, $$4);
         this.r.put($$1, $$2);
      }

      return $$2;
   }

   public ald g(are $$0) {
      UUID $$1 = $$0.cG();
      ald $$2 = this.s.get($$1);
      if ($$2 == null) {
         Path $$3 = this.k.a(eve.a).resolve($$1 + ".json");
         $$2 = new ald(this.k.aC(), this, this.k.aD(), $$3, $$0);
         this.s.put($$1, $$2);
      }

      $$2.a($$0);
      return $$2;
   }

   public void a(int $$0) {
      this.w = $$0;
      this.a(new aer($$0));

      for (ard $$1 : this.k.L()) {
         if ($$1 != null) {
            $$1.m().a($$0);
         }
      }
   }

   public void b(int $$0) {
      this.x = $$0;
      this.a(new afh($$0));

      for (ard $$1 : this.k.L()) {
         if ($$1 != null) {
            $$1.m().b($$0);
         }
      }
   }

   public List<are> t() {
      return this.l;
   }

   @Nullable
   public are a(UUID $$0) {
      return this.m.get($$0);
   }

   public boolean d(GameProfile $$0) {
      return false;
   }

   public void u() {
      for (ald $$0 : this.s.values()) {
         $$0.a(this.k.aD());
      }

      this.a(new zn(axh.a(this.v)));
      dbm $$1 = this.k.aI();
      aga $$2 = new aga($$1.b(), $$1.d());

      for (are $$3 : this.l) {
         $$3.f.b($$2);
         $$3.J().a($$3);
      }
   }

   public boolean v() {
      return this.y;
   }
}
