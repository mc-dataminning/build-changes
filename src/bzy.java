import java.util.List;
import java.util.stream.Collectors;
import javax.annotation.Nullable;

public class bzy extends cat {
   private static final int i = 2;
   private static final int j = 32;
   private static final int k = 10;
   private static final int l = 7;

   public bzy(bth $$0, double $$1) {
      super($$0, $$1, 240, false);
   }

   @Nullable
   @Override
   protected evz h() {
      float $$0 = this.b.dP().z.i();
      if (this.b.dP().z.i() < 0.3F) {
         return this.k();
      } else {
         evz $$1;
         if ($$0 < 0.7F) {
            $$1 = this.l();
            if ($$1 == null) {
               $$1 = this.m();
            }
         } else {
            $$1 = this.m();
            if ($$1 == null) {
               $$1 = this.l();
            }
         }

         return $$1 == null ? this.k() : $$1;
      }
   }

   @Nullable
   private evz k() {
      return cdw.a(this.b, 10, 7);
   }

   @Nullable
   private evz l() {
      aqk $$0 = (aqk)this.b.dP();
      List<clu> $$1 = $$0.a(bsj.bj, this.b.cK().g(32.0), this::a);
      if ($$1.isEmpty()) {
         return null;
      } else {
         clu $$2 = $$1.get(this.b.dP().z.a($$1.size()));
         evz $$3 = $$2.dn();
         return cdw.a(this.b, 10, 7, $$3);
      }
   }

   @Nullable
   private evz m() {
      kc $$0 = this.n();
      if ($$0 == null) {
         return null;
      } else {
         ja $$1 = this.a($$0);
         return $$1 == null ? null : cdw.a(this.b, 10, 7, evz.c($$1));
      }
   }

   @Nullable
   private kc n() {
      aqk $$0 = (aqk)this.b.dP();
      List<kc> $$1 = kc.a(kc.a(this.b), 2).filter($$1x -> $$0.b($$1x) == 0).collect(Collectors.toList());
      return $$1.isEmpty() ? null : $$1.get($$0.z.a($$1.size()));
   }

   @Nullable
   private ja a(kc $$0) {
      aqk $$1 = (aqk)this.b.dP();
      cec $$2 = $$1.y();
      List<ja> $$3 = $$2.c($$0x -> true, $$0.q(), 8, cec.b.b).map(ced::f).collect(Collectors.toList());
      return $$3.isEmpty() ? null : $$3.get($$1.z.a($$3.size()));
   }

   private boolean a(clu $$0) {
      return $$0.a(this.b.dP().Z());
   }
}
