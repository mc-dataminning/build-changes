import com.google.common.collect.ImmutableList;
import com.google.common.collect.Lists;
import java.util.List;
import java.util.Optional;

public class eux extends eui {
   private static final sw c = sw.c("options.graphics.fabulous").a(n.u);
   private static final sw k = sw.a("options.graphics.warning.message", c, c);
   private static final sw l = sw.c("options.graphics.warning.title").a(n.m);
   private static final sw m = sw.c("options.graphics.warning.accept");
   private static final sw n = sw.c("options.graphics.warning.cancel");
   private eqd o;
   private final fjr p;
   private final int q;

   private static enq<?>[] a(enr $$0) {
      return new enq[]{
         $$0.i(),
         $$0.d(),
         $$0.k(),
         $$0.e(),
         $$0.j(),
         $$0.g(),
         $$0.J(),
         $$0.W(),
         $$0.al(),
         $$0.z(),
         $$0.ak(),
         $$0.h(),
         $$0.V(),
         $$0.am(),
         $$0.y(),
         $$0.K(),
         $$0.ae(),
         $$0.f(),
         $$0.af(),
         $$0.aa(),
         $$0.ah(),
         $$0.ai()
      };
   }

   public eux(euq $$0, enr $$1) {
      super($$0, $$1, sw.c("options.videoTitle"));
      this.p = $$0.f.ag();
      this.p.i();
      if ($$1.i().c() == eng.c) {
         this.p.e();
      }

      this.q = $$1.y().c();
   }

   @Override
   protected void b() {
      this.o = new eqd(this.f, this.g, this.h, 32, this.h - 32, 25);
      int $$0 = -1;
      ehn $$1 = this.f.aM();
      ehi $$2 = $$1.t();
      int $$3;
      if ($$2 == null) {
         $$3 = -1;
      } else {
         Optional<ehm> $$4 = $$1.f();
         $$3 = $$4.<Integer>map($$2::a).orElse(-1);
      }

      enq<Integer> $$6 = new enq<>("options.fullscreen.resolution", enq.a(), ($$1x, $$2x) -> {
         if ($$2 == null) {
            return sw.c("options.fullscreen.unavailable");
         } else {
            return $$2x == -1 ? enr.a($$1x, sw.c("options.fullscreen.current")) : enr.a($$1x, sw.b($$2.a($$2x).toString()));
         }
      }, new enq.f(-1, $$2 != null ? $$2.e() - 1 : -1), $$3, $$2x -> {
         if ($$2 != null) {
            $$1.a($$2x == -1 ? Optional.empty() : Optional.of($$2.a($$2x)));
         }
      });
      this.o.a($$6);
      this.o.a(this.b.A());
      this.o.a(a(this.b));
      this.e(this.o);
      this.d(epi.a(sv.d, $$1x -> {
         this.f.m.aq();
         $$1.g();
         this.f.a(this.a);
      }).a(this.g / 2 - 100, this.h - 27, 200, 20).a());
   }

   @Override
   public void ax_() {
      if (this.b.y().c() != this.q) {
         this.f.b(this.b.y().c());
         this.f.O();
      }

      super.ax_();
   }

   @Override
   public boolean a(double $$0, double $$1, int $$2) {
      int $$3 = this.b.al().c();
      if (super.a($$0, $$1, $$2)) {
         if (this.b.al().c() != $$3) {
            this.f.a();
         }

         if (this.p.g()) {
            List<sw> $$4 = Lists.newArrayList(new sw[]{k, sv.q});
            String $$5 = this.p.j();
            if ($$5 != null) {
               $$4.add(sv.q);
               $$4.add(sw.a("options.graphics.warning.renderer", $$5).a(n.h));
            }

            String $$6 = this.p.l();
            if ($$6 != null) {
               $$4.add(sv.q);
               $$4.add(sw.a("options.graphics.warning.vendor", $$6).a(n.h));
            }

            String $$7 = this.p.k();
            if ($$7 != null) {
               $$4.add(sv.q);
               $$4.add(sw.a("options.graphics.warning.version", $$7).a(n.h));
            }

            this.f.a(new eum(l, $$4, ImmutableList.of(new eum.a(m, $$0x -> {
               this.b.i().a(eng.c);
               enn.N().f.f();
               this.p.e();
               this.f.a(this);
            }), new eum.a(n, $$0x -> {
               this.p.f();
               this.f.a(this);
            }))));
         }

         return true;
      } else {
         return false;
      }
   }

   @Override
   public boolean a(double $$0, double $$1, double $$2) {
      if (euq.p()) {
         enq<Integer> $$3 = this.b.al();
         int $$4 = $$3.c() + (int)Math.signum($$2);
         if ($$4 != 0) {
            $$3.a($$4);
            if ($$3.c() == $$4) {
               this.f.a();
               return true;
            }
         }

         return false;
      } else {
         return super.a($$0, $$1, $$2);
      }
   }

   @Override
   public void a(eox $$0, int $$1, int $$2, float $$3) {
      this.a($$0, this.o, $$1, $$2, $$3);
   }
}
