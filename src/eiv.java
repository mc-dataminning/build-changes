import java.util.Optional;
import javax.annotation.Nullable;

public abstract class eiv extends ein {
   @Override
   public eio d() {
      return eiq.b;
   }

   @Override
   public eio e() {
      return eiq.c;
   }

   @Override
   public cqh a() {
      return cqp.qy;
   }

   @Override
   public void a(cxb $$0, ib $$1, eip $$2, axd $$3) {
      if (!$$2.b() && !$$2.c(a)) {
         if ($$3.a(64) == 0) {
            $$0.a((double)$$1.u() + 0.5, (double)$$1.v() + 0.5, (double)$$1.w() + 0.5, aty.BC, atz.e, $$3.i() * 0.25F + 0.75F, $$3.i() + 0.5F, false);
         }
      } else if ($$3.a(10) == 0) {
         $$0.a(kc.aj, (double)$$1.u() + $$3.j(), (double)$$1.v() + $$3.j(), (double)$$1.w() + $$3.j(), 0.0, 0.0, 0.0);
      }
   }

   @Nullable
   @Override
   public ka h() {
      return kc.m;
   }

   @Override
   protected boolean a(cxb $$0) {
      return $$0.Z().b(cwx.U);
   }

   @Override
   protected void a(cxc $$0, ib $$1, dnb $$2) {
      dki $$3 = $$2.t() ? $$0.c_($$1) : null;
      dac.a($$2, $$0, $$1, $$3);
   }

   @Override
   public int b(cxe $$0) {
      return 4;
   }

   @Override
   public dnb b(eip $$0) {
      return dae.G.o().a(der.b, Integer.valueOf(e($$0)));
   }

   @Override
   public boolean a(eio $$0) {
      return $$0 == eiq.c || $$0 == eiq.b;
   }

   @Override
   public int c(cxe $$0) {
      return 1;
   }

   @Override
   public int a(cxe $$0) {
      return 5;
   }

   @Override
   public boolean a(eip $$0, cwh $$1, ib $$2, eio $$3, ih $$4) {
      return $$4 == ih.a && !$$3.a(aus.a);
   }

   @Override
   protected float c() {
      return 100.0F;
   }

   @Override
   public Optional<atx> j() {
      return Optional.of(aty.dh);
   }

   public static class a extends eiv {
      @Override
      protected void a(dnc.a<eio, eip> $$0) {
         super.a($$0);
         $$0.a(b);
      }

      @Override
      public int d(eip $$0) {
         return $$0.c(b);
      }

      @Override
      public boolean c(eip $$0) {
         return false;
      }
   }

   public static class b extends eiv {
      @Override
      public int d(eip $$0) {
         return 8;
      }

      @Override
      public boolean c(eip $$0) {
         return true;
      }
   }
}
