import java.util.List;
import java.util.Objects;

public class fzb extends fzf<csx> {
   private static final frd h = new frd(
      aku.b("recipe_book/filter_enabled"),
      aku.b("recipe_book/filter_disabled"),
      aku.b("recipe_book/filter_enabled_highlighted"),
      aku.b("recipe_book/filter_disabled_highlighted")
   );
   private static final wp i = wp.c("gui.recipebook.toggleRecipes.craftable");
   private static final List<fzf.a> j = List.of(
      new fzf.a(fzl.a), new fzf.a(cxl.pL, cxl.pD, dbx.c), new fzf.a(cxl.eO, dbx.a), new fzf.a(cxl.rj, cxl.pb, dbx.d), new fzf.a(cxl.mg, dbx.b)
   );

   public fzb(csx $$0) {
      super($$0, j);
   }

   @Override
   protected boolean a(cut $$0) {
      return this.f.m() == $$0 || this.f.n().contains($$0);
   }

   private boolean b(ddb $$0) {
      int $$1 = this.f.o();
      int $$2 = this.f.p();
      Objects.requireNonNull($$0);

      return switch ($$0) {
         case ddf $$3 -> $$1 >= $$3.b() && $$2 >= $$3.c();
         case ddg $$4 -> $$1 * $$2 >= $$4.b().size();
         default -> false;
      };
   }

   @Override
   protected void a(fzd $$0, ddb $$1, bak $$2) {
      $$0.b(this.f.m(), $$2, $$1.d());
      Objects.requireNonNull($$1);
      switch ($$1) {
         case ddf $$3:
            List<cut> $$4 = this.f.n();
            akf.a(this.f.o(), this.f.p(), $$3.b(), $$3.c(), $$3.f(), ($$3x, $$4x, $$5x, $$6x) -> {
               cut $$7x = $$4.get($$4x);
               $$0.a($$7x, $$2, $$3x);
            });
            break;
         case ddg $$5:
            label15: {
               List<cut> $$6 = this.f.n();
               int $$7 = Math.min($$5.b().size(), $$6.size());

               for (int $$8 = 0; $$8 < $$7; $$8++) {
                  $$0.a($$6.get($$8), $$2, $$5.b().get($$8));
               }
               break label15;
            }
      }
   }

   @Override
   protected void a() {
      this.e.a(h);
   }

   @Override
   protected wp b() {
      return i;
   }

   @Override
   protected void a(fzj $$0, cpw $$1) {
      $$0.a($$1, this::b);
   }
}
