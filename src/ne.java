...
public class ne implements lj {
   ...
   public ne(ll $$0) {
      this.d = $$0.a(ll.b.b, "blockstates");
      this.e = $$0.a(ll.b.b, "models");
   }

   @Override
   public CompletableFuture<?> a(lh $$0) {
      Map<dcv, nf> $$1 = Maps.newHashMap();
      Consumer<nf> $$2 = $$1x -> {
         dcv $$2x = $$1x.a();
         nf $$3x = $$1.put($$2x, $$1x);
         if ($$3x != null) {
      ...
      Map<akf, Supplier<JsonElement>> $$3 = Maps.newHashMap();
      Set<cry> $$4 = Sets.newHashSet();
      BiConsumer<akf, Supplier<JsonElement>> $$5 = ($$1x, $$2x) -> {
         Supplier<JsonElement> $$3x = $$3.put($$1x, $$2x);
         if ($$3x != null) {
            throw new IllegalStateException("Duplicate model definition for " + $$1x);
         }
      };
      Consumer<cry> $$6 = $$4::add;
      new nc($$2, $$5, $$6).a();
      new nd($$5).a();
      List<dcv> $$7 = lc.e.g().stream().filter($$0x -> true).map(Entry::getValue).filter($$1x -> !$$1.containsKey($$1x)).toList();
      if (!$$7.isEmpty()) {
   ...
   private <T> CompletableFuture<?> a(lh $$0, Map<T, ? extends Supplier<JsonElement>> $$1, Function<T, Path> $$2) {
