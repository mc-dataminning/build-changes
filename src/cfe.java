import javax.annotation.Nullable;

public abstract class cfe extends cfd implements cfh {
   private il<clb> c = il.a(36, clb.b);
   @Nullable
   private agg d;
   private long e;

   protected cfe(bkm<?> $$0, crs $$1) {
      super($$0, $$1);
   }

   protected cfe(bkm<?> $$0, double $$1, double $$2, double $$3, crs $$4) {
      super($$0, $$4, $$1, $$2, $$3);
   }

   @Override
   public void a(bjg $$0) {
      super.a($$0);
      this.a($$0, this.dL(), this);
   }

   @Override
   public clb a(int $$0) {
      return this.f_($$0);
   }

   @Override
   public clb a(int $$0, int $$1) {
      return this.b($$0, $$1);
   }

   @Override
   public clb b(int $$0) {
      return this.e_($$0);
   }

   @Override
   public void a(int $$0, clb $$1) {
      this.c($$0, $$1);
   }

   @Override
   public blq a_(int $$0) {
      return this.g_($$0);
   }

   @Override
   public void e() {
   }

   @Override
   public boolean a(cdm $$0) {
      return this.g($$0);
   }

   @Override
   public void a(bki.c $$0) {
      if (!this.dL().B && $$0.a()) {
         bim.a(this.dL(), this, this);
      }

      super.a($$0);
   }

   @Override
   protected void b(rz $$0) {
      super.b($$0);
      this.c($$0);
   }

   @Override
   protected void a(rz $$0) {
      super.a($$0);
      this.b_($$0);
   }

   @Override
   public biq a(cdm $$0, bip $$1) {
      return this.c_($$0);
   }

   @Override
   protected void t() {
      float $$0 = 0.98F;
      if (this.d == null) {
         int $$1 = 15 - cge.b(this);
         $$0 += (float)$$1 * 0.001F;
      }

      if (this.aX()) {
         $$0 *= 0.95F;
      }

      this.f(this.do().d((double)$$0, 0.0, (double)$$0));
   }

   @Override
   public void a() {
      this.f();
   }

   public void a(agg $$0, long $$1) {
      this.d = $$0;
      this.e = $$1;
   }

   @Nullable
   @Override
   public cge createMenu(int $$0, cdl $$1, cdm $$2) {
      if (this.d != null && $$2.N_()) {
         return null;
      } else {
         this.f($$1.m);
         return this.a($$0, $$1);
      }
   }

   protected abstract cge a(int var1, cdl var2);

   @Nullable
   @Override
   public agg B() {
      return this.d;
   }

   @Override
   public void a(@Nullable agg $$0) {
      this.d = $$0;
   }

   @Override
   public long C() {
      return this.e;
   }

   @Override
   public void a(long $$0) {
      this.e = $$0;
   }

   @Override
   public il<clb> E() {
      return this.c;
   }

   @Override
   public void F() {
      this.c = il.a(this.b(), clb.b);
   }
}
