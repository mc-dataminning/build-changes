...
public class beg {
   ...
   private JsonElement a(List<Pair<dhk, beq<bei>>> $$0) {
      JsonObject $$1 = new JsonObject();
      $$1.addProperty("durationNanosTotal", $$0.stream().mapToDouble($$0x -> (double)((beq)$$0x.getSecond()).f().toNanos()).sum());
      JsonArray $$2 = ac.a(new JsonArray(), $$1x -> $$1.add("status", $$1x));

      for (Pair<dhk, beq<bei>> $$3 : $$0) {
         beq<bei> $$4 = (beq<bei>)$$3.getSecond();
         JsonObject $$5 = ac.a(new JsonObject(), $$2::add);
         $$5.addProperty("state", ((dhk)$$3.getFirst()).toString());
   ...
   private JsonElement b(List<beo> $$0) {
      if ($$0.isEmpty()) {
         ...
         Map<Integer, Double> $$4 = bdw.a($$2);
         $$4.forEach(($$1x, $$2x) -> $$1.addProperty("p" + $$1x, $$2x));
         return $$1;
      }
   }

   private JsonElement b(bee $$0) {
      JsonObject $$1 = new JsonObject();
   ...
   private JsonElement a(bem $$0) {
      ...
      $$0.e().forEach($$1x -> {
         JsonObject $$2x = new JsonObject();
         $$2.add($$2x);
         bem.b $$3 = (bem.b)$$1x.getFirst();
         bem.a $$4 = (bem.a)$$1x.getSecond();
   ...
   private JsonElement c(List<bej> $$0) {
      JsonObject $$1 = new JsonObject();
      BiFunction<List<bej>, ToDoubleFunction<bej>, JsonObject> $$2 = ($$0x, $$1x) -> {
         JsonObject $$2x = new JsonObject();
      ...
      $$1.add("jvm", (JsonElement)$$2.apply($$0, bej::a));
      $$1.add("userJvm", (JsonElement)$$2.apply($$0, bej::b));
      $$1.add("system", (JsonElement)$$2.apply($$0, bej::c));
      return $$1;
   }
}
