import com.google.common.collect.ImmutableMap;
import java.util.List;
import java.util.Map;

public class ggc implements gfo<dqr> {
   private static final String a = "stick";
   private static final int b = -988212;
   private static final int c = ayz.h(16);
   private static final float d = 0.6666667F;
   private static final evr e = new evr(0.0, 0.33333334F, 0.046666667F);
   private final Map<dtp, ggc.a> f;
   private final fgp g;

   public ggc(gfp.a $$0) {
      this.f = dtp.a().collect(ImmutableMap.toImmutableMap($$0x -> $$0x, $$1 -> new ggc.a($$0.a(fwz.a($$1)))));
      this.g = $$0.f();
   }

   public void a(dqr $$0, float $$1, ezy $$2, gdo $$3, int $$4, int $$5) {
      dsc $$6 = $$0.n();
      dlz $$7 = (dlz)$$6.b();
      dtp $$8 = dlz.a($$7);
      ggc.a $$9 = this.f.get($$8);
      $$9.b.k = $$6.b() instanceof dmu;
      this.a($$0, $$2, $$3, $$4, $$5, $$6, $$7, $$8, $$9);
   }

   public float b() {
      return 0.6666667F;
   }

   public float c() {
      return 0.6666667F;
   }

   void a(dqr $$0, ezy $$1, gdo $$2, int $$3, int $$4, dsc $$5, dlz $$6, dtp $$7, fuw $$8) {
      $$1.a();
      this.a($$1, -$$6.g($$5), $$5);
      this.a($$1, $$2, $$3, $$4, $$7, $$8);
      this.a($$0.ay_(), $$0.j(), $$1, $$2, $$3, $$0.b(), $$0.c(), true);
      this.a($$0.ay_(), $$0.k(), $$1, $$2, $$3, $$0.b(), $$0.c(), false);
      $$1.b();
   }

   void a(ezy $$0, float $$1, dsc $$2) {
      $$0.a(0.5F, 0.75F * this.b(), 0.5F);
      $$0.a(a.d.rotationDegrees($$1));
      if (!($$2.b() instanceof dmu)) {
         $$0.a(0.0F, -0.3125F, -0.4375F);
      }
   }

   void a(ezy $$0, gdo $$1, int $$2, int $$3, dtp $$4, fuw $$5) {
      $$0.a();
      float $$6 = this.b();
      $$0.b($$6, -$$6, -$$6);
      grd $$7 = this.a($$4);
      fac $$8 = $$7.a($$1, $$5::a);
      this.a($$0, $$2, $$3, $$5, $$8);
      $$0.b();
   }

   void a(ezy $$0, int $$1, int $$2, fuw $$3, fac $$4) {
      ggc.a $$5 = (ggc.a)$$3;
      $$5.a.a($$0, $$4, $$1, $$2);
   }

   grd a(dtp $$0) {
      return ged.a($$0);
   }

   void a(iz $$0, dqs $$1, ezy $$2, gdo $$3, int $$4, int $$5, int $$6, boolean $$7) {
      $$2.a();
      this.a($$2, $$7, this.d());
      int $$8 = a($$1);
      int $$9 = 4 * $$5 / 2;
      ayl[] $$10 = $$1.a(fff.Q().aS(), $$1x -> {
         List<ayl> $$2x = this.g.c($$1x, $$6);
         return $$2x.isEmpty() ? ayl.a : $$2x.get(0);
      });
      int $$11;
      boolean $$12;
      int $$13;
      if ($$1.a()) {
         $$11 = $$1.b().g();
         $$12 = a($$0, $$11);
         $$13 = 15728880;
      } else {
         $$11 = $$8;
         $$12 = false;
         $$13 = $$4;
      }

      for (int $$17 = 0; $$17 < 4; $$17++) {
         ayl $$18 = $$10[$$17];
         float $$19 = (float)(-this.g.a($$18) / 2);
         if ($$12) {
            this.g.a($$18, $$19, (float)($$17 * $$5 - $$9), $$11, $$8, $$2.c().a(), $$3, $$13);
         } else {
            this.g.a($$18, $$19, (float)($$17 * $$5 - $$9), $$11, false, $$2.c().a(), $$3, fgp.a.c, 0, $$13);
         }
      }

      $$2.b();
   }

   private void a(ezy $$0, boolean $$1, evr $$2) {
      if (!$$1) {
         $$0.a(a.d.rotationDegrees(180.0F));
      }

      float $$3 = 0.015625F * this.c();
      $$0.a($$2.c, $$2.d, $$2.e);
      $$0.b($$3, -$$3, $$3);
   }

   evr d() {
      return e;
   }

   static boolean a(iz $$0, int $$1) {
      if ($$1 == cti.p.g()) {
         return true;
      } else {
         fff $$2 = fff.Q();
         gcq $$3 = $$2.s;
         if ($$3 != null && $$2.m.aA().a() && $$3.gA()) {
            return true;
         } else {
            bsu $$4 = $$2.an();
            return $$4 != null && $$4.f(evr.b($$0)) < (double)c;
         }
      }
   }

   public static int a(dqs $$0) {
      int $$1 = $$0.b().g();
      if ($$1 == cti.p.g() && $$0.a()) {
         return -988212;
      } else {
         double $$2 = 0.4;
         int $$3 = (int)((double)ayj.b.b($$1) * 0.4);
         int $$4 = (int)((double)ayj.b.c($$1) * 0.4);
         int $$5 = (int)((double)ayj.b.d($$1) * 0.4);
         return ayj.b.a(0, $$3, $$4, $$5);
      }
   }

   public static ggc.a a(fww $$0, dtp $$1) {
      return new ggc.a($$0.a(fwz.a($$1)));
   }

   public static fxg f() {
      fxi $$0 = new fxi();
      fxj $$1 = $$0.a();
      $$1.a("sign", fxf.c().a(0, 0).a(-12.0F, -14.0F, -1.0F, 24.0F, 12.0F, 2.0F), fxc.a);
      $$1.a("stick", fxf.c().a(0, 14).a(-1.0F, -2.0F, -1.0F, 2.0F, 14.0F, 2.0F), fxc.a);
      return fxg.a($$0, 64, 32);
   }

   public static final class a extends fuw {
      public final fxa a;
      public final fxa b;

      public a(fxa $$0) {
         super(gdw::e);
         this.a = $$0;
         this.b = $$0.b("stick");
      }

      @Override
      public void a(ezy $$0, fac $$1, int $$2, int $$3, float $$4, float $$5, float $$6, float $$7) {
         this.a.a($$0, $$1, $$2, $$3, $$4, $$5, $$6, $$7);
      }
   }
}
