import com.mojang.serialization.MapCodec;
import javax.annotation.Nullable;

public class ddb extends cut {
   public static final MapCodec<ddb> a = b(ddb::new);
   public static final dhn b = dhm.B;

   @Override
   public MapCodec<ddb> a() {
      return a;
   }

   public ddb(dgv.d $$0) {
      super($$0);
      this.k(this.o().a(b, Boolean.valueOf(false)));
   }

   @Override
   public void b(dgw $$0, crs $$1, ht $$2, dgw $$3, boolean $$4) {
      if (!$$3.a($$0.b())) {
         if ($$1.B($$2)) {
            a($$1, $$2);
            $$1.a($$2, false);
         }
      }
   }

   @Override
   public void a(dgw $$0, crs $$1, ht $$2, cut $$3, ht $$4, boolean $$5) {
      if ($$1.B($$2)) {
         a($$1, $$2);
         $$1.a($$2, false);
      }
   }

   @Override
   public dgw a(crs $$0, ht $$1, dgw $$2, cdm $$3) {
      if (!$$0.x_() && !$$3.f() && $$2.c(b)) {
         a($$0, $$1);
      }

      return super.a($$0, $$1, $$2, $$3);
   }

   @Override
   public void a(crs $$0, ht $$1, crk $$2) {
      if (!$$0.B) {
         cag $$3 = new cag($$0, (double)$$1.u() + 0.5, (double)$$1.v(), (double)$$1.w() + 0.5, $$2.e());
         int $$4 = $$3.s();
         $$3.b((short)($$0.z.a($$4 / 4) + $$4 / 8));
         $$0.b($$3);
      }
   }

   public static void a(crs $$0, ht $$1) {
      a($$0, $$1, null);
   }

   private static void a(crs $$0, ht $$1, @Nullable bky $$2) {
      if (!$$0.B) {
         cag $$3 = new cag($$0, (double)$$1.u() + 0.5, (double)$$1.v(), (double)$$1.w() + 0.5, $$2);
         $$0.b($$3);
         $$0.a(null, $$3.dq(), $$3.ds(), $$3.dw(), aqn.xP, aqo.e, 1.0F, 1.0F);
         $$0.a($$2, dlg.I, $$1);
      }
   }

   @Override
   public biq a(dgw $$0, crs $$1, ht $$2, cdm $$3, bip $$4, eje $$5) {
      clb $$6 = $$3.b($$4);
      if (!$$6.a(cle.nE) && !$$6.a(cle.tg)) {
         return super.a($$0, $$1, $$2, $$3, $$4, $$5);
      } else {
         a($$1, $$2, $$3);
         $$1.a($$2, cuv.a.o(), 11);
         ckw $$7 = $$6.d();
         if (!$$3.f()) {
            if ($$6.a(cle.nE)) {
               $$6.a(1, $$3, $$1x -> $$1x.d($$4));
            } else {
               $$6.h(1);
            }
         }

         $$3.b(aqx.c.b($$7));
         return biq.a($$1.B);
      }
   }

   @Override
   public void a(crs $$0, dgw $$1, eje $$2, cee $$3) {
      if (!$$0.B) {
         ht $$4 = $$2.a();
         bki $$5 = $$3.v();
         if ($$3.bM() && $$3.a($$0, $$4)) {
            a($$0, $$4, $$5 instanceof bky ? (bky)$$5 : null);
            $$0.a($$4, false);
         }
      }
   }

   @Override
   public boolean a(crk $$0) {
      return false;
   }

   @Override
   protected void a(dgx.a<cut, dgw> $$0) {
      $$0.a(b);
   }
}
