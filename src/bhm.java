...
public class bhm extends DataFix {
   private static final Int2ObjectMap<String> a = af.a(new Int2ObjectOpenHashMap(), $$0 -> {
   ...
   private static <T> Dynamic<T> a(Dynamic<T> $$0) {
      ...
      Optional<Dynamic<T>> $$1 = $$0.get("HiddenEffect").result().map(bhm::a);
      return $$0.replaceField("HiddenEffect", "hidden_effect", $$1);
   }

   private static <T> Dynamic<T> b(Dynamic<T> $$0, String $$1, String $$2) {
      Optional<Dynamic<T>> $$3 = $$0.get($$1).asStreamOpt().result().map($$1x -> $$0.createList($$1x.map(bhm::a)));
   ...
   private TypeRewriteRule a() {
      Type<?> $$0 = this.getInputSchema().getType(bit.s);
      return this.fixTypeEverywhereTyped("BlockEntityMobEffectIdFix", $$0, $$0x -> this.a($$0x, bit.s, "minecraft:beacon", $$0xx -> {
   ...
   private TypeRewriteRule b() {
      ...
      return this.fixTypeEverywhereTyped("EntityMobEffectIdFix", $$0, $$0x -> {
         $$0x = this.a($$0x, bit.D, "minecraft:mooshroom", bhm::c);
         $$0x = this.a($$0x, bit.D, "minecraft:arrow", bhm::d);
         $$0x = this.a($$0x, bit.D, "minecraft:area_effect_cloud", bhm::e);
         return $$0x.update(DSL.remainderFinder(), bhm::f);
      });
   }

   private TypeRewriteRule c() {
      Type<?> $$0 = this.getInputSchema().getType(bit.b);
      return this.fixTypeEverywhereTyped("PlayerMobEffectIdFix", $$0, $$0x -> $$0x.update(DSL.remainderFinder(), bhm::f));
   }

   private static <T> Dynamic<T> g(Dynamic<T> $$0) {
      Optional<Dynamic<T>> $$1 = $$0.get("Effects").asStreamOpt().result().map($$1x -> $$0.createList($$1x.map(bhm::b)));
      return $$0.replaceField("Effects", "effects", $$1);
   }

   private TypeRewriteRule d() {
      OpticFinder<Pair<String, String>> $$0 = DSL.fieldFinder("id", DSL.named(bit.F.typeName(), bko.a()));
      ...
      return this.fixTypeEverywhereTyped("ItemStackMobEffectIdFix", $$1, $$2x -> {
         ...
         if ($$3.isPresent()) {
            ...
            if ($$4.equals("minecraft:suspicious_stew")) {
               return $$2x.updateTyped($$2, $$0xx -> $$0xx.update(DSL.remainderFinder(), bhm::g));
            }
