...
public class lx implements ki {
   private final kk.a d;
   private final kk.a e;

   public lx(kk $$0) {
      this.d = $$0.a(kk.b.b, "blockstates");
      this.e = $$0.a(kk.b.b, "models");
   }

   @Override
   public CompletableFuture<?> a(kg $$0) {
      Map<cvz, ly> $$1 = Maps.newHashMap();
      Consumer<ly> $$2 = $$1x -> {
         cvz $$2x = $$1x.a();
         ly $$3x = $$1.put($$2x, $$1x);
         if ($$3x != null) {
      ...
      Map<agt, Supplier<JsonElement>> $$3 = Maps.newHashMap();
      Set<cmc> $$4 = Sets.newHashSet();
      BiConsumer<agt, Supplier<JsonElement>> $$5 = ($$1x, $$2x) -> {
         Supplier<JsonElement> $$3x = $$3.put($$1x, $$2x);
         if ($$3x != null) {
            throw new IllegalStateException("Duplicate model definition for " + $$1x);
         }
      };
      Consumer<cmc> $$6 = $$4::add;
      new lv($$2, $$5, $$6).a();
      new lw($$5).a();
      List<cvz> $$7 = kb.e.g().stream().filter($$0x -> true).map(Entry::getValue).filter($$1x -> !$$1.containsKey($$1x)).toList();
      if (!$$7.isEmpty()) {
   ...
   private <T> CompletableFuture<?> a(kg $$0, Map<T, ? extends Supplier<JsonElement>> $$1, Function<T, Path> $$2) {
