import java.util.EnumSet;
import java.util.List;
import javax.annotation.Nullable;

public abstract class cbj extends cbi {
   @Nullable
   private ht b;
   private boolean c;
   private boolean d;

   protected cbj(bku<? extends cbj> $$0, csa $$1) {
      super($$0, $$1);
   }

   @Override
   protected void B() {
      super.B();
      this.bO.a(4, new cbj.a<>(this, 0.7, 0.595));
   }

   @Override
   public void b(rz $$0) {
      super.b($$0);
      if (this.b != null) {
         $$0.a("PatrolTarget", so.a(this.b));
      }

      $$0.a("PatrolLeader", this.c);
      $$0.a("Patrolling", this.d);
   }

   @Override
   public void a(rz $$0) {
      super.a($$0);
      if ($$0.e("PatrolTarget")) {
         this.b = so.b($$0.p("PatrolTarget"));
      }

      this.c = $$0.q("PatrolLeader");
      this.d = $$0.q("Patrolling");
   }

   public boolean gg() {
      return true;
   }

   @Nullable
   @Override
   public blz a(csp $$0, biv $$1, blk $$2, @Nullable blz $$3, @Nullable rz $$4) {
      if ($$2 != blk.p && $$2 != blk.h && $$2 != blk.d && $$0.E_().i() < 0.06F && this.gg()) {
         this.c = true;
      }

      if (this.gj()) {
         this.a(bkv.f, cfb.s());
         this.a(bkv.f, 2.0F);
      }

      if ($$2 == blk.p) {
         this.d = true;
      }

      return super.a($$0, $$1, $$2, $$3, $$4);
   }

   public static boolean b(bku<? extends cbj> $$0, csb $$1, blk $$2, ht $$3, ats $$4) {
      return $$1.a(csj.b, $$3) > 8 ? false : c($$0, $$1, $$2, $$3, $$4);
   }

   @Override
   public boolean h(double $$0) {
      return !this.d || $$0 > 16384.0;
   }

   public void i(ht $$0) {
      this.b = $$0;
      this.d = true;
   }

   public ht gh() {
      return this.b;
   }

   public boolean gi() {
      return this.b != null;
   }

   public void w(boolean $$0) {
      this.c = $$0;
      this.d = true;
   }

   public boolean gj() {
      return this.c;
   }

   public boolean gm() {
      return true;
   }

   public void gn() {
      this.b = this.dn().b(-500 + this.ag.a(1000), 0, -500 + this.ag.a(1000));
      this.d = true;
   }

   protected boolean go() {
      return this.d;
   }

   protected void x(boolean $$0) {
      this.d = $$0;
   }

   public static class a<T extends cbj> extends bsb {
      private static final int a = 200;
      private final T b;
      private final double c;
      private final double d;
      private long e;

      public a(T $$0, double $$1, double $$2) {
         this.b = $$0;
         this.c = $$1;
         this.d = $$2;
         this.e = -1L;
         this.a(EnumSet.of(bsb.a.a));
      }

      @Override
      public boolean a() {
         boolean $$0 = this.b.dN().W() < this.e;
         return this.b.go() && this.b.q() == null && !this.b.cP() && this.b.gi() && !$$0;
      }

      @Override
      public void c() {
      }

      @Override
      public void d() {
      }

      @Override
      public void e() {
         boolean $$0 = this.b.gj();
         bup $$1 = this.b.N();
         if ($$1.l()) {
            List<cbj> $$2 = this.h();
            if (this.b.go() && $$2.isEmpty()) {
               this.b.x(false);
            } else if ($$0 && this.b.gh().a(this.b.dl(), 10.0)) {
               this.b.gn();
            } else {
               eju $$3 = eju.c(this.b.gh());
               eju $$4 = this.b.dl();
               eju $$5 = $$4.d($$3);
               $$3 = $$5.b(90.0F).a(0.4).e($$3);
               eju $$6 = $$3.d($$4).d().a(10.0).e($$4);
               ht $$7 = ht.a($$6);
               $$7 = this.b.dN().a(dmr.a.f, $$7);
               if (!$$1.a((double)$$7.u(), (double)$$7.v(), (double)$$7.w(), $$0 ? this.d : this.c)) {
                  this.i();
                  this.e = this.b.dN().W() + 200L;
               } else if ($$0) {
                  for (cbj $$8 : $$2) {
                     $$8.i($$7);
                  }
               }
            }
         }
      }

      private List<cbj> h() {
         return this.b.dN().a(cbj.class, this.b.cH().g(16.0), $$0 -> $$0.gm() && !$$0.t(this.b));
      }

      private boolean i() {
         ats $$0 = this.b.eh();
         ht $$1 = this.b.dN().a(dmr.a.f, this.b.dn().b(-8 + $$0.a(16), 0, -8 + $$0.a(16)));
         return this.b.N().a((double)$$1.u(), (double)$$1.v(), (double)$$1.w(), this.c);
      }
   }
}
