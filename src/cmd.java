import javax.annotation.Nullable;

public abstract class cmd extends cmo {
   public double b;
   public double c;
   public double d;

   protected cmd(bsa<? extends cmd> $$0, dax $$1) {
      super($$0, $$1);
   }

   protected cmd(bsa<? extends cmd> $$0, double $$1, double $$2, double $$3, dax $$4) {
      this($$0, $$4);
      this.a_($$1, $$2, $$3);
   }

   public cmd(bsa<? extends cmd> $$0, double $$1, double $$2, double $$3, double $$4, double $$5, double $$6, dax $$7) {
      this($$0, $$7);
      this.b($$1, $$2, $$3, this.dF(), this.dH());
      this.at();
      this.q($$4, $$5, $$6);
   }

   public cmd(bsa<? extends cmd> $$0, bso $$1, double $$2, double $$3, double $$4, dax $$5) {
      this($$0, $$1.du(), $$1.dw(), $$1.dA(), $$2, $$3, $$4, $$5);
      this.c($$1);
      this.a($$1.dF(), $$1.dH());
   }

   @Override
   protected void a(ajv.a $$0) {
   }

   @Override
   public boolean a(double $$0) {
      double $$1 = this.cK().a() * 4.0;
      if (Double.isNaN($$1)) {
         $$1 = 4.0;
      }

      $$1 *= 64.0;
      return $$0 < $$1 * $$1;
   }

   protected dag.a af_() {
      return dag.a.a;
   }

   @Override
   public void l() {
      bru $$0 = this.s();
      if (this.dP().B || ($$0 == null || !$$0.dK()) && this.dP().B(this.dp())) {
         super.l();
         if (this.u()) {
            this.g(1);
         }

         eui $$1 = cmq.a(this, this::b, this.af_());
         if ($$1.c() != eui.a.a) {
            this.a($$1);
         }

         this.aS();
         euk $$2 = this.ds();
         double $$3 = this.du() + $$2.c;
         double $$4 = this.dw() + $$2.d;
         double $$5 = this.dA() + $$2.e;
         cmq.a(this, 0.2F);
         float $$8;
         if (this.be()) {
            for (int $$6 = 0; $$6 < 4; $$6++) {
               float $$7 = 0.25F;
               this.dP().a(ky.d, $$3 - $$2.c * 0.25, $$4 - $$2.d * 0.25, $$5 - $$2.e * 0.25, $$2.c, $$2.d, $$2.e);
            }

            $$8 = this.x();
         } else {
            $$8 = this.w();
         }

         this.g($$2.b(this.b, this.c, this.d).a((double)$$8));
         kw $$10 = this.v();
         if ($$10 != null) {
            this.dP().a($$10, $$3, $$4 + 0.5, $$5, 0.0, 0.0, 0.0);
         }

         this.a_($$3, $$4, $$5);
      } else {
         this.ao();
      }
   }

   @Override
   protected boolean b(bru $$0) {
      return super.b($$0) && !$$0.ag;
   }

   @Override
   protected boolean u() {
      return true;
   }

   @Nullable
   protected kw v() {
      return ky.ae;
   }

   protected float w() {
      return 0.95F;
   }

   protected float x() {
      return 0.8F;
   }

   @Override
   public void b(ud $$0) {
      super.b($$0);
      $$0.a("power", this.a(new double[]{this.b, this.c, this.d}));
   }

   @Override
   public void a(ud $$0) {
      super.a($$0);
      if ($$0.b("power", 9)) {
         uj $$1 = $$0.c("power", 6);
         if ($$1.size() == 3) {
            this.b = $$1.h(0);
            this.c = $$1.h(1);
            this.d = $$1.h(2);
         }
      }
   }

   @Override
   public boolean bz() {
      return true;
   }

   @Override
   public float bK() {
      return 1.0F;
   }

   @Override
   public boolean a(bqn $$0, float $$1) {
      if (this.b($$0)) {
         return false;
      } else {
         this.bv();
         bru $$2 = $$0.d();
         if ($$2 != null) {
            if (!this.dP().B) {
               euk $$3 = $$2.bL();
               this.g($$3);
               this.b = $$3.c * 0.1;
               this.c = $$3.d * 0.1;
               this.d = $$3.e * 0.1;
               this.c($$2);
            }

            return true;
         } else {
            return false;
         }
      }
   }

   @Override
   public float bt() {
      return 1.0F;
   }

   @Override
   public ze<abq> dl() {
      bru $$0 = this.s();
      int $$1 = $$0 == null ? 0 : $$0.al();
      return new abr(this.al(), this.cz(), this.du(), this.dw(), this.dA(), this.dH(), this.dF(), this.ak(), $$1, new euk(this.b, this.c, this.d), 0.0);
   }

   @Override
   public void a(abr $$0) {
      super.a($$0);
      double $$1 = $$0.j();
      double $$2 = $$0.k();
      double $$3 = $$0.l();
      this.q($$1, $$2, $$3);
   }

   private void q(double $$0, double $$1, double $$2) {
      double $$3 = Math.sqrt($$0 * $$0 + $$1 * $$1 + $$2 * $$2);
      if ($$3 != 0.0) {
         this.b = $$0 / $$3 * 0.1;
         this.c = $$1 / $$3 * 0.1;
         this.d = $$2 / $$3 * 0.1;
      }
   }
}
