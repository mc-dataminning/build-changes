...
public class SpearAnimations {
   ...
   public static <T extends HumanoidRenderState> void thirdPersonHandUse(
      final ModelPart arm, final ModelPart head, final HumanoidArm holdingArm, final ItemStack item, final T state
   ) {
      ...
      if (!(state.ticksUsingItem <= 0.0F) && (!state.isUsingItem || state.useItemHand.asArm(state.mainArm) == holdingArm)) {
         ...
         if (kineticWeapon != null) {
            ...
            arm.yRot = arm.yRot + (float)(-invert) * params.swayScaleFast() * (float) (Math.PI / 180.0) * params.swayIntensity() * 1.0F;
            arm.zRot = arm.zRot + (float)(-invert) * params.swayScaleSlow() * (float) (Math.PI / 180.0) * params.swayIntensity() * 0.5F;
            arm.xRot = arm.xRot
               + (float) (Math.PI / 180.0)
                  * (
   ...
   public static <S extends ArmedEntityRenderState> void thirdPersonUseItem(
      ...
      if (kineticWeapon != null && timeHeld != 0.0F) {
         ...
         poseStack.translate(
            0.0, (double)(-hitFeedback) * 0.4, (double)(-kineticWeapon.forwardMovement() * (raiseProgressModified - params.raiseBackProgress()) + hitFeedback)
         );
         ...
         poseStack.rotateAround(
            Axis.YP.rotationDegrees((float)(invert * 90) * (params.raiseProgress() - params.swayProgress() + 3.0F * retract + attack)), 0.0F, 0.0F, 0.125F
         );
      }
   }
   ...
   public static void firstPersonUse(
      ...
      if (kineticWeapon != null) {
         ...
         poseStack.translate(
            ...
            (double)params.raiseProgressStart() * 0.05 + (double)params.raiseProgressEnd() * -0.05 + (double)(params.swayScaleSlow() * 0.005F)
         );
         poseStack.rotateAround(
            Axis.XP
               .rotationDegrees(
         ...
         poseStack.rotateAround(
            Axis.YN
               .rotationDegrees(
                  (float)invert * (-90.0F * progress(params.raiseProgress(), 0.5F, 0.55F) + 90.0F * params.swayProgress() + 2.0F * params.swayScaleSlow())
               ),
            (float)invert * 0.15F,
            0.0F,
            0.0F
   ...
   public static void firstPersonAttack(final float attack, final PoseStack poseStack, final int invert, final HumanoidArm arm) {
      ...
      poseStack.translate(
         (float)invert * 0.1F * (startingAmount - middleAmount), -0.075F * (startingAmount - endingAmount), 0.65F * (startingAmount - middleAmount)
      );
   ...
   private static record UseParams(
      float raiseProgress,
      ...
      public static SpearAnimations.UseParams fromKineticWeapon(final KineticWeapon kineticWeapon, final float time) {
         ...
         float swayScaleSlow = Mth.sin((double)(time * 19.0F * (float) (Math.PI / 180.0))) * swayIntensity;
         float swayScaleFast = Mth.sin((double)(time * 30.0F * (float) (Math.PI / 180.0))) * swayIntensity;
