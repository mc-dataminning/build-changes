...
public class SkeletonModel<S extends SkeletonRenderState> extends HumanoidModel<S> {
   ...
   public void setupAnim(final S state) {
      ...
      if (state.isAggressive && !state.isHoldingBow) {
         ...
         float swing2 = Mth.sin((double)(swingAnimation * (float) Math.PI));
         float swing = Mth.sin((double)((1.0F - (1.0F - swingAnimation) * (1.0F - swingAnimation)) * (float) Math.PI));
         ...
         this.rightArm.xRot -= swing2 * 1.2F - swing * 0.4F;
         this.leftArm.xRot -= swing2 * 1.2F - swing * 0.4F;
