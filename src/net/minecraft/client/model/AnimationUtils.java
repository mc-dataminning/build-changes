...
public class AnimationUtils {
   ...
   public static void bobModelPart(final ModelPart modelPart, final float ageInTicks, final float scale) {
      modelPart.zRot = modelPart.zRot + scale * (Mth.cos((double)(ageInTicks * 0.09F)) * 0.05F + 0.05F);
      modelPart.xRot = modelPart.xRot + scale * Mth.sin((double)(ageInTicks * 0.067F)) * 0.05F;
   }
   ...
   public static <T extends UndeadRenderState> void animateZombieArms(
      ...
      if (!usingSpear) {
         ...
         float attackYRotModifier = (raiseArms ? 1.0F : -1.0F) * Mth.sin((double)(state.swingAnimation * (float) Math.PI));
         float attackXRotModifier = Mth.sin((double)((1.0F - (1.0F - state.swingAnimation) * (1.0F - state.swingAnimation)) * (float) Math.PI));
