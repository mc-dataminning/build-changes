...
public class AdvancementWidget {
   ...
   private AdvancementWidget(final Minecraft minecraft, final AdvancementNode advancementNode, final DisplayInfo display) {
      ...
      this.description = Language.getInstance()
         .getVisualOrder(
            this.findOptimalLines(ComponentUtils.mergeStyles(display.description(), Style.EMPTY.withColor(display.type().getChatColor())), longestDescLine)
         );
   ...
   @Nullable
   public static AdvancementWidget createWidget(final Minecraft minecraft, final AdvancementNode node) {
      Optional<DisplayInfo> display = node.advancement().display();
      return display.isEmpty() ? null : new AdvancementWidget(minecraft, node, display.get());
   }
   ...
   @Nullable
   private static AdvancementHolder findFirstVisibleParent(AdvancementNode node) {
      do {
         node = node.parent();
      } while (node != null && node.advancement().display().isEmpty());

      return node != null && !node.advancement().display().isEmpty() ? node.holder() : null;
   }
   ...
   public void extractHover(
      final GuiGraphicsExtractor graphics, final int xo, final int yo, final float fade, final int screenxo, final int screenyo, final int screenWidth
   ) {
