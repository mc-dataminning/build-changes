...
public class WorldOptionsScreen extends Screen implements HasGamemasterPermissionReaction, HasDifficultyReaction {
   ...
   private static final Component GENERAL_TITLE = Component.translatable("options.worldOptions.general.title")
   ...
   private static final Tooltip ALLOW_COMMANDS_DISABLED_HARDCORE_TOOLTIP = Tooltip.create(
      Component.translatable("options.worldOptions.allow_commands.disabled.tooltip")
   );
   private static final Tooltip ALLOW_COMMANDS_DISABLED_DEMO_TOOLTIP = Tooltip.create(
      Component.translatable("options.worldOptions.allow_commands.disabled.demo.tooltip")
   );
   private static final Component RESTRICTIONS = Component.translatable("restrictions_screen.button");
   private static final Component MULTIPLAYER_TITLE = Component.translatable("options.worldOptions.multiplayer.title")
      .withStyle(ChatFormatting.UNDERLINE, ChatFormatting.BOLD);
   ...
   private static final Tooltip GUEST_COMMAND_ACCESS_DISABLED_MULTIPLAYER_SCOPE_OFF = Tooltip.create(
      Component.translatable("options.worldOptions.guest.command_access.disabled.scope.tooltip")
   );
   private static final Tooltip GUEST_COMMAND_ACCESS_DISABLED_COMMANDS_OFF = Tooltip.create(
      Component.translatable("options.worldOptions.guest.command_access.disabled.commands.tooltip")
   );
   ...
   private static final Tooltip FORCE_GAME_MODE_OFF_MULTIPLAYER_TOOLTIP = Tooltip.create(
      Component.translatable("options.worldOptions.guest.force_game_mode.off.scope.tooltip")
   );
   private static final Tooltip FORCE_GAME_MODE_OFF_HARDCORE_TOOLTIP = Tooltip.create(
      Component.translatable("options.worldOptions.guest.force_game_mode.off.hardcore.tooltip")
   );
   private static final Tooltip FORCE_GAME_MODE_OFF_COMMANDS_TOOLTIP = Tooltip.create(
      Component.translatable("options.worldOptions.guest.force_game_mode.off.commands.tooltip")
   );
   ...
   @Nullable
   private GameType initialDefaultGameMode;
   @Nullable
   private GameType wantedDefaultGameMode;
   @Nullable
   private GameType initialPersonalGameMode;
   @Nullable
   private GameType wantedPersonalGameMode;
   @Nullable
   ...
   @Nullable
   private Boolean initialGuestCommandAccess;
   @Nullable
   private Boolean wantedGuestCommandAccess;
   @Nullable
   private CycleButton<Boolean> guestCommandAccessButton;
   @Nullable
   private Boolean initialForceGameMode;
   @Nullable
   private Boolean wantedForceGameMode;
   @Nullable
            ...
            var2x -> {
               if (this.wantedDifficultyLocked != this.initialDifficultyLocked) {
                  ...
                  this.minecraft
                     .gui
                     .setScreen(
                        new PopupScreen.Builder(this, DIFFICULTY_LOCK_TITLE)
                           .addMessage(Component.translatable("difficulty.lock.question", difficultyDisplayName))
                           .addButton(CommonComponents.GUI_YES, var2xx -> {
                              this.applyChanges(singleplayerServer);
                              this.minecraft.gui.setScreen(this.lastScreen);
                           })
                           .addButton(CommonComponents.GUI_NO, var1x -> this.minecraft.gui.setScreen(this))
                           .build()
                     );
               } else {
                  this.applyChanges(singleplayerServer);
   ...
   private void generalOptions(final LinearLayout content, @Nullable final IntegratedServer singleplayerServer) {
      ...
      if (host) {
         ...
         this.defaultGameModeButton = rowHelper.addChild(
            this.createGameModeButton(
               singleplayerServer, DEFAULT_GAME_MODE, DEFAULT_GAME_MODE_TOOLTIP, this.wantedDefaultGameMode, value -> this.wantedDefaultGameMode = value
            ),
            2
         );
         ...
         if (this.wantedPersonalGameMode != null) {
            this.personalGameModeButton = rowHelper.addChild(
               this.createGameModeButton(
                  singleplayerServer, PERSONAL_GAME_MODE, PERSONAL_GAME_MODE_TOOLTIP, this.wantedPersonalGameMode, value -> this.wantedPersonalGameMode = value
               ),
               2
            );
         }
   ...
   private Button createGameRulesButton(@Nullable final IntegratedServer singleplayerServer) {
   ...
   private CycleButton<GameType> createGameModeButton(
      final IntegratedServer singleplayerServer,
      final Component buttonName,
      final Tooltip tooltip,
      final GameType defaultValue,
      final Consumer<GameType> gameTypeToChange
   ) {
      CycleButton<GameType> gameModeButton = CycleButton.builder(GameType::getShortDisplayName, defaultValue)
         .withValues(GameType.values())
         .withTooltip(var1 -> tooltip)
         .create(0, 0, 308, 20, buttonName, (var2, value) -> {
   ...
   private CycleButton<Boolean> createAllowCommandsButton(final IntegratedServer singleplayerServer) {
      CycleButton<Boolean> allowCommandsButton = CycleButton.onOffBuilder(singleplayerServer.getWorldData().isAllowCommands())
         .withTooltip(var0 -> ALLOW_COMMANDS_TOOLTIP)
         .create(ALLOW_COMMANDS, (var2x, allowCommands) -> {
      ...
      } else if (!singleplayerServer.isHardcore()
   ...
   private void updateButton(
      ...
      @Nullable final IntegratedServer singleplayerServer,
      @Nullable final Tooltip tooltip,
      ...
      if (widget != null) {
         ...
         boolean shouldButtonsBeActive = this.minecraft.player != null
            && (
               this.wantedAllowCommands == null ? this.minecraft.player.permissions().hasPermission(Permissions.COMMANDS_GAMEMASTER) : this.wantedAllowCommands
            );
   ...
   private void multiplayerOptions(final LinearLayout content, final IntegratedServer singleplayerServer) {
      ...
      helper.addChild(
         CycleButton.onOffBuilder(singleplayerServer.getMultiplayerScope() == MinecraftServer.MultiplayerScope.LAN)
            .withTooltip(value -> Tooltip.create(value ? MinecraftServer.MultiplayerScope.LAN.getTooltip() : MinecraftServer.MultiplayerScope.OFF.getTooltip()))
            .create(Component.translatable("menu.multiplayerOptions.lan"), (var2, value) -> {
      ...
      this.guestCommandAccessButton = helper.addChild(
         CycleButton.onOffBuilder(this.initialGuestCommandAccess)
            .withTooltip(var0 -> GUEST_COMMAND_ACCESS_TOOLTIP)
            .create(GUEST_COMMAND_ACCESS, (var2, value) -> {
      ...
      this.forceGameModeButton = helper.addChild(
         CycleButton.onOffBuilder(this.initialForceGameMode)
            .withTooltip(value -> value ? FORCE_GAME_MODE_ON_TOOLTIP : FORCE_GAME_MODE_OFF_TOOLTIP)
            .create(FORCE_GAME_MODE, (var1, value) -> {
   ...
   protected void applyChanges(@Nullable final IntegratedServer singleplayerServer) {
      ...
      if (this.wantedDifficultyLocked != null && this.wantedDifficultyLocked != this.initialDifficultyLocked && this.minecraft.getConnection() != null) {
         ...
         WorldOptionsScreen.DifficultyButtons.updateDifficultyButtonsState(
   ...
   protected void extractMenuBackground(final GuiGraphicsExtractor graphics) {
      ...
      graphics.blit(
         ...
         (float)this.width,
         (float)(this.height - this.layout.getFooterHeight() + (int)this.scrollArea.getScrollAmount()),
         this.width,
   ...
   public void extractRenderState(final GuiGraphicsExtractor graphics, final int xm, final int ym, final float a) {
      ...
      graphics.blit(
         RenderPipelines.GUI_TEXTURED, Screen.INWORLD_HEADER_SEPARATOR, this.layout.getX(), this.layout.getHeaderHeight() - 2, 0.0F, 0.0F, this.width, 2, 32, 2
      );
      graphics.blit(
         RenderPipelines.GUI_TEXTURED,
   ...
   private void updatePermissionDependentButtons(
      @Nullable final IntegratedServer singleplayerServer, final boolean allowCommands, final boolean affectGameRulesButton
   ) {
      if (!allowCommands) {
      ...
      this.updateButton(
         this.defaultGameModeButton, singleplayerServer, DEFAULT_GAME_MODE_TOOLTIP, GAME_MODE_DISABLED_OPERATOR_TOOLTIP, GAME_MODE_DISABLED_HARDCORE_TOOLTIP
      );
      this.updateButton(
         this.personalGameModeButton, singleplayerServer, PERSONAL_GAME_MODE_TOOLTIP, GAME_MODE_DISABLED_OPERATOR_TOOLTIP, GAME_MODE_DISABLED_HARDCORE_TOOLTIP
      );
   ...
   private boolean hasSettingsChanges() {
      return this.wantedDifficulty != this.initialDifficulty
         || this.wantedDifficultyLocked != this.initialDifficultyLocked
         || this.wantedDefaultGameMode != this.initialDefaultGameMode
         || this.wantedPersonalGameMode != this.initialPersonalGameMode
         || this.wantedAllowCommands != this.initialAllowCommands
         || this.wantedMultiplayerScope != this.initialMultiplayerScope
         || this.wantedGuestCommandAccess != this.initialGuestCommandAccess
         || this.wantedForceGameMode != this.initialForceGameMode
         || this.lanPortChanged();
   }
   ...
   private void setPortError(final EditBox portEdit, @Nullable final Component errorMessage) {
   ...
   private static record DifficultyButtons(LayoutElement layout, CycleButton<Difficulty> difficultyButton, LockIconButton lockButton, Level level) {
      ...
      private void refresh(final Minecraft minecraft, final WorldOptionsScreen worldOptionsScreen, final boolean forceLevelDifficulty) {
         this.difficultyButton
            .setValue(
               forceLevelDifficulty
                  ? this.level.getDifficulty()
                  : (worldOptionsScreen.wantedDifficulty != null ? worldOptionsScreen.wantedDifficulty : this.level.getDifficulty())
            );
         this.lockButton
            .setLocked(worldOptionsScreen.wantedDifficultyLocked != null ? worldOptionsScreen.wantedDifficultyLocked : isDifficultyLocked(this.level));
      ...
      private static void updateDifficultyButtonsState(
         final Minecraft minecraft, final Level level, final CycleButton<Difficulty> difficultyButton, final LockIconButton lockButton
      ) {
         if (minecraft.player != null
            && (
               minecraft.player.permissions().hasPermission(Permissions.COMMANDS_GAMEMASTER)
                  || minecraft.getSingleplayerServer() != null && minecraft.getSingleplayerServer().isSingleplayerOwner(minecraft.player.nameAndId())
            )) {
