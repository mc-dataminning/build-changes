...
public class GuiGraphicsExtractor {
   ...
   public void blit(
      ...
      this.innerBlit(
         ...
         (u + (float)srcWidth) / (float)textureWidth,
         (v + 0.0F) / (float)textureHeight,
         (v + (float)srcHeight) / (float)textureHeight,
         color
      );
   }
   ...
   private void blitSprite(
      ...
      if (width != 0 && height != 0) {
         this.innerBlit(
            ...
            y,
            y + height,
            sprite.getU((float)textureX / (float)spriteWidth),
            sprite.getU((float)(textureX + width) / (float)spriteWidth),
            sprite.getV((float)textureY / (float)spriteHeight),
            sprite.getV((float)(textureY + height) / (float)spriteHeight),
            color
         );
      }
   }
      ...
      if (width > 0 && height > 0) {
         if (nineSlice.stretchInner()) {
            this.innerBlit(
               ...
               y + height,
               sprite.getU((float)textureX / (float)spriteWidth),
               sprite.getU((float)(textureX + textureWidth) / (float)spriteWidth),
               sprite.getV((float)textureY / (float)spriteHeight),
               sprite.getV((float)(textureY + textureHeight) / (float)spriteHeight),
               color
            );
         } else {
      ...
      if (width > 0 && height > 0) {
         if (tileWidth > 0 && tileHeight > 0) {
            ...
            this.innerTiledBlit(
               ...
               y + height,
               sprite.getU((float)textureX / (float)spriteWidth),
               sprite.getU((float)(textureX + tileWidth) / (float)spriteWidth),
               sprite.getV((float)textureY / (float)spriteHeight),
               sprite.getV((float)(textureY + tileHeight) / (float)spriteHeight),
               color
            );
         } else {
   ...
   public void map(final MapRenderState mapRenderState) {
      ...
      for (MapRenderState.MapDecorationRenderState decoration : mapRenderState.decorations) {
         if (decoration.renderOnFrame) {
            ...
            this.pose.rotate((float) (Math.PI / 180.0) * (float)decoration.rot * 360.0F / 16.0F);
   ...
   public void entity(
      ...
      @Nullable final Quaternionfc overrideCameraAngle,
      final int x0,
      final int y0,
      final int x1,
      final int y1
   ...
   public void profilerChart(final List<ResultField> chartData, final int x0, final int y0, final int x1, final int y1) {
      this.guiRenderState
         .addPicturesInPictureState(new GuiProfilerChartRenderState(new Matrix3x2f(this.pose), chartData, x0, y0, x1, y1, this.scissorStack.peek()));
   }
   ...
   public void setTooltipForNextFrame(final Font font, final ItemStack itemStack, final int xo, final int yo) {
      this.setTooltipForNextFrame(
         font, Screen.getTooltipFromItem(this.minecraft, itemStack), itemStack.getTooltipImage(), xo, yo, itemStack.get(DataComponents.TOOLTIP_STYLE), true
      );
   }
   ...
   public void setTooltipForNextFrame(
      ...
      final int yo,
      @Nullable final Identifier style,
      final boolean extraSpaceAfterFirstLine
   ) {
