...
public class AdvancementToast implements Toast {
   ...
   public AdvancementToast(final AdvancementHolder advancement) {
      ...
      this.iconItem = advancement.value().display().map(d -> d.icon().create()).orElse(ItemStack.EMPTY);
   }
   ...
   public void update(final ToastManager manager, final long fullyVisibleForMs) {
      ...
      if (display == null) {
         ...
         this.wantedVisibility = (double)fullyVisibleForMs >= 5000.0 * manager.getNotificationDisplayTimeMultiplier()
   ...
   private boolean isChallengeAdvancement() {
      ...
      return displayInfo.isPresent() && displayInfo.get().type().equals(AdvancementType.CHALLENGE);
   }
   ...
   public void extractRenderState(final GuiGraphicsExtractor graphics, final Font font, final long fullyVisibleForMs) {
      ...
      if (display != null) {
         ...
         if (lines.size() == 1) {
            graphics.text(font, display.type().getDisplayName(), 30, 7, titleColor, false);
            ...
            if (fullyVisibleForMs < 1500L) {
               ...
               graphics.text(font, display.type().getDisplayName(), 30, 11, ARGB.color(alpha, titleColor), false);
            } else {
