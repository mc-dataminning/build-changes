...
public class EntityHitboxDebugRenderer implements DebugRenderer.SimpleDebugRenderer {
      ...
      if (this.minecraft.level != null) {
         ...
         for (Entity entity : this.minecraft.level.entitiesForRendering()) {
            if (!entity.isInvisible()) {
               ...
               if (this.minecraft.levelExtractor.isEntityVisible(entity, frustum, camX, camY, camZ, entityPartialTicks, chunkFadeDuration)
   ...
   @Nullable
   private Entity getServerEntity(final Entity entity) {
   ...
   private void showHitboxes(final Entity entity, final float partialTicks, final boolean isServerEntity) {
      ...
      if (vehicle != null) {
         ...
         Gizmos.cuboid(
            new AABB(
               position.x - (double)width, position.y, position.z - (double)width, position.x + (double)width, position.y + 0.0625, position.z + (double)width
            ),
      ...
      if (entity instanceof LivingEntity) {
         ...
         float padding = 0.01F;
         Gizmos.cuboid(
            new AABB(bb.minX, bb.minY + (double)entity.getEyeHeight() - 0.01F, bb.minZ, bb.maxX, bb.minY + (double)entity.getEyeHeight() + 0.01F, bb.maxZ),
