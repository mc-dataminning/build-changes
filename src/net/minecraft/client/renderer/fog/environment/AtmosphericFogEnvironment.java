...
public class AtmosphericFogEnvironment extends FogEnvironment {
   ...
   public Vector3fc getBaseColor(final ClientLevel level, final Camera camera, final int renderDistance, final float partialTicks) {
      Vector3fc fogColor = camera.attributeProbe().getValue(EnvironmentAttributes.FOG_COLOR, partialTicks);
      if (renderDistance >= 4) {
         ...
         if (lookingAtTheSunFactor > 0.0F) {
            Vector4fc color = camera.attributeProbe().getValue(EnvironmentAttributes.SUNRISE_SUNSET_COLOR, partialTicks);
            float alpha = color.w();
            if (alpha > 0.0F) {
               fogColor = ARGB.srgbLerp(lookingAtTheSunFactor * alpha, fogColor, new Vector3f().set(color));
            }
         }
      }

      Vector3fc skyColor = camera.attributeProbe().getValue(EnvironmentAttributes.SKY_COLOR, partialTicks);
      ...
      return ARGB.srgbLerp(skyColorMixFactor, fogColor, new Vector3f().set(skyColor));
   }
   ...
   private void updateRainFogState(final Camera camera, final ClientLevel level, final DeltaTracker deltaTracker) {
      ...
      float skyLightLevelMultiplier = Mth.clamp(
         ((float)level.getLightEngine().getLayerListener(LightLayer.SKY).getLightValue(blockPos) - 8.0F) / 7.0F, 0.0F, 1.0F
      );
