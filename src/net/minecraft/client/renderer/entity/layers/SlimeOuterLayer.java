      ...
      if (!state.isInvisible || appearsGlowingWithInvisibility) {
         ...
         if (appearsGlowingWithInvisibility) {
            submitNodeCollector.order(1)
               .submitModel(this.model, state, poseStack, RenderTypes.outline(SlimeRenderer.SLIME_LOCATION), lightCoords, overlayCoords, state.outlineColor);
         } else {
            submitNodeCollector.order(1)
               .submitModel(
                  this.model, state, poseStack, RenderTypes.entityTranslucent(SlimeRenderer.SLIME_LOCATION), lightCoords, overlayCoords, state.outlineColor
               );
         }
      }
   }
