...
public class SoundEngine {
   ...
   private synchronized void loadLibrary() {
      if (!this.loaded) {
         try {
            ...
            LOGGER.error(MARKER, "Error starting SoundSystem. Turning off sounds & music", var3);
         }
      }
   }
   ...
   private static float getRange(final boolean isRelative, final SoundInstance.Attenuation instance, final float attenuationDistance) {
      return !isRelative && instance != SoundInstance.Attenuation.NONE ? attenuationDistance : Float.POSITIVE_INFINITY;
   }
