...
public class FirstPersonHandsAndItems {
   ...
   public void extractRenderState(final LocalPlayer player, final float partialTicks, final FirstPersonHandsAndItemsRenderState state) {
      ...
      player.minecraft
         ...
         .updateForTopItem(
            state.mainHandRenderState, state.mainHandItem, mainHandDisplayContext, player.level(), player, player.getId() + mainHandDisplayContext.ordinal()
         );
      player.minecraft
         .getItemModelResolver()
         .updateForTopItem(
            state.offHandRenderState, state.offHandItem, offHandDisplayContext, player.level(), player, player.getId() + offHandDisplayContext.ordinal()
         );
   ...
   private boolean shouldInstantlyReplaceVisibleItem(final ItemStack currentlyVisibleItem, final ItemStack expectedItem, final LocalPlayer player) {
      return ItemStack.matchesIgnoringComponents(currentlyVisibleItem, expectedItem, DataComponentType::ignoreSwapAnimation)
         ? true
         : !player.minecraft.getItemModelResolver().shouldPlaySwapAnimation(expectedItem);
   }
   ...
   public static FirstPersonHandsAndItemsRenderState.HandRenderSelection evaluateWhichHandsToRender(final LocalPlayer player) {
      ...
      if (!holdsBow && !holdsCrossbow) {
         ...
         if (!usedItemStack.is(Items.BOW) && !usedItemStack.is(Items.CROSSBOW)) {
            return usedHand == InteractionHand.MAIN_HAND && isChargedCrossbow(offhandItem)
               ? FirstPersonHandsAndItemsRenderState.HandRenderSelection.RENDER_MAIN_HAND_ONLY
               : FirstPersonHandsAndItemsRenderState.HandRenderSelection.RENDER_BOTH_HANDS;
         } else {
