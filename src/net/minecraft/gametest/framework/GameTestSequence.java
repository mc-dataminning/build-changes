   ...
   private void tick(final int tick) {
      ...
      while (iterator.hasNext()) {
         ...
         if (event.minimumDelay != null && event.minimumDelay > (long)delay) {
            this.parent
               .fail(new GameTestAssertException(Component.translatable("test.error.sequence.minimum_tick", (long)prevTick + event.minimumDelay), tick));
            break;
         }

         if (event.expectedDelay != null && event.expectedDelay != (long)delay) {
            this.parent
               .fail(new GameTestAssertException(Component.translatable("test.error.sequence.invalid_tick", (long)prevTick + event.expectedDelay), tick));
            break;
         }
      }
   }
