         ...
         for (int xx = 0; xx < 16; xx++) {
            for (int zz = 0; zz < 16; zz++) {
               for (int yyx = 0; yyx < 8; yyx++) {
                  ...
                  if (check) {
                     BlockPos offsetPos = origin.offset(xx, yyx, zz);
                     BlockState blockState = level.getBlockState(offsetPos);
                     if (yyx >= 4 && blockState.liquid()) {
                        return false;
                     }

                     if (yyx < 4 && !blockState.isSolid() && blockState != fluid) {
                        return false;
                     }
               ...
               for (int zz = 0; zz < 16; zz++) {
                  for (int yyxxx = 0; yyxxx < 8; yyxxx++) {
                     ...
                     if (check && (yyxxx < 4 || random.nextInt(2) != 0)) {
                        BlockPos offset = origin.offset(xx, yyxxx, zz);
                        BlockState blockStatex = level.getBlockState(offset);
                        if (blockStatex.isSolid() && this.canReplaceWithBarrier.test(level, offset)) {
