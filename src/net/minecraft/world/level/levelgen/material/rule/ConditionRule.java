...
public record ConditionRule(MaterialCondition ifTrue, MaterialRule thenRun) implements MaterialRule {
   public static final MapCodec<ConditionRule> CODEC = RecordCodecBuilder.mapCodec(
      i -> i.group(
               MaterialCondition.CODEC.fieldOf("if_true").forGetter(ConditionRule::ifTrue),
               MaterialRule.CODEC.fieldOf("then_run").forGetter(ConditionRule::thenRun)
            )
            .apply(i, ConditionRule::new)
   );
