...
public record FromInt(Holder<ContextIntProvider> input) implements ContextFloatProvider, UnaryProvider<ContextIntProvider> {
   public static final MapCodec<FromInt> MAP_CODEC = UnaryProvider.codec(ContextIntProviders.CODEC, FromInt::new);
   ...
   public float getFloatUnsafe(final LootContext context) {
      return ContextFloatProvider.intToFloatSafe(this.input().value().getIntUnsafe(context));
   }
}
