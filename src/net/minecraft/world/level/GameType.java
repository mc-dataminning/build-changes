...
public enum GameType implements StringRepresentable {
   ...
   public static final StreamCodec<ByteBuf, Optional<GameType>> OPTIONAL_STREAM_CODEC = ByteBufCodecs.OPTIONAL_VAR_INT
      .map(
         id -> id.isPresent() ? Optional.of(byId(id.getAsInt())) : Optional.empty(),
         gameType -> gameType.isPresent() ? OptionalInt.of(gameType.get().getId()) : OptionalInt.empty()
      );
   @Deprecated
   ...
   @Nullable
   public static GameType byName(final String name, @Nullable final GameType defaultMode) {
