...
public abstract class ChunkGenerator {
   ...
   private void doCreateBiomes(final Blender blender, final RandomState randomState, final ChunkAccess protoChunk) {
      ...
      try {
         ...
         BiomeResolver biomeResolver = this.biomeSource
            .createResolverForChunk(
               climateSampler,
               QuartPos.fromBlock(pos.getMinBlockX()),
               QuartPos.fromBlock(protoChunk.getMinY()),
               QuartPos.fromBlock(pos.getMinBlockZ()),
               QuartPos.fromBlock(16),
               QuartPos.fromBlock(protoChunk.getHeight()),
               QuartPos.fromBlock(16)
            );
   ...
   public void applyBiomeDecoration(final WorldGenLevel level, final ChunkAccess chunk, final StructureManager structureManager) {
      ...
      if (!SharedConstants.debugVoidTerrain(centerPos)) {
         ...
         try {
               ...
               if (stepIndex < featureStepCount) {
                  ...
                  for (int featureIndex = 0; featureIndex < numberOfFeaturesInStep; featureIndex++) {
                     ...
                     try {
                        ...
                        CrashReport report = CrashReport.forThrowable(var31, "Feature placement");
                        report.addCategory("Feature").setDetail("Description", currentlyGenerating::get);
                        throw new ReportedException(report);
                     }
                  }
            ...
            CrashReport report = CrashReport.forThrowable(var32, "Biome decoration");
   ...
   public WeightedList<MobSpawnSettings.SpawnerData> getMobsAt(
      final Level level, final StructureManager structureManager, final MobCategory mobCategory, final BlockPos pos
   ) {
      ...
      return level.environmentAttributes().getValue(EnvironmentAttributes.NATURAL_MOB_SPAWNS, pos).getMobsToSpawn(mobCategory);
   }
            ...
            .forEach(
               set -> {
                  ...
                  for (StructureSet.StructureSelectionEntry structure : structures) {
                     StructureStart existingStart = structureManager.getStartForStructure(structure.structure().value(), centerChunk);
   ...
   public abstract CompletableFuture<ChunkAccess> buildTerrain(
      ChunkAccess chunk,
      Blender blender,
   ...
   public abstract void addDebugScreenInfo(
