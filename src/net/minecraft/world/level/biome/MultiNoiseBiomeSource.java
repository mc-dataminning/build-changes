...
public class MultiNoiseBiomeSource extends BiomeSource {
   ...
   public BiomeResolver createResolverForChunk(
      ...
      DensityVolume volume = new DensityVolume(
         quartSizeX, quartSizeY, quartSizeZ, QuartPos.toBlock(minQuartX), QuartPos.toBlock(minQuartY), QuartPos.toBlock(minQuartZ), 4, 4, 4
      );
      ...
      return (quartX, quartY, quartZ) -> {
         ...
         return this.getNoiseBiome(
            Climate.target(temperature.get(index), vegetation.get(index), continents.get(index), erosion.get(index), depth.get(index), ridges.get(index))
         );
      };
   }
   ...
   public void addDebugInfo(final List<String> result, final BlockPos feetPos, final Climate.Sampler sampler) {
      ...
      result.add(
         "Biome builder PV: "
            ...
            + " C: "
            + biomeBuilder.getDebugStringForContinentalness((double)continentalness)
            + " E: "
            + biomeBuilder.getDebugStringForErosion((double)erosion)
            + " T: "
            + biomeBuilder.getDebugStringForTemperature((double)temperature)
            + " H: "
            + biomeBuilder.getDebugStringForHumidity((double)humidity)
      );
   }
}
