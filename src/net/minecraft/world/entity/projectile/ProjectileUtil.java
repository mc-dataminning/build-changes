...
public final class ProjectileUtil {
   ...
   public static Collection<EntityHitResult> getManyEntityHitResult(
      ...
      final boolean projectSurfaceHitLocation
   ) {
      return getManyEntityHitResult(
         level, source, from, to, targetSearchArea, matching, computeMargin(source), ClipContext.Block.COLLIDER, includeFromEntity, projectSurfaceHitLocation
      );
   }
            ...
            if (exactHit.isPresent()) {
               ...
               if (!outsideHit.isEmpty()) {
                  ...
                  if (surfaceHit.isPresent()) {
                     collector.add(new EntityHitResult(entity, projectSurfaceHitLocation ? surfaceHit.get() : outsideHitPosition));
                  }
               }
            }
         }
   ...
   public static AbstractArrow getMobArrow(final LivingEntity mob, final ItemStack projectile, final float power, @Nullable final ItemStack firedFromWeapon) {
