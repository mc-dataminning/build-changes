import com.google.common.collect.UnmodifiableIterator;
import javax.annotation.Nullable;
import org.joml.Vector3f;

public class byo extends bxx implements bmh, bna {
   private static final agm<Boolean> bT = agp.a(byo.class, ago.k);
   private static final agm<Integer> bU = agp.a(byo.class, ago.b);
   private static final cpy bW = cpy.a(cna.tY, cna.tZ, cna.vd);
   private final bmg bX = new bmg(this.an, bU, bT);

   public byo(bly<? extends byo> $$0, cto $$1) {
      super($$0, $$1);
   }

   @Override
   protected void B() {
      this.bO.a(0, new bta(this));
      this.bO.a(1, new btz(this, 1.25));
      this.bO.a(3, new bss(this, 1.0));
      this.bO.a(4, new buo(this, 1.2, cpy.a(cna.nQ), false));
      this.bO.a(4, new buo(this, 1.2, bW, false));
      this.bO.a(5, new btf(this, 1.1));
      this.bO.a(6, new but(this, 1.0));
      this.bO.a(7, new bto(this, cfh.class, 6.0F));
      this.bO.a(8, new bub(this));
   }

   public static bnp.a u() {
      return bmm.C().a(bnq.l, 10.0).a(bnq.m, 0.25);
   }

   @Nullable
   @Override
   public bmk cN() {
      return (bmk)(this.i() && this.cQ() instanceof cfh $$0 && $$0.b(cna.nQ) ? $$0 : super.cN());
   }

   @Override
   public void a(agm<?> $$0) {
      if (bU.equals($$0) && this.dM().B) {
         this.bX.a();
      }

      super.a($$0);
   }

   @Override
   protected void c_() {
      super.c_();
      this.an.a(bT, false);
      this.an.a(bU, 0);
   }

   @Override
   public void b(sn $$0) {
      super.b($$0);
      this.bX.a($$0);
   }

   @Override
   public void a(sn $$0) {
      super.a($$0);
      this.bX.b($$0);
   }

   @Override
   protected arq y() {
      return arr.sD;
   }

   @Override
   protected arq d(bks $$0) {
      return arr.sF;
   }

   @Override
   protected arq n_() {
      return arr.sE;
   }

   @Override
   protected void b(hx $$0, djg $$1) {
      this.a(arr.sH, 0.15F, 1.0F);
   }

   @Override
   public bka b(cfh $$0, bjz $$1) {
      boolean $$2 = this.m($$0.b($$1));
      if (!$$2 && this.i() && !this.bP() && !$$0.fI()) {
         if (!this.dM().B) {
            $$0.n(this);
         }

         return bka.a(this.dM().B);
      } else {
         bka $$3 = super.b($$0, $$1);
         if (!$$3.a()) {
            cmx $$4 = $$0.b($$1);
            return $$4.a(cna.nK) ? $$4.a($$0, this, $$1) : bka.d;
         } else {
            return $$3;
         }
      }
   }

   @Override
   public boolean g() {
      return this.bx() && !this.o_();
   }

   @Override
   protected void ey() {
      super.ey();
      if (this.i()) {
         this.a(cna.nK);
      }
   }

   @Override
   public boolean i() {
      return this.bX.d();
   }

   @Override
   public void a(@Nullable ars $$0) {
      this.bX.a(true);
      if ($$0 != null) {
         this.dM().a(null, this, arr.sG, $$0, 0.5F, 1.0F);
      }
   }

   @Override
   public els b(bmk $$0) {
      ic $$1 = this.cF();
      if ($$1.o() == ic.a.b) {
         return super.b($$0);
      } else {
         int[][] $$2 = che.a($$1);
         hx $$3 = this.dm();
         hx.a $$4 = new hx.a();
         UnmodifiableIterator var6 = $$0.fA().iterator();

         while (var6.hasNext()) {
            bmw $$5 = (bmw)var6.next();
            eln $$6 = $$0.e($$5);

            for (int[] $$7 : $$2) {
               $$4.d($$3.u() + $$7[0], $$3.v(), $$3.w() + $$7[1]);
               double $$8 = this.dM().j($$4);
               if (che.a($$8)) {
                  els $$9 = els.a($$4, $$8);
                  if (che.a(this.dM(), $$0, $$6.c($$9))) {
                     $$0.b($$5);
                     return $$9;
                  }
               }
            }
         }

         return super.b($$0);
      }
   }

   @Override
   public void a(and $$0, bmj $$1) {
      if ($$0.ak() != bjx.a) {
         cdj $$2 = bly.bu.a((cto)$$0);
         if ($$2 != null) {
            $$2.a(blz.a, new cmx(cna.oS));
            $$2.b(this.dr(), this.dt(), this.dx(), this.dC(), this.dE());
            $$2.t(this.fU());
            $$2.a(this.o_());
            if (this.ae()) {
               $$2.b(this.af());
               $$2.n(this.cB());
            }

            $$2.fJ();
            $$0.b($$2);
            this.am();
         } else {
            super.a($$0, $$1);
         }
      } else {
         super.a($$0, $$1);
      }
   }

   @Override
   protected void a(cfh $$0, els $$1) {
      super.a($$0, $$1);
      this.a($$0.dC(), $$0.dE() * 0.5F);
      this.N = this.aU = this.aW = this.dC();
      this.bX.b();
   }

   @Override
   protected els b(cfh $$0, els $$1) {
      return new els(0.0, 0.0, 1.0);
   }

   @Override
   protected float e(cfh $$0) {
      return (float)(this.b(bnq.m) * 0.225 * (double)this.bX.c());
   }

   @Override
   public boolean a() {
      return this.bX.a(this.eg());
   }

   @Nullable
   public byo b(and $$0, blp $$1) {
      return bly.aw.a((cto)$$0);
   }

   @Override
   public boolean m(cmx $$0) {
      return bW.a($$0);
   }

   @Override
   public els cJ() {
      return new els(0.0, (double)(0.6F * this.cI()), (double)(this.dg() * 0.4F));
   }

   @Override
   protected Vector3f a(blu $$0, blv $$1, float $$2) {
      return new Vector3f(0.0F, $$1.b - 0.03125F * $$2, 0.0F);
   }
}
