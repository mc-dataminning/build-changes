import com.mojang.serialization.MapCodec;
import com.mojang.serialization.codecs.RecordCodecBuilder;
import java.util.List;
import java.util.concurrent.CompletableFuture;
import java.util.stream.Collectors;
import java.util.stream.StreamSupport;

public class eck extends dyu {
   public static final MapCodec<eck> c = RecordCodecBuilder.mapCodec($$0 -> $$0.group(akt.d(dhs.b)).apply($$0, $$0.stable(eck::new)));
   private static final int h = 2;
   private static final List<dwy> i = StreamSupport.stream(mb.e.spliterator(), false).flatMap($$0 -> $$0.l().a().stream()).collect(Collectors.toList());
   private static final int j = ayz.f(ayz.c((float)i.size()));
   private static final int k = ayz.f((float)i.size() / (float)j);
   protected static final dwy d = djp.a.m();
   protected static final dwy e = djp.iu.m();
   public static final int f = 70;
   public static final int g = 60;

   public eck(jr.c<dhl> $$0) {
      super(new dhw($$0));
   }

   @Override
   protected MapCodec<? extends dyu> b() {
      return c;
   }

   @Override
   public void a(arl $$0, dhf $$1, edh $$2, dyt $$3) {
   }

   @Override
   public void a(dhh $$0, dyt $$1, dhf $$2) {
      ji.a $$3 = new ji.a();
      dfp $$4 = $$1.f();
      int $$5 = $$4.h;
      int $$6 = $$4.i;

      for (int $$7 = 0; $$7 < 16; $$7++) {
         for (int $$8 = 0; $$8 < 16; $$8++) {
            int $$9 = kk.a($$5, $$7);
            int $$10 = kk.a($$6, $$8);
            $$0.a($$3.d($$9, 60, $$10), e, 2);
            dwy $$11 = a($$9, $$10);
            $$0.a($$3.d($$9, 70, $$10), $$11, 2);
         }
      }
   }

   @Override
   public CompletableFuture<dyt> a(edv $$0, edh $$1, dhf $$2, dyt $$3) {
      return CompletableFuture.completedFuture($$3);
   }

   @Override
   public int a(int $$0, int $$1, ect.a $$2, dgl $$3, edh $$4) {
      return 0;
   }

   @Override
   public dgv a(int $$0, int $$1, dgl $$2, edh $$3) {
      return new dgv(0, new dwy[0]);
   }

   @Override
   public void a(List<String> $$0, edh $$1, ji $$2) {
   }

   public static dwy a(int $$0, int $$1) {
      dwy $$2 = d;
      if ($$0 > 0 && $$1 > 0 && $$0 % 2 != 0 && $$1 % 2 != 0) {
         $$0 /= 2;
         $$1 /= 2;
         if ($$0 <= j && $$1 <= k) {
            int $$3 = ayz.a($$0 * j + $$1);
            if ($$3 < i.size()) {
               $$2 = i.get($$3);
            }
         }
      }

      return $$2;
   }

   @Override
   public void a(arl $$0, long $$1, edh $$2, dhn $$3, dhf $$4, dyt $$5) {
   }

   @Override
   public void a(arl $$0) {
   }

   @Override
   public int g() {
      return 0;
   }

   @Override
   public int e() {
      return 384;
   }

   @Override
   public int f() {
      return 63;
   }
}
