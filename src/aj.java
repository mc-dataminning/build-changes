...
public class aj implements Comparable<aj> {
   ...
   private static final Codec<Instant> c = ayt.a(b).xmap(Instant::from, $$0 -> $$0.atZone(ZoneId.systemDefault()));
   private static final Codec<Map<String, aq>> d = Codec.unboundedMap(Codec.STRING, c)
      .xmap(
         ...
         $$0 -> $$0.entrySet()
               .stream()
               .filter($$0x -> ((aq)$$0x.getValue()).a())
               .collect(Collectors.toMap(Entry::getKey, $$0x -> Objects.requireNonNull(((aq)$$0x.getValue()).d())))
      );
   public static final Codec<aj> a = RecordCodecBuilder.create(
      $$0 -> $$0.group(d.optionalFieldOf("criteria", Map.of()).forGetter($$0x -> $$0x.e), Codec.BOOL.fieldOf("done").orElse(true).forGetter(aj::a))
            .apply($$0, ($$0x, $$1) -> new aj(new HashMap<>($$0x)))
   );
   private final Map<String, aq> e;
   private ak f = ak.b;

   private aj(Map<String, aq> $$0) {
      this.e = $$0;
   }
   ...
   public static aj b(wf $$0) {
      Map<String, aq> $$1 = $$0.a(wf::p, aq::b);
      return new aj($$1);
   }

   @Nullable
   public aq c(String $$0) {
      return this.e.get($$0);
   }
   ...
   public xi d() {
      if (this.e.isEmpty()) {
         ...
         if ($$0 <= 1) {
            ...
            int $$1 = this.h();
            return xi.a("advancements.progress", $$1, $$0);
         }
      }
   }
   ...
   public Iterable<String> e() {
      ...
      for (Entry<String, aq> $$1 : this.e.entrySet()) {
         if (!$$1.getValue().a()) {
            $$0.add($$1.getKey());
         }
      }
   ...
   @Nullable
   public Instant g() {
      return this.e.values().stream().map(aq::d).filter(Objects::nonNull).min(Comparator.naturalOrder()).orElse(null);
   }
