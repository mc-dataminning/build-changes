import com.mojang.serialization.MapCodec;
import com.mojang.serialization.codecs.RecordCodecBuilder;
import java.util.Optional;

public class djc extends dej implements ded {
   public static final MapCodec<djc> a = RecordCodecBuilder.mapCodec(
      $$0 -> $$0.group(akm.a(lf.aC).fieldOf("feature").forGetter($$0x -> $$0x.d), u()).apply($$0, djc::new)
   );
   protected static final float b = 3.0F;
   protected static final evf c = dea.a(5.0, 0.0, 5.0, 11.0, 6.0, 11.0);
   private final akm<dzm<?, ?>> d;

   @Override
   public MapCodec<djc> a() {
      return a;
   }

   public djc(akm<dzm<?, ?>> $$0, drc.d $$1) {
      super($$1);
      this.d = $$0;
   }

   @Override
   protected evf a(drd $$0, daf $$1, io $$2, eur $$3) {
      return c;
   }

   @Override
   protected void b(drd $$0, aqn $$1, io $$2, aym $$3) {
      if ($$3.a(25) == 0) {
         int $$4 = 5;
         int $$5 = 4;

         for (io $$6 : io.c($$2.b(-4, -1, -4), $$2.b(4, 1, 4))) {
            if ($$1.a_($$6).a(this)) {
               if (--$$4 <= 0) {
                  return;
               }
            }
         }

         io $$7 = $$2.b($$3.a(3) - 1, $$3.a(2) - $$3.a(2), $$3.a(3) - 1);

         for (int $$8 = 0; $$8 < 4; $$8++) {
            if ($$1.u($$7) && $$0.a($$1, $$7)) {
               $$2 = $$7;
            }

            $$7 = $$2.b($$3.a(3) - 1, $$3.a(2) - $$3.a(2), $$3.a(3) - 1);
         }

         if ($$1.u($$7) && $$0.a($$1, $$7)) {
            $$1.a($$7, $$0, 2);
         }
      }
   }

   @Override
   protected boolean b(drd $$0, daf $$1, io $$2) {
      return $$0.i($$1, $$2);
   }

   @Override
   protected boolean a(drd $$0, dbc $$1, io $$2) {
      io $$3 = $$2.d();
      drd $$4 = $$1.a_($$3);
      return $$4.a(avx.ba) ? true : $$1.b($$2, 0) < 13 && this.b($$4, $$1, $$3);
   }

   public boolean a(aqn $$0, io $$1, drd $$2, aym $$3) {
      Optional<? extends ix<dzm<?, ?>>> $$4 = $$0.H_().d(lf.aC).b(this.d);
      if ($$4.isEmpty()) {
         return false;
      } else {
         $$0.a($$1, false);
         if ($$4.get().a().a($$0, $$0.l().g(), $$3, $$1)) {
            return true;
         } else {
            $$0.a($$1, $$2, 3);
            return false;
         }
      }
   }

   @Override
   public boolean b(dbc $$0, io $$1, drd $$2) {
      return true;
   }

   @Override
   public boolean a(daz $$0, aym $$1, io $$2, drd $$3) {
      return (double)$$1.i() < 0.4;
   }

   @Override
   public void a(aqn $$0, aym $$1, io $$2, drd $$3) {
      this.a($$0, $$2, $$3, $$1);
   }
}
