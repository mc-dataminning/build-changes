import com.google.common.base.MoreObjects;
import com.google.common.collect.Lists;
import java.util.List;

public class fzi {
   static final alp b = alp.b("spectator/close");
   static final alp c = alp.b("spectator/scroll_left");
   static final alp d = alp.b("spectator/scroll_right");
   private static final fzk e = new fzi.a();
   private static final fzk f = new fzi.b(-1, true);
   private static final fzk g = new fzi.b(1, true);
   private static final fzk h = new fzi.b(1, false);
   private static final int i = 8;
   static final xk j = xk.c("spectatorMenu.close");
   static final xk k = xk.c("spectatorMenu.previous_page");
   static final xk l = xk.c("spectatorMenu.next_page");
   public static final fzk a = new fzk() {
      @Override
      public void a(fzi $$0) {
      }

      @Override
      public xk aU_() {
         return xj.a;
      }

      @Override
      public void a(fnl $$0, float $$1, float $$2) {
      }

      @Override
      public boolean aV_() {
         return false;
      }
   };
   private final fzl m;
   private fzj n;
   private int o = -1;
   int p;

   public fzi(fzl $$0) {
      this.n = new fzh();
      this.m = $$0;
   }

   public fzk a(int $$0) {
      int $$1 = $$0 + this.p * 6;
      if (this.p > 0 && $$0 == 0) {
         return f;
      } else if ($$0 == 7) {
         return $$1 < this.n.a().size() ? g : h;
      } else if ($$0 == 8) {
         return e;
      } else {
         return $$1 >= 0 && $$1 < this.n.a().size() ? (fzk)MoreObjects.firstNonNull(this.n.a().get($$1), a) : a;
      }
   }

   public List<fzk> a() {
      List<fzk> $$0 = Lists.newArrayList();

      for (int $$1 = 0; $$1 <= 8; $$1++) {
         $$0.add(this.a($$1));
      }

      return $$0;
   }

   public fzk b() {
      return this.a(this.o);
   }

   public fzj c() {
      return this.n;
   }

   public void b(int $$0) {
      fzk $$1 = this.a($$0);
      if ($$1 != a) {
         if (this.o == $$0 && $$1.aV_()) {
            $$1.a(this);
         } else {
            this.o = $$0;
         }
      }
   }

   public void d() {
      this.m.a(this);
   }

   public int e() {
      return this.o;
   }

   public void a(fzj $$0) {
      this.n = $$0;
      this.o = -1;
      this.p = 0;
   }

   public fzm f() {
      return new fzm(this.a(), this.o);
   }

   static class a implements fzk {
      @Override
      public void a(fzi $$0) {
         $$0.d();
      }

      @Override
      public xk aU_() {
         return fzi.j;
      }

      @Override
      public void a(fnl $$0, float $$1, float $$2) {
         $$0.a(glq::H, fzi.b, 0, 0, 16, 16, ayf.a($$2, $$1, $$1, $$1));
      }

      @Override
      public boolean aV_() {
         return true;
      }
   }

   static class b implements fzk {
      private final int a;
      private final boolean b;

      public b(int $$0, boolean $$1) {
         this.a = $$0;
         this.b = $$1;
      }

      @Override
      public void a(fzi $$0) {
         $$0.p = $$0.p + this.a;
      }

      @Override
      public xk aU_() {
         return this.a < 0 ? fzi.k : fzi.l;
      }

      @Override
      public void a(fnl $$0, float $$1, float $$2) {
         int $$3 = ayf.a($$2, $$1, $$1, $$1);
         if (this.a < 0) {
            $$0.a(glq::H, fzi.c, 0, 0, 16, 16, $$3);
         } else {
            $$0.a(glq::H, fzi.d, 0, 0, 16, 16, $$3);
         }
      }

      @Override
      public boolean aV_() {
         return this.b;
      }
   }
}
