...
public class bfl extends DataFix {
   ...
   private static <T> Dynamic<T> a(Dynamic<T> $$0) {
      ...
      Optional<Dynamic<T>> $$1 = $$0.get("HiddenEffect").result().map(bfl::a);
      return $$0.replaceField("HiddenEffect", "hidden_effect", $$1);
   }

   private static <T> Dynamic<T> b(Dynamic<T> $$0, String $$1, String $$2) {
      Optional<Dynamic<T>> $$3 = $$0.get($$1).asStreamOpt().result().map($$1x -> $$0.createList($$1x.map(bfl::a)));
   ...
   private TypeRewriteRule a() {
      Type<?> $$0 = this.getInputSchema().getType(bgr.s);
      return this.fixTypeEverywhereTyped("BlockEntityMobEffectIdFix", $$0, $$0x -> this.a($$0x, bgr.s, "minecraft:beacon", $$0xx -> {
   ...
   private TypeRewriteRule b() {
      ...
      return this.fixTypeEverywhereTyped("EntityMobEffectIdFix", $$0, $$0x -> {
         $$0x = this.a($$0x, bgr.B, "minecraft:mooshroom", bfl::c);
         $$0x = this.a($$0x, bgr.B, "minecraft:arrow", bfl::d);
         $$0x = this.a($$0x, bgr.B, "minecraft:area_effect_cloud", bfl::e);
         return $$0x.update(DSL.remainderFinder(), bfl::f);
      });
   }

   private TypeRewriteRule c() {
      Type<?> $$0 = this.getInputSchema().getType(bgr.b);
      return this.fixTypeEverywhereTyped("PlayerMobEffectIdFix", $$0, $$0x -> $$0x.update(DSL.remainderFinder(), bfl::f));
   }

   private static <T> Dynamic<T> g(Dynamic<T> $$0) {
      Optional<Dynamic<T>> $$1 = $$0.get("Effects").asStreamOpt().result().map($$1x -> $$0.createList($$1x.map(bfl::b)));
      ...
      return this.fixTypeEverywhereTyped("ItemStackMobEffectIdFix", $$1, $$2x -> {
         ...
         if ($$3.isPresent()) {
            ...
            if ($$4.equals("minecraft:suspicious_stew")) {
               return $$2x.updateTyped($$2, $$0xx -> $$0xx.update(DSL.remainderFinder(), bfl::g));
            }
