import com.mojang.logging.LogUtils;
import com.mojang.serialization.MapCodec;
import com.mojang.serialization.codecs.RecordCodecBuilder;
import java.util.List;
import org.slf4j.Logger;

public class fcc extends fcg {
   private static final Logger b = LogUtils.getLogger();
   public static final MapCodec<fcc> a = RecordCodecBuilder.mapCodec(
      $$0 -> a($$0).and(alq.a(mi.bs).fieldOf("name").forGetter($$0x -> $$0x.c)).apply($$0, fcc::new)
   );
   private final alq<fch> c;

   private fcc(List<fec> $$0, alq<fch> $$1) {
      super($$0);
      this.c = $$1;
   }

   @Override
   public fci<fcc> b() {
      return fcj.H;
   }

   @Override
   public void a(faz $$0) {
      if (!$$0.b()) {
         $$0.b("Uses reference to " + this.c.a() + ", but references are not allowed");
      } else if ($$0.a(this.c)) {
         $$0.b("Function " + this.c.a() + " is recursively called");
      } else {
         super.a($$0);
         $$0.a()
            .c(this.c)
            .ifPresentOrElse($$1 -> $$1.a().a($$0.a(".{" + this.c.a() + "}", this.c)), () -> $$0.b("Unknown function table called " + this.c.a()));
      }
   }

   @Override
   protected dak a(dak $$0, fat $$1) {
      fch $$2 = $$1.a().c(this.c).map(jg::a).orElse(null);
      if ($$2 == null) {
         b.warn("Unknown function: {}", this.c.a());
         return $$0;
      } else {
         fat.c<?> $$3 = fat.a($$2);
         if ($$1.b($$3)) {
            dak var5;
            try {
               var5 = $$2.apply($$0, $$1);
            } finally {
               $$1.c($$3);
            }

            return var5;
         } else {
            b.warn("Detected infinite loop in loot tables");
            return $$0;
         }
      }
   }

   public static fcg.a<?> a(alq<fch> $$0) {
      return a($$1 -> new fcc($$1, $$0));
   }
}
