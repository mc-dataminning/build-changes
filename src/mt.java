...
public class mt implements ky {
   ...
   public mt(la $$0) {
      this.d = $$0.a(la.b.b, "blockstates");
      this.e = $$0.a(la.b.b, "models");
   }

   @Override
   public CompletableFuture<?> a(kw $$0) {
      Map<dby, mu> $$1 = Maps.newHashMap();
      Consumer<mu> $$2 = $$1x -> {
         dby $$2x = $$1x.a();
         mu $$3x = $$1.put($$2x, $$1x);
         if ($$3x != null) {
      ...
      Map<ajt, Supplier<JsonElement>> $$3 = Maps.newHashMap();
      Set<cre> $$4 = Sets.newHashSet();
      BiConsumer<ajt, Supplier<JsonElement>> $$5 = ($$1x, $$2x) -> {
         Supplier<JsonElement> $$3x = $$3.put($$1x, $$2x);
         if ($$3x != null) {
            throw new IllegalStateException("Duplicate model definition for " + $$1x);
         }
      };
      Consumer<cre> $$6 = $$4::add;
      new mr($$2, $$5, $$6).a();
      new ms($$5).a();
      List<dby> $$7 = kr.e.g().stream().filter($$0x -> true).map(Entry::getValue).filter($$1x -> !$$1.containsKey($$1x)).toList();
      if (!$$7.isEmpty()) {
   ...
   private <T> CompletableFuture<?> a(kw $$0, Map<T, ? extends Supplier<JsonElement>> $$1, Function<T, Path> $$2) {
