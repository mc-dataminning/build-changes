...
public class RealmsEditInviteCodeScreen extends AbstractRealmsCodeScreen {
   ...
   private static final Component NEVER_EXPIRES_LABEL = Component.translatable(
         "mco.configure.world.invite_codes.edit.expires", InviteCodeExpiration.NEVER.getLabel()
      )
      .withColor(-6250336);
   ...
   private RealmsEditInviteCodeScreen.ExpirationSlider expirationSlider;
   @Nullable
   ...
   public void init() {
      ...
      GridLayout topRow = content.addChild(new GridLayout().spacing(8));
      LinearLayout codeColumn = topRow.addChild(LinearLayout.vertical().spacing(4), 0, 0, topRow.newCellSettings().alignVerticallyMiddle());
      ...
      this.activeButton = CycleButton.onOffBuilder(this.active).create(0, 0, 100, 20, ACTIVE_LABEL, (var1x, value) -> this.setActive(value));
      topRow.addChild(this.activeButton, 0, 1, topRow.newCellSettings().alignVerticallyMiddle());
      this.expirationSlider = content.addChild(new RealmsEditInviteCodeScreen.ExpirationSlider(308));
      ...
      rowHelper.addChild(Button.builder(COPY_LABEL, var1x -> this.onCopy()).width(150).build());
      this.deleteButton = rowHelper.addChild(Button.builder(DELETE_LABEL, var1x -> this.onDelete()).width(150).build());
      this.saveButton = rowHelper.addChild(Button.builder(CommonComponents.GUI_DONE, var1x -> this.onSave()).width(150).build());
      this.backButton = rowHelper.addChild(Button.builder(CommonComponents.GUI_BACK, var1x -> this.onClose()).width(150).build());
   ...
   private void onDelete() {
      ...
      RealmsUtil.runAsync(client -> client.deleteInviteCode(this.inviteCode.code()), exception -> this.handleFailure(exception, "Couldn't delete invite code"))
         .thenRunAsync(() -> {
   ...
   private void onSave() {
      if (this.active == this.inviteCode.enabled() && !this.expirationChanged) {
         ...
         this.setControlsActive(false);
         Instant expirationDate = InviteCodeExpiration.resolveExpirationDate(
            this.inviteCode.expirationDate(), this.expiration, this.expirationChanged, Instant.now()
         );
         RealmsUtil.runAsync(
               client -> client.updateInviteCode(this.realmId, this.inviteCode.code(), this.active, expirationDate),
               exception -> this.handleFailure(exception, "Couldn't update invite code")
            )
            .thenRunAsync(() -> {
               this.minecraft.gui.setScreen(this.parentScreen);
               this.parentScreen.refreshInviteCodes();
   ...
   private class ExpirationSlider extends AbstractSliderButton {
      ...
      private ExpirationSlider(final int width) {
         ...
         super(
            ...
            (double)RealmsEditInviteCodeScreen.this.expiration.ordinal() / (double)(InviteCodeExpiration.values().length - 1)
         );
      ...
      protected void updateMessage() {
         this.setMessage(
            CommonComponents.optionNameValue(RealmsEditInviteCodeScreen.EXPIRATION_DATE_LABEL, RealmsEditInviteCodeScreen.this.expiration.getLabel())
         );
      }
