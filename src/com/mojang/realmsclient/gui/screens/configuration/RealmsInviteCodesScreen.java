...
public class RealmsInviteCodesScreen extends AbstractRealmsCodeScreen {
   ...
   private RealmsInviteCodesScreen.InviteCodeSelectionList inviteCodeList;
   ...
   public void init() {
      ...
      Component headerTitle = (Component)(this.fetchedInviteCodes
         ? Component.translatable("mco.configure.world.invite_codes.title", this.inviteCodes.size(), 5)
         : this.getTitle());
      header.addChild(new StringWidget(headerTitle, this.font));
      int headerHeight = 33;
      if (this.fetchedInviteCodes && this.inviteCodes.isEmpty()) {
         MultiLineTextWidget subtitle = header.addChild(
            new MultiLineTextWidget(Component.translatable("mco.configure.world.invite_codes.subtitle", 5).withColor(-6250336), this.font)
               .setCentered(true)
               .setMaxWidth(308)
         );
      ...
      this.copyButton = actionRow.addChild(Button.builder(COPY_LABEL, var1x -> this.onCopy()).width(97).build());
      this.deleteButton = actionRow.addChild(Button.builder(DELETE_LABEL, var1x -> this.onDelete()).width(97).build());
      this.editButton = actionRow.addChild(Button.builder(EDIT_LABEL, var1x -> this.onEdit()).width(97).build());
      LinearLayout manageRow = footer.addChild(LinearLayout.horizontal().spacing(8));
      this.createButton = manageRow.addChild(Button.builder(CREATE_LABEL, var1x -> this.onCreate()).width(150).build());
      this.backButton = manageRow.addChild(Button.builder(CommonComponents.GUI_BACK, var1x -> this.onClose()).width(150).build());
      this.layout.setFooterHeight(64);
      if (this.fetchedInviteCodes) {
         this.inviteCodeList = this.layout.addToContents(new RealmsInviteCodesScreen.InviteCodeSelectionList(this.inviteCodes));
      } else {
   ...
   private void fetchInviteCodes() {
      ...
      RealmsUtil.<InviteCodeList>supplyAsync(
            client -> client.inviteCodes(this.realmId), exception -> this.handleFailure(exception, "Couldn't get invite codes", errorReturnScreen)
         )
   ...
   private void onDelete() {
      if (this.inviteCodeList != null) {
         ...
         if (selected != null) {
            ...
            RealmsUtil.runAsync(
                  client -> client.deleteInviteCode(selected.code()), exception -> this.handleFailure(exception, "Couldn't delete invite code", this)
               )
               .thenRunAsync(() -> {
                  if (this.backButton != null) {
                     this.backButton.active = true;
   ...
   private void onCreate() {
      if (this.fetchedInviteCodes && this.inviteCodes.size() < 5) {
         ...
         RealmsUtil.runAsync(
               client -> client.createInviteCode(this.realmId, true, null), exception -> this.handleFailure(exception, "Couldn't create invite code", this)
            )
            .thenRunAsync(() -> {
               if (this.backButton != null) {
                  this.backButton.active = true;
   ...
   void refreshInviteCodes() {
      if (this.inviteCodeList != null) {
         this.inviteCodeList.setSelected(null);
      }

      this.fetchInviteCodes();
   }

   private class InviteCodeSelectionList extends ObjectSelectionList<RealmsInviteCodesScreen.InviteCodeSelectionList.Entry> {
      public InviteCodeSelectionList(final List<InviteCode> inviteCodes) {
         Objects.requireNonNull(RealmsInviteCodesScreen.this);
         super(
            Minecraft.getInstance(),
            RealmsInviteCodesScreen.this.width,
            RealmsInviteCodesScreen.this.layout.getContentHeight(),
            RealmsInviteCodesScreen.this.layout.getHeaderHeight(),
            28
         );
         inviteCodes.forEach(inviteCode -> this.addEntry(new RealmsInviteCodesScreen.InviteCodeSelectionList.Entry(inviteCode)));
      }

      @Nullable
      public InviteCode getSelectedCode() {
         RealmsInviteCodesScreen.InviteCodeSelectionList.Entry selected = this.getSelected();
         return selected == null ? null : selected.inviteCode;
      }

      public void setSelected(final RealmsInviteCodesScreen.InviteCodeSelectionList.Entry entry) {
      ...
      private class Entry extends ObjectSelectionList.Entry<RealmsInviteCodesScreen.InviteCodeSelectionList.Entry> {
         ...
         public void extractContent(final GuiGraphicsExtractor graphics, final int mouseX, final int mouseY, final boolean hovered, final float a) {
            ...
            graphics.text(RealmsInviteCodesScreen.this.font, this.inviteCode.code(), this.getContentX() + 8, textY, -1);
         }
