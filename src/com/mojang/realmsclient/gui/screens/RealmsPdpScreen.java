...
public class RealmsPdpScreen extends RealmsScreen {
   ...
   @Nullable
   private final Runnable openRealmsScreenWhenAvailable;
   private RealmsPdpScreen.PdpList pdpList;
   ...
   public RealmsPdpScreen(
      final Screen lastScreen, final boolean trialAvailable, final Runnable openJoinRealmScreen, @Nullable final Runnable openRealmsScreenWhenAvailable
   ) {
      super(TITLE);
   ...
   public void init() {
      ...
      this.pdpList = this.layout.addToContents(new RealmsPdpScreen.PdpList(this.minecraft));
      ...
      footer.addChild(Button.builder(CommonComponents.GUI_BACK, var1x -> this.onClose()).build());
   ...
   private DataFetcher.Subscription initDataFetcher(final RealmsDataFetcher dataSource) {
      ...
      if (this.openRealmsScreenWhenAvailable != null) {
         result.subscribe(
            dataSource.serverListUpdateTask,
            serverListData -> this.realmsAvailable = !serverListData.serverList().isEmpty() || !serverListData.availableSnapshotServers().isEmpty()
         );
      }
   ...
   private abstract static class AbstractPdpEntry extends ObjectSelectionList.Entry<RealmsPdpScreen.AbstractPdpEntry> {
   ...
   private class PdpFeatureEntry extends RealmsPdpScreen.AbstractPdpEntry {
   ...
   private class PdpHeaderEntry extends RealmsPdpScreen.AbstractPdpEntry {
      private static final int DESCRIPTION_ICON_SIZE = 8;
      @Nullable
      private final RealmsPdpScreen.PdpHeaderEntry.Subtitle subtitle;

      public PdpHeaderEntry(final Component title, @Nullable final RealmsPdpScreen.PdpHeaderEntry.Subtitle subtitle) {
         Objects.requireNonNull(RealmsPdpScreen.this);
         super(title);
      ...
      public void extractContent(final GuiGraphicsExtractor graphics, final int mouseX, final int mouseY, final boolean hovered, final float a) {
         ...
         if (this.subtitle != null) {
            int iconX = this.getContentXMiddle() - Math.min(182, RealmsPdpScreen.this.font.width(this.subtitle.subTitle)) / 2 - 12;
            int iconY = textY + (9 - 8) / 2;
            graphics.blitSprite(RenderPipelines.GUI_TEXTURED, this.subtitle.icon, iconX, iconY, 8, 8);
            int subtitleX = this.getContentXMiddle() - Math.min(182, RealmsPdpScreen.this.font.width(this.subtitle.subTitle)) / 2;
      ...
      private int titleX() {
         return this.getContentXMiddle() - Math.min(182, RealmsPdpScreen.this.font.width(this.title)) / 2;
      }
   ...
   private class PdpList extends ObjectSelectionList<RealmsPdpScreen.AbstractPdpEntry> {
      private PdpList(final Minecraft minecraft) {
         ...
         if (RealmsPdpScreen.this.trialAvailable) {
            this.addHeader(
               RealmsPdpScreen.TRIAL_TITLE,
               new RealmsPdpScreen.PdpHeaderEntry.Subtitle(RealmsPdpScreen.TRIAL_DESCRIPTION, RealmsPdpScreen.TRIAL_AVAILABLE_SPRITE)
            );
         } else {
            this.addHeader(RealmsPdpScreen.TRIAL_TITLE, null);
         }
         ...
         this.addFeature(
            RealmsPdpScreen.PRIVATE_SERVER_IMAGE,
            RealmsPdpScreen.PRIVATE_SERVER_TITLE,
            RealmsPdpScreen.PRIVATE_SERVER_BACKUPS,
            RealmsPdpScreen.PRIVATE_SERVER_EASY_TO_MANAGE,
            RealmsPdpScreen.PRIVATE_SERVER_WORLD_SLOTS,
            RealmsPdpScreen.PRIVATE_SERVER_SECURE
         );
      }

      private void addHeader(final Component title, @Nullable final RealmsPdpScreen.PdpHeaderEntry.Subtitle subTitle) {
      ...
      private void addFeature(final Identifier image, final Component title, final Component... descriptions) {
         RealmsPdpScreen.PdpFeatureEntry entry = RealmsPdpScreen.this.new PdpFeatureEntry(image, title, descriptions);
         this.addEntry(entry, entry.entryHeight());
      }
