import java.util.List;
import java.util.function.Consumer;
import javax.annotation.Nullable;

public class fbx extends fdb {
   static final ahg b = new ahg("container/slot");
   private static final int c = 18;
   private static final int k = 20;
   private static final int l = 1;
   private static final int m = 1;
   private static final int n = 2;
   private static final int o = 2;
   protected final fhr a;
   private final Consumer<dxj> p;
   dxj q;
   private vf r;
   private vf t;
   private fbx.a u;
   private exg v;

   public fbx(fhr $$0, Consumer<dxj> $$1, dxj $$2) {
      super(vf.c("createWorld.customize.flat.title"));
      this.a = $$0;
      this.p = $$1;
      this.q = $$2;
   }

   public dxj m() {
      return this.q;
   }

   public void a(dxj $$0) {
      this.q = $$0;
   }

   @Override
   protected void aN_() {
      this.r = vf.c("createWorld.customize.flat.tile");
      this.t = vf.c("createWorld.customize.flat.height");
      this.u = this.d(new fbx.a());
      this.v = this.d(exg.a(vf.c("createWorld.customize.flat.removeLayer"), $$0 -> {
         if (this.D()) {
            List<dxg> $$1 = this.q.e();
            int $$2 = this.u.l().indexOf(this.u.i());
            int $$3 = $$1.size() - $$2 - 1;
            $$1.remove($$3);
            this.u.a($$1.isEmpty() ? null : this.u.l().get(Math.min($$2, $$1.size() - 1)));
            this.q.g();
            this.u.d();
            this.n();
         }
      }).a(this.g / 2 - 155, this.h - 52, 150, 20).a());
      this.d(exg.a(vf.c("createWorld.customize.presets"), $$0 -> {
         this.f.a(new fcx(this));
         this.q.g();
         this.n();
      }).a(this.g / 2 + 5, this.h - 52, 150, 20).a());
      this.d(exg.a(ve.d, $$0 -> {
         this.p.accept(this.q);
         this.f.a(this.a);
         this.q.g();
      }).a(this.g / 2 - 155, this.h - 28, 150, 20).a());
      this.d(exg.a(ve.e, $$0 -> {
         this.f.a(this.a);
         this.q.g();
      }).a(this.g / 2 + 5, this.h - 28, 150, 20).a());
      this.q.g();
      this.n();
   }

   void n() {
      this.v.j = this.D();
   }

   private boolean D() {
      return this.u.i() != null;
   }

   @Override
   public void aE_() {
      this.f.a(this.a);
   }

   @Override
   public void a(ewu $$0, int $$1, int $$2, float $$3) {
      super.a($$0, $$1, $$2, $$3);
      $$0.a(this.i, this.e, this.g / 2, 8, 16777215);
      int $$4 = this.g / 2 - 92 - 16;
      $$0.b(this.i, this.r, $$4, 32, 16777215);
      $$0.b(this.i, this.t, $$4 + 2 + 213 - this.i.a(this.t), 32, 16777215);
   }

   class a extends eyc<fbx.a.a> {
      public a() {
         super(fbx.this.f, fbx.this.g, fbx.this.h - 103, 43, 24);

         for (int $$0 = 0; $$0 < fbx.this.q.e().size(); $$0++) {
            this.b(new fbx.a.a());
         }
      }

      public void a(@Nullable fbx.a.a $$0) {
         super.a($$0);
         fbx.this.n();
      }

      @Override
      protected int c() {
         return this.g - 70;
      }

      public void d() {
         int $$0 = this.l().indexOf(this.i());
         this.m();

         for (int $$1 = 0; $$1 < fbx.this.q.e().size(); $$1++) {
            this.b(new fbx.a.a());
         }

         List<fbx.a.a> $$2 = this.l();
         if ($$0 >= 0 && $$0 < $$2.size()) {
            this.a($$2.get($$0));
         }
      }

      class a extends eyc.a<fbx.a.a> {
         @Override
         public void a(ewu $$0, int $$1, int $$2, int $$3, int $$4, int $$5, int $$6, int $$7, boolean $$8, float $$9) {
            dxg $$10 = fbx.this.q.e().get(fbx.this.q.e().size() - $$1 - 1);
            djh $$11 = $$10.b();
            cmy $$12 = this.a($$11);
            this.a($$0, $$3, $$2, $$12);
            $$0.a(fbx.this.i, $$12.y(), $$3 + 18 + 5, $$2 + 3, 16777215, false);
            vf $$13;
            if ($$1 == 0) {
               $$13 = vf.a("createWorld.customize.flat.layer.top", $$10.a());
            } else if ($$1 == fbx.this.q.e().size() - 1) {
               $$13 = vf.a("createWorld.customize.flat.layer.bottom", $$10.a());
            } else {
               $$13 = vf.a("createWorld.customize.flat.layer", $$10.a());
            }

            $$0.a(fbx.this.i, $$13, $$3 + 2 + 213 - fbx.this.i.a($$13), $$2 + 3, 16777215, false);
         }

         private cmy a(djh $$0) {
            cmt $$1 = $$0.b().k();
            if ($$1 == cnb.a) {
               if ($$0.a(cws.G)) {
                  $$1 = cnb.qw;
               } else if ($$0.a(cws.H)) {
                  $$1 = cnb.qx;
               }
            }

            return new cmy($$1);
         }

         @Override
         public vf a() {
            dxg $$0 = fbx.this.q.e().get(fbx.this.q.e().size() - a.this.l().indexOf(this) - 1);
            cmy $$1 = this.a($$0.b());
            return (vf)(!$$1.b() ? vf.a("narrator.select", $$1.y()) : ve.a);
         }

         @Override
         public boolean a(double $$0, double $$1, int $$2) {
            a.this.a(this);
            return true;
         }

         private void a(ewu $$0, int $$1, int $$2, cmy $$3) {
            this.a($$0, $$1 + 1, $$2 + 1);
            if (!$$3.b()) {
               $$0.b($$3, $$1 + 2, $$2 + 2);
            }
         }

         private void a(ewu $$0, int $$1, int $$2) {
            $$0.a(fbx.b, $$1, $$2, 0, 18, 18);
         }
      }
   }
}
