import com.mojang.serialization.Codec;
import com.mojang.serialization.codecs.RecordCodecBuilder;
import java.util.List;
import java.util.concurrent.CompletableFuture;
import java.util.concurrent.Executor;
import java.util.stream.Stream;

public class dke extends dgx {
   public static final Codec<dke> c = RecordCodecBuilder.create($$0 -> $$0.group(dtc.a.fieldOf("settings").forGetter(dke::g)).apply($$0, $$0.stable(dke::new)));
   private final dtc d;

   public dke(dtc $$0) {
      super(new cqv($$0.d()), ac.b($$0::a));
      this.d = $$0;
   }

   @Override
   public dgy a(hg<dvf> $$0, dkx $$1, long $$2) {
      Stream<he<dvf>> $$3 = this.d.c().map(hi::a).orElseGet(() -> $$0.b().map($$0xx -> $$0xx));
      return dgy.a($$1, $$2, this.b, $$3);
   }

   @Override
   protected Codec<? extends dgx> a() {
      return c;
   }

   public dtc g() {
      return this.d;
   }

   @Override
   public void a(akr $$0, cqe $$1, dkx $$2, dgw $$3) {
   }

   @Override
   public int a(cpo $$0) {
      return $$0.C_() + Math.min($$0.D_(), this.d.f().size());
   }

   @Override
   public CompletableFuture<dgw> a(Executor $$0, dll $$1, dkx $$2, cqe $$3, dgw $$4) {
      List<dfa> $$5 = this.d.f();
      gu.a $$6 = new gu.a();
      dkj $$7 = $$4.a(dkj.a.c);
      dkj $$8 = $$4.a(dkj.a.a);

      for (int $$9 = 0; $$9 < Math.min($$4.D_(), $$5.size()); $$9++) {
         dfa $$10 = $$5.get($$9);
         if ($$10 != null) {
            int $$11 = $$4.C_() + $$9;

            for (int $$12 = 0; $$12 < 16; $$12++) {
               for (int $$13 = 0; $$13 < 16; $$13++) {
                  $$4.a($$6.d($$12, $$11, $$13), $$10, false);
                  $$7.a($$12, $$11, $$13, $$10);
                  $$8.a($$12, $$11, $$13, $$10);
               }
            }
         }
      }

      return CompletableFuture.completedFuture($$4);
   }

   @Override
   public int a(int $$0, int $$1, dkj.a $$2, cpo $$3, dkx $$4) {
      List<dfa> $$5 = this.d.f();

      for (int $$6 = Math.min($$5.size(), $$3.aj()) - 1; $$6 >= 0; $$6--) {
         dfa $$7 = $$5.get($$6);
         if ($$7 != null && $$2.e().test($$7)) {
            return $$3.C_() + $$6 + 1;
         }
      }

      return $$3.C_();
   }

   @Override
   public cpy a(int $$0, int $$1, cpo $$2, dkx $$3) {
      return new cpy($$2.C_(), this.d.f().stream().limit((long)$$2.D_()).map($$0x -> $$0x == null ? csn.a.n() : $$0x).toArray(dfa[]::new));
   }

   @Override
   public void a(List<String> $$0, dkx $$1, gu $$2) {
   }

   @Override
   public void a(akr $$0, long $$1, dkx $$2, cqm $$3, cqe $$4, dgw $$5, dkf.a $$6) {
   }

   @Override
   public void a(akr $$0) {
   }

   @Override
   public int f() {
      return 0;
   }

   @Override
   public int d() {
      return 384;
   }

   @Override
   public int e() {
      return -63;
   }
}
