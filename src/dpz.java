import java.util.List;
import javax.annotation.Nullable;

public class dpz extends dpj implements bre, exe.a {
   public static final String d = "sherds";
   public static final String e = "item";
   public static final int f = 1;
   public long g;
   @Nullable
   public dpz.a h;
   private dqn k;
   private cur l;
   @Nullable
   protected ale<eqp> i;
   protected long j;

   public dpz(iz $$0, dse $$1) {
      super(dpl.O, $$0, $$1);
      this.l = cur.l;
      this.k = dqn.a;
   }

   @Override
   protected void b(us $$0, jk.a $$1) {
      super.b($$0, $$1);
      this.k.a($$0);
      if (!this.b_($$0) && !this.l.e()) {
         $$0.a("item", this.l.a($$1));
      }
   }

   @Override
   protected void a(us $$0, jk.a $$1) {
      super.a($$0, $$1);
      this.k = dqn.b($$0);
      if (!this.a_($$0)) {
         if ($$0.b("item", 10)) {
            this.l = cur.a($$1, (vp)$$0.p("item")).orElse(cur.l);
         } else {
            this.l = cur.l;
         }
      }
   }

   public acp j() {
      return acp.a(this);
   }

   @Override
   public us a(jk.a $$0) {
      return this.e($$0);
   }

   public je k() {
      return this.n().c(dsu.R);
   }

   public dqn l() {
      return this.k;
   }

   public void c(cur $$0) {
      this.a($$0);
   }

   public cur u() {
      cur $$0 = cuu.eC.w();
      $$0.b(this.s());
      return $$0;
   }

   public static cur a(dqn $$0) {
      cur $$1 = cuu.eC.w();
      $$1.b(km.Z, $$0);
      return $$1;
   }

   @Nullable
   @Override
   public ale<eqp> aw_() {
      return this.i;
   }

   @Override
   public void a(@Nullable ale<eqp> $$0) {
      this.i = $$0;
   }

   @Override
   public long ax_() {
      return this.j;
   }

   @Override
   public void a(long $$0) {
      this.j = $$0;
   }

   @Override
   protected void a(ki.a $$0) {
      super.a($$0);
      $$0.a(km.Z, this.k);
      $$0.a(km.aa, cxn.a(List.of(this.l)));
   }

   @Override
   protected void a(dpj.b $$0) {
      super.a($$0);
      this.k = $$0.a(km.Z, dqn.a);
      this.l = $$0.a(km.aa, cxn.a).a();
   }

   @Override
   public void a(us $$0) {
      super.a($$0);
      $$0.r("sherds");
      $$0.r("item");
   }

   @Override
   public cur f() {
      this.e_(null);
      return this.l;
   }

   @Override
   public cur c(int $$0) {
      this.e_(null);
      cur $$1 = this.l.a($$0);
      if (this.l.e()) {
         this.l = cur.l;
      }

      return $$1;
   }

   @Override
   public void b(cur $$0) {
      this.e_(null);
      this.l = $$0;
   }

   @Override
   public dpj v() {
      return this;
   }

   public void a(dpz.a $$0) {
      if (this.n != null && !this.n.x_()) {
         this.n.a(this.ay_(), this.n().b(), 1, $$0.ordinal());
      }
   }

   @Override
   public boolean a_(int $$0, int $$1) {
      if (this.n != null && $$0 == 1 && $$1 >= 0 && $$1 < dpz.a.values().length) {
         this.g = this.n.Z();
         this.h = dpz.a.values()[$$1];
         return true;
      } else {
         return super.a_($$0, $$1);
      }
   }

   public static enum a {
      a(7),
      b(10);

      public final int c;

      private a(final int $$0) {
         this.c = $$0;
      }
   }
}
