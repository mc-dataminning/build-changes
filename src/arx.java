import com.mojang.logging.LogUtils;
import java.util.Collections;
import java.util.List;
import java.util.Locale;
import java.util.concurrent.atomic.AtomicLong;
import java.util.function.Predicate;
import java.util.function.Supplier;
import javax.annotation.Nullable;
import net.minecraft.server.MinecraftServer;
import org.slf4j.Logger;

public class arx implements dgk {
   private static final Logger a = LogUtils.getLogger();
   private final bag<arg> b;
   private final dxq c;
   private final arp d;
   private final long e;
   private final etx f;
   private final azu h;
   private final dzq i;
   private final fbv<diq> j = new fbv<>($$0x -> this.y($$0x).o());
   private final fbv<eru> k = new fbv<>($$0x -> this.y($$0x).p());
   private final dgq l;
   private final dyu m;
   @Nullable
   private Supplier<String> n;
   private final AtomicLong o = new AtomicLong();
   private static final alj p = alj.b("worldgen_region_random");

   public arx(arp $$0, bag<arg> $$1, dyu $$2, dxq $$3) {
      this.m = $$2;
      this.b = $$1;
      this.c = $$3;
      this.d = $$0;
      this.e = $$0.D();
      this.f = $$0.C_();
      this.h = $$0.m().i().a(p).a(this.c.f().l());
      this.i = $$0.F_();
      this.l = new dgq(this, dgq.a(this.e));
   }

   public boolean a(des $$0, int $$1) {
      return this.d.m().a.b($$0, $$1);
   }

   public des b() {
      return this.c.f();
   }

   @Override
   public void a(@Nullable Supplier<String> $$0) {
      this.n = $$0;
   }

   @Override
   public dxq a(int $$0, int $$1) {
      return this.a($$0, $$1, dyr.c);
   }

   @Nullable
   @Override
   public dxq a(int $$0, int $$1, dyr $$2, boolean $$3) {
      int $$4 = this.c.f().e($$0, $$1);
      dyr $$5 = $$4 >= this.m.b().b() ? null : this.m.b().a($$4);
      arg $$6;
      if ($$5 != null) {
         $$6 = this.b.a($$0, $$1);
         if ($$2.c($$5)) {
            dxq $$7 = $$6.a($$5);
            if ($$7 != null) {
               return $$7;
            }
         }
      } else {
         $$6 = null;
      }

      o $$9 = o.a(new IllegalStateException("Requested chunk unavailable during world generation"), "Exception generating new chunk");
      p $$10 = $$9.a("Chunk request details");
      $$10.a("Requested chunk", String.format(Locale.ROOT, "%d, %d", $$0, $$1));
      $$10.a("Generating status", () -> this.m.a().f());
      $$10.a("Requested status", $$2::f);
      $$10.a("Actual status", () -> $$6 == null ? "[out of cache bounds]" : $$6.p().f());
      $$10.a("Maximum allowed status", () -> $$5 == null ? "null" : $$5.f());
      $$10.a("Dependencies", this.m.b()::toString);
      $$10.a("Requested distance", $$4);
      $$10.a("Generating chunk", this.c.f()::toString);
      throw new z($$9);
   }

   @Override
   public boolean b(int $$0, int $$1) {
      int $$2 = this.c.f().e($$0, $$1);
      return $$2 < this.m.b().b();
   }

   @Override
   public dvv a_(jh $$0) {
      return this.a(kj.a($$0.u()), kj.a($$0.w())).a_($$0);
   }

   @Override
   public erv b_(jh $$0) {
      return this.y($$0).b_($$0);
   }

   @Nullable
   @Override
   public cou a(double $$0, double $$1, double $$2, double $$3, Predicate<bul> $$4) {
      return null;
   }

   @Override
   public int D_() {
      return 0;
   }

   @Override
   public dgq H_() {
      return this.l;
   }

   @Override
   public jq<dgo> a(int $$0, int $$1, int $$2) {
      return this.d.a($$0, $$1, $$2);
   }

   @Override
   public float a(jm $$0, boolean $$1) {
      return 1.0F;
   }

   @Override
   public erk B_() {
      return this.d.B_();
   }

   @Override
   public boolean a(jh $$0, boolean $$1, @Nullable bul $$2, int $$3) {
      dvv $$4 = this.a_($$0);
      if ($$4.l()) {
         return false;
      } else {
         if ($$1) {
            dsy $$5 = $$4.x() ? this.c_($$0) : null;
            diq.a($$4, (dfm)this.d, $$0, $$5, $$2, cwm.k);
         }

         return this.a($$0, dis.a.m(), 3, $$3);
      }
   }

   @Nullable
   @Override
   public dsy c_(jh $$0) {
      dxq $$1 = this.y($$0);
      dsy $$2 = $$1.c_($$0);
      if ($$2 != null) {
         return $$2;
      } else {
         ul $$3 = $$1.f($$0);
         dvv $$4 = $$1.a_($$0);
         if ($$3 != null) {
            if ("DUMMY".equals($$3.l("id"))) {
               if (!$$4.x()) {
                  return null;
               }

               $$2 = ((dli)$$4.b()).a($$0, $$4);
            } else {
               $$2 = dsy.a($$0, $$4, $$3, this.d.J_());
            }

            if ($$2 != null) {
               $$1.a($$2);
               return $$2;
            }
         }

         if ($$4.x()) {
            a.warn("Tried to access a block entity before it was created. {}", $$0);
         }

         return null;
      }
   }

   @Override
   public boolean f_(jh $$0) {
      int $$1 = kj.a($$0.u());
      int $$2 = kj.a($$0.w());
      des $$3 = this.b();
      int $$4 = Math.abs($$3.g - $$1);
      int $$5 = Math.abs($$3.h - $$2);
      if ($$4 <= this.m.d() && $$5 <= this.m.d()) {
         if (this.c.x()) {
            dfo $$6 = this.c.y();
            if ($$6.e($$0.v())) {
               return false;
            }
         }

         return true;
      } else {
         ae.b(
            "Detected setBlock in a far chunk ["
               + $$1
               + ", "
               + $$2
               + "], pos: "
               + $$0
               + ", status: "
               + this.m.a()
               + (this.n == null ? "" : ", currently generating: " + this.n.get())
         );
         return false;
      }
   }

   @Override
   public boolean a(jh $$0, dvv $$1, int $$2, int $$3) {
      if (!this.f_($$0)) {
         return false;
      } else {
         dxq $$4 = this.y($$0);
         dvv $$5 = $$4.a($$0, $$1, false);
         if ($$5 != null) {
            this.d.a($$0, $$5, $$1);
         }

         if ($$1.x()) {
            if ($$4.j().d() == dyv.b) {
               dsy $$6 = ((dli)$$1.b()).a($$0, $$1);
               if ($$6 != null) {
                  $$4.a($$6);
               } else {
                  $$4.d($$0);
               }
            } else {
               ul $$7 = new ul();
               $$7.a("x", $$0.u());
               $$7.a("y", $$0.v());
               $$7.a("z", $$0.w());
               $$7.a("id", "DUMMY");
               $$4.a($$7);
            }
         } else if ($$5 != null && $$5.x()) {
            $$4.d($$0);
         }

         if ($$1.l(this, $$0)) {
            this.f($$0);
         }

         return true;
      }
   }

   private void f(jh $$0) {
      this.y($$0).e($$0);
   }

   @Override
   public boolean b(bul $$0) {
      int $$1 = kj.a($$0.dz());
      int $$2 = kj.a($$0.dF());
      this.a($$1, $$2).a($$0);
      return true;
   }

   @Override
   public boolean a(jh $$0, boolean $$1) {
      return this.a($$0, dis.a.m(), 3);
   }

   @Override
   public dxl E_() {
      return this.d.E_();
   }

   @Override
   public boolean A_() {
      return false;
   }

   @Deprecated
   @Override
   public arp a() {
      return this.d;
   }

   @Override
   public ke J_() {
      return this.d.J_();
   }

   @Override
   public crq J() {
      return this.d.J();
   }

   @Override
   public etx C_() {
      return this.f;
   }

   @Override
   public bsi d_(jh $$0) {
      if (!this.b(kj.a($$0.u()), kj.a($$0.w()))) {
         throw new RuntimeException("We are asking a region for a chunk out of bound");
      } else {
         return new bsi(this.d.ak(), this.d.ac(), 0L, this.d.ap());
      }
   }

   @Nullable
   @Override
   public MinecraftServer p() {
      return this.d.p();
   }

   @Override
   public dxu Q() {
      return this.d.m();
   }

   @Override
   public long D() {
      return this.e;
   }

   @Override
   public fbm<diq> S() {
      return this.j;
   }

   @Override
   public fbm<eru> R() {
      return this.k;
   }

   @Override
   public int O() {
      return this.d.O();
   }

   @Override
   public azu G_() {
      return this.h;
   }

   @Override
   public int a(ebq.a $$0, int $$1, int $$2) {
      return this.a(kj.a($$1), kj.a($$2)).a($$0, $$1 & 15, $$2 & 15) + 1;
   }

   @Override
   public void a(@Nullable cou $$0, jh $$1, awm $$2, awo $$3, float $$4, float $$5) {
   }

   @Override
   public void a(lq $$0, double $$1, double $$2, double $$3, double $$4, double $$5, double $$6) {
   }

   @Override
   public void a(@Nullable cou $$0, int $$1, jh $$2, int $$3) {
   }

   @Override
   public void a(jq<ear> $$0, ezy $$1, ear.a $$2) {
   }

   @Override
   public dzq F_() {
      return this.i;
   }

   @Override
   public boolean a(jh $$0, Predicate<dvv> $$1) {
      return $$1.test(this.a_($$0));
   }

   @Override
   public boolean b(jh $$0, Predicate<erv> $$1) {
      return $$1.test(this.b_($$0));
   }

   @Override
   public <T extends bul> List<T> a(eaf<bul, T> $$0, ezt $$1, Predicate<? super T> $$2) {
      return Collections.emptyList();
   }

   @Override
   public List<bul> a(@Nullable bul $$0, ezt $$1, @Nullable Predicate<? super bul> $$2) {
      return Collections.emptyList();
   }

   @Override
   public List<cou> y() {
      return Collections.emptyList();
   }

   @Override
   public int K_() {
      return this.d.K_();
   }

   @Override
   public int L_() {
      return this.d.L_();
   }

   @Override
   public long I_() {
      return this.o.getAndIncrement();
   }
}
