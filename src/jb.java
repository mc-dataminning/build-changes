...
public interface jb<T> extends Iterable<ix<T>> {
   ...
   Either<awm<T>, List<ix<T>>> c();

   Optional<ix<T>> a(aym var1);
   ...
   @Deprecated
   @VisibleForTesting
   static <T> jb.c<T> a(ja<T> $$0, awm<T> $$1) {
   ...
   public static final class a<T> extends jb.b<T> {
      ...
      @Override
      public Either<awm<T>, List<ix<T>>> c() {
         return Either.right(this.b);
      }

      @Override
      public Optional<awm<T>> d() {
