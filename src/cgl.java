import com.google.common.collect.ImmutableList;
import com.mojang.logging.LogUtils;
import com.mojang.serialization.Dynamic;
import java.util.Objects;
import java.util.Optional;
import java.util.UUID;
import java.util.function.BiConsumer;
import javax.annotation.Nullable;
import org.slf4j.Logger;

public class cgl extends bty implements cmi, dxe {
   private static final Logger e = LogUtils.getLogger();
   private static final kd bY = new kd(1, 1, 1);
   private static final int bZ = 5;
   private static final float ca = 55.0F;
   private static final float cb = 15.0F;
   private static final cys cc = cys.a(cus.oC);
   private static final int cd = 6000;
   private static final int ce = 3;
   private static final akk<Boolean> cf = ako.a(cgl.class, akm.k);
   private static final akk<Boolean> cg = ako.a(cgl.class, akm.k);
   protected static final ImmutableList<cea<? extends cdz<? super cgl>>> b = ImmutableList.of(cea.c, cea.d, cea.f, cea.b);
   protected static final ImmutableList<cct<?>> c = ImmutableList.of(
      cct.t, cct.n, cct.h, cct.m, cct.E, cct.x, cct.L, cct.aM, cct.aN, cct.aO, cct.aP, cct.Z, new cct[0]
   );
   public static final ImmutableList<Float> d = ImmutableList.of(
      0.5625F, 0.625F, 0.75F, 0.9375F, 1.0F, 1.0F, 1.125F, 1.25F, 1.5F, 1.875F, 2.0F, 2.25F, new Float[]{2.5F, 3.0F, 3.75F, 4.0F}
   );
   private final dws<dxe.b> ch;
   private dxe.a ci;
   private final dxe.d cj;
   private final dws<cgl.a> ck;
   private final brd cl = new brd(1);
   @Nullable
   private iz cm;
   private long cn;
   private float co;
   private float cp;
   private float cq;
   private float cr;
   private float cs;

   public cgl(bta<? extends cgl> $$0, dby $$1) {
      super($$0, $$1);
      this.bP = new bzn(this, 20, true);
      this.a_(this.fV());
      this.cj = new cgl.b();
      this.ci = new dxe.a();
      this.ch = new dws<>(new dxe.b(this));
      this.ck = new dws<>(new cgl.a(this.cj.b(), dwv.E.a().a()));
   }

   @Override
   protected bur.b<cgl> dT() {
      return bur.a(c, b);
   }

   @Override
   protected bur<?> a(Dynamic<?> $$0) {
      return cgm.a(this.dT().a($$0));
   }

   @Override
   public bur<cgl> dS() {
      return (bur<cgl>)super.dS();
   }

   public static buw.a s() {
      return btr.A().a(bux.q, 20.0).a(bux.j, 0.1F).a(bux.r, 0.1F).a(bux.c, 2.0).a(bux.k, 48.0);
   }

   @Override
   protected cdb b(dby $$0) {
      ccz $$1 = new ccz(this, $$0);
      $$1.b(false);
      $$1.a(true);
      $$1.c(true);
      return $$1;
   }

   @Override
   protected void a(ako.a $$0) {
      super.a($$0);
      $$0.a(cf, false);
      $$0.a(cg, true);
   }

   @Override
   public void a(evr $$0) {
      if (this.da()) {
         if (this.be()) {
            this.a(0.02F, $$0);
            this.a(btu.a, this.ds());
            this.h(this.ds().a(0.8F));
         } else if (this.bs()) {
            this.a(0.02F, $$0);
            this.a(btu.a, this.ds());
            this.h(this.ds().a(0.5));
         } else {
            this.a(this.fn(), $$0);
            this.a(btu.a, this.ds());
            this.h(this.ds().a(0.91F));
         }
      }

      this.r(false);
   }

   @Override
   public boolean a(brn $$0, float $$1) {
      if ($$0.d() instanceof cmx $$2) {
         Optional<UUID> $$3 = this.dS().c(cct.aM);
         if ($$3.isPresent() && $$2.cz().equals($$3.get())) {
            return false;
         }
      }

      return super.a($$0, $$1);
   }

   @Override
   protected void b(iz $$0, dsc $$1) {
   }

   @Override
   protected void a(double $$0, boolean $$1, dsc $$2, iz $$3) {
   }

   @Override
   protected avz v() {
      return this.c(btb.a) ? awa.a : awa.b;
   }

   @Override
   protected avz d(brn $$0) {
      return awa.d;
   }

   @Override
   protected avz o_() {
      return awa.c;
   }

   @Override
   protected float fe() {
      return 0.4F;
   }

   @Override
   protected void Z() {
      this.dP().ag().a("allayBrain");
      this.dS().a((arf)this.dP(), this);
      this.dP().ag().c();
      this.dP().ag().a("allayActivityUpdate");
      cgm.a(this);
      this.dP().ag().c();
      super.Z();
   }

   @Override
   public void n_() {
      super.n_();
      if (!this.dP().B && this.bD() && this.ai % 10 == 0) {
         this.c(1.0F);
      }

      if (this.gq() && this.gv() && this.ai % 20 == 0) {
         this.w(false);
         this.cm = null;
      }

      this.gw();
   }

   @Override
   public void l() {
      super.l();
      if (this.dP().B) {
         this.cp = this.co;
         if (this.u()) {
            this.co = ayz.a(this.co + 1.0F, 0.0F, 5.0F);
         } else {
            this.co = ayz.a(this.co - 1.0F, 0.0F, 5.0F);
         }

         if (this.gq()) {
            this.cq++;
            this.cs = this.cr;
            if (this.gr()) {
               this.cr++;
            } else {
               this.cr--;
            }

            this.cr = ayz.a(this.cr, 0.0F, 15.0F);
         } else {
            this.cq = 0.0F;
            this.cr = 0.0F;
            this.cs = 0.0F;
         }
      } else {
         dxe.c.a(this.dP(), this.ci, this.cj);
         if (this.gn()) {
            this.w(false);
         }
      }
   }

   @Override
   public boolean fV() {
      return !this.gu() && this.u();
   }

   public boolean u() {
      return !this.b(bqt.a).e();
   }

   @Override
   public boolean f(cup $$0) {
      return false;
   }

   private boolean gu() {
      return this.dS().a(cct.aP, ccu.a);
   }

   @Override
   protected bqu b(cmx $$0, bqt $$1) {
      cup $$2 = $$0.b($$1);
      cup $$3 = this.b(bqt.a);
      if (this.gq() && this.n($$2) && this.gz()) {
         this.gx();
         this.dP().a(this, (byte)18);
         this.dP().a($$0, this, awa.E, awb.g, 2.0F, 1.0F);
         this.a($$0, $$2);
         return bqu.a;
      } else if ($$3.e() && !$$2.e()) {
         cup $$4 = $$2.c(1);
         this.a(bqt.a, $$4);
         this.a($$0, $$2);
         this.dP().a($$0, this, awa.e, awb.g, 2.0F, 1.0F);
         this.dS().a(cct.aM, $$0.cz());
         return bqu.a;
      } else if (!$$3.e() && $$1 == bqt.a && $$2.e()) {
         this.a(btb.a, cup.l);
         this.dP().a($$0, this, awa.f, awb.g, 2.0F, 1.0F);
         this.a(bqt.a);

         for (cup $$5 : this.y().f()) {
            bvk.a(this, $$5, this.dn());
         }

         this.dS().b(cct.aM);
         $$0.i($$3);
         return bqu.a;
      } else {
         return super.b($$0, $$1);
      }
   }

   public void b(iz $$0, boolean $$1) {
      if ($$1) {
         if (!this.gq()) {
            this.cm = $$0;
            this.w(true);
         }
      } else if ($$0.equals(this.cm) || this.cm == null) {
         this.cm = null;
         this.w(false);
      }
   }

   @Override
   public brd y() {
      return this.cl;
   }

   @Override
   protected kd V() {
      return bY;
   }

   @Override
   public boolean k(cup $$0) {
      cup $$1 = this.b(bqt.a);
      return !$$1.e() && this.dP().ab().b(dbu.c) && this.cl.c($$0) && this.d($$1, $$0);
   }

   private boolean d(cup $$0, cup $$1) {
      return cup.b($$0, $$1) && !this.e($$0, $$1);
   }

   private boolean e(cup $$0, cup $$1) {
      cwq $$2 = $$0.a(km.G);
      cwq $$3 = $$1.a(km.G);
      return !Objects.equals($$2, $$3);
   }

   @Override
   protected void b(cjh $$0) {
      cmi.a(this, this, $$0);
   }

   @Override
   protected void Y() {
      super.Y();
      agt.a(this);
   }

   @Override
   public boolean aV() {
      return !this.aE();
   }

   @Override
   public void a(BiConsumer<dws<?>, arf> $$0) {
      if (this.dP() instanceof arf $$1) {
         $$0.accept(this.ch, $$1);
         $$0.accept(this.ck, $$1);
      }
   }

   public boolean gq() {
      return this.ao.a(cf);
   }

   public void w(boolean $$0) {
      if (!this.dP().B && this.db() && (!$$0 || !this.gn())) {
         this.ao.a(cf, $$0);
      }
   }

   private boolean gv() {
      return this.cm == null || !this.cm.a(this.dn(), (double)dwv.E.a().a()) || !this.dP().a_(this.cm).a(dfb.dT);
   }

   public float G(float $$0) {
      return ayz.i($$0, this.cp, this.co) / 5.0F;
   }

   public boolean gr() {
      float $$0 = this.cq % 55.0F;
      return $$0 < 15.0F;
   }

   public float H(float $$0) {
      return ayz.i($$0, this.cs, this.cr) / 15.0F;
   }

   @Override
   public boolean a(cup $$0, cup $$1) {
      return !this.d($$0, $$1);
   }

   @Override
   protected void eD() {
      super.eD();
      this.cl.f().forEach(this::b);
      cup $$0 = this.a(btb.a);
      if (!$$0.e() && !dab.g($$0)) {
         this.b($$0);
         this.a(btb.a, cup.l);
      }
   }

   @Override
   public boolean h(double $$0) {
      return false;
   }

   @Override
   public void b(us $$0) {
      super.b($$0);
      this.b($$0, this.dR());
      dxe.a.a.encodeStart(vg.a, this.ci).resultOrPartial(e::error).ifPresent($$1 -> $$0.a("listener", $$1));
      $$0.a("DuplicationCooldown", this.cn);
      $$0.a("CanDuplicate", this.gz());
   }

   @Override
   public void a(us $$0) {
      super.a($$0);
      this.a($$0, this.dR());
      if ($$0.b("listener", 10)) {
         dxe.a.a.parse(new Dynamic(vg.a, $$0.p("listener"))).resultOrPartial(e::error).ifPresent($$0x -> this.ci = $$0x);
      }

      this.cn = (long)$$0.h("DuplicationCooldown");
      this.ao.a(cg, $$0.q("CanDuplicate"));
   }

   @Override
   protected boolean go() {
      return false;
   }

   private void gw() {
      if (this.cn > 0L) {
         this.cn--;
      }

      if (!this.dP().x_() && this.cn == 0L && !this.gz()) {
         this.ao.a(cg, true);
      }
   }

   private boolean n(cup $$0) {
      return cc.a($$0);
   }

   private void gx() {
      cgl $$0 = bta.a.a(this.dP());
      if ($$0 != null) {
         $$0.e(this.dn());
         $$0.fU();
         $$0.gy();
         this.gy();
         this.dP().b($$0);
      }
   }

   private void gy() {
      this.cn = 6000L;
      this.ao.a(cg, false);
   }

   private boolean gz() {
      return this.ao.a(cg);
   }

   private void a(cmx $$0, cup $$1) {
      $$1.a(1, $$0);
   }

   @Override
   public evr cM() {
      return new evr(0.0, (double)this.cL() * 0.6, (double)this.dj() * 0.1);
   }

   @Override
   public void b(byte $$0) {
      if ($$0 == 18) {
         for (int $$1 = 0; $$1 < 3; $$1++) {
            this.gA();
         }
      } else {
         super.b($$0);
      }
   }

   private void gA() {
      double $$0 = this.ah.k() * 0.02;
      double $$1 = this.ah.k() * 0.02;
      double $$2 = this.ah.k() * 0.02;
      this.dP().a(li.Q, this.d(1.0), this.dx() + 0.5, this.g(1.0), $$0, $$1, $$2);
   }

   @Override
   public dxe.a gs() {
      return this.ci;
   }

   @Override
   public dxe.d gt() {
      return this.cj;
   }

   class a implements dwx {
      private final dwz b;
      private final int c;

      public a(final dwz $$0, final int $$1) {
         this.b = $$0;
         this.c = $$1;
      }

      @Override
      public dwz a() {
         return this.b;
      }

      @Override
      public int b() {
         return this.c;
      }

      @Override
      public boolean a(arf $$0, ji<dwv> $$1, dwv.a $$2, evr $$3) {
         if ($$1.a(dwv.E)) {
            cgl.this.b(iz.a($$3), true);
            return true;
         } else if ($$1.a(dwv.F)) {
            cgl.this.b(iz.a($$3), false);
            return true;
         } else {
            return false;
         }
      }
   }

   class b implements dxe.d {
      private static final int b = 16;
      private final dwz c = new dwt(cgl.this, cgl.this.cL());

      @Override
      public int a() {
         return 16;
      }

      @Override
      public dwz b() {
         return this.c;
      }

      @Override
      public boolean a(arf $$0, iz $$1, ji<dwv> $$2, dwv.a $$3) {
         if (cgl.this.gg()) {
            return false;
         } else {
            Optional<jh> $$4 = cgl.this.dS().c(cct.aN);
            if ($$4.isEmpty()) {
               return true;
            } else {
               jh $$5 = $$4.get();
               return $$5.a().equals($$0.af()) && $$5.b().equals($$1);
            }
         }
      }

      @Override
      public void a(arf $$0, iz $$1, ji<dwv> $$2, @Nullable bsu $$3, @Nullable bsu $$4, float $$5) {
         if ($$2.a(dwv.H)) {
            cgm.a(cgl.this, new iz($$1));
         }
      }

      @Override
      public axf<dwv> c() {
         return aww.e;
      }
   }
}
