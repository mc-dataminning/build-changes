import java.util.EnumSet;
import java.util.List;
import javax.annotation.Nullable;

public abstract class ces extends cer {
   @Nullable
   private hz b;
   private boolean c;
   private boolean d;

   protected ces(bnw<? extends ces> $$0, cvr $$1) {
      super($$0, $$1);
   }

   @Override
   protected void B() {
      super.B();
      this.bO.a(4, new ces.a<>(this, 0.7, 0.595));
   }

   @Override
   public void b(sw $$0) {
      super.b($$0);
      if (this.b != null) {
         $$0.a("PatrolTarget", tl.a(this.b));
      }

      $$0.a("PatrolLeader", this.c);
      $$0.a("Patrolling", this.d);
   }

   @Override
   public void a(sw $$0) {
      super.a($$0);
      if ($$0.e("PatrolTarget")) {
         this.b = tl.b($$0.p("PatrolTarget"));
      }

      this.c = $$0.q("PatrolLeader");
      this.d = $$0.q("Patrolling");
   }

   public boolean gh() {
      return true;
   }

   @Nullable
   @Override
   public bpa a(cwg $$0, blu $$1, bom $$2, @Nullable bpa $$3, @Nullable sw $$4) {
      if ($$2 != bom.p && $$2 != bom.h && $$2 != bom.d && $$0.F_().i() < 0.06F && this.gh()) {
         this.c = true;
      }

      if (this.gk()) {
         this.a(bnx.f, ciu.s());
         this.a(bnx.f, 2.0F);
      }

      if ($$2 == bom.p) {
         this.d = true;
      }

      return super.a($$0, $$1, $$2, $$3, $$4);
   }

   public static boolean b(bnw<? extends ces> $$0, cvs $$1, bom $$2, hz $$3, awp $$4) {
      return $$1.a(cwa.b, $$3) > 8 ? false : c($$0, $$1, $$2, $$3, $$4);
   }

   @Override
   public boolean h(double $$0) {
      return !this.d || $$0 > 16384.0;
   }

   public void i(hz $$0) {
      this.b = $$0;
      this.d = true;
   }

   public hz gi() {
      return this.b;
   }

   public boolean gj() {
      return this.b != null;
   }

   public void w(boolean $$0) {
      this.c = $$0;
      this.d = true;
   }

   public boolean gk() {
      return this.c;
   }

   public boolean gn() {
      return true;
   }

   public void go() {
      this.b = this.dm().b(-500 + this.af.a(1000), 0, -500 + this.af.a(1000));
      this.d = true;
   }

   protected boolean gp() {
      return this.d;
   }

   protected void x(boolean $$0) {
      this.d = $$0;
   }

   public static class a<T extends ces> extends bvf {
      private static final int a = 200;
      private final T b;
      private final double c;
      private final double d;
      private long e;

      public a(T $$0, double $$1, double $$2) {
         this.b = $$0;
         this.c = $$1;
         this.d = $$2;
         this.e = -1L;
         this.a(EnumSet.of(bvf.a.a));
      }

      @Override
      public boolean a() {
         boolean $$0 = this.b.dM().X() < this.e;
         return this.b.gp() && this.b.q() == null && !this.b.cO() && this.b.gj() && !$$0;
      }

      @Override
      public void c() {
      }

      @Override
      public void d() {
      }

      @Override
      public void e() {
         boolean $$0 = this.b.gk();
         bxt $$1 = this.b.N();
         if ($$1.l()) {
            List<ces> $$2 = this.h();
            if (this.b.gp() && $$2.isEmpty()) {
               this.b.x(false);
            } else if ($$0 && this.b.gi().a(this.b.dk(), 10.0)) {
               this.b.go();
            } else {
               enz $$3 = enz.c(this.b.gi());
               enz $$4 = this.b.dk();
               enz $$5 = $$4.d($$3);
               $$3 = $$5.b(90.0F).a(0.4).e($$3);
               enz $$6 = $$3.d($$4).d().a(10.0).e($$4);
               hz $$7 = hz.a($$6);
               $$7 = this.b.dM().a(dqv.a.f, $$7);
               if (!$$1.a((double)$$7.u(), (double)$$7.v(), (double)$$7.w(), $$0 ? this.d : this.c)) {
                  this.i();
                  this.e = this.b.dM().X() + 200L;
               } else if ($$0) {
                  for (ces $$8 : $$2) {
                     $$8.i($$7);
                  }
               }
            }
         }
      }

      private List<ces> h() {
         return this.b.dM().a(ces.class, this.b.cH().g(16.0), $$0 -> $$0.gn() && !$$0.s(this.b));
      }

      private boolean i() {
         awp $$0 = this.b.eh();
         hz $$1 = this.b.dM().a(dqv.a.f, this.b.dm().b(-8 + $$0.a(16), 0, -8 + $$0.a(16)));
         return this.b.N().a((double)$$1.u(), (double)$$1.v(), (double)$$1.w(), this.c);
      }
   }
}
