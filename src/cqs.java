import com.mojang.serialization.Codec;
import com.mojang.serialization.codecs.RecordCodecBuilder;
import java.util.stream.Stream;

public class cqs extends cqo {
   public static final Codec<cqs> b = RecordCodecBuilder.create(
      $$0 -> $$0.group(cqk.d.fieldOf("biomes").forGetter($$0x -> $$0x.c), Codec.intRange(0, 62).fieldOf("scale").orElse(2).forGetter($$0x -> $$0x.e))
            .apply($$0, cqs::new)
   );
   private final hi<cqk> c;
   private final int d;
   private final int e;

   public cqs(hi<cqk> $$0, int $$1) {
      this.c = $$0;
      this.d = $$1 + 2;
      this.e = $$1;
   }

   @Override
   protected Stream<he<cqk>> b() {
      return this.c.a();
   }

   @Override
   protected Codec<? extends cqo> a() {
      return b;
   }

   @Override
   public he<cqk> getNoiseBiome(int $$0, int $$1, int $$2, cqt.f $$3) {
      return this.c.a(Math.floorMod(($$0 >> this.d) + ($$2 >> this.d), this.c.b()));
   }
}
