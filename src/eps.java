import java.util.HashSet;
import java.util.List;
import java.util.Set;
import javax.annotation.Nullable;

public class eps {
   private final List<epq> a;
   @Nullable
   private eps.a b;
   private int c;
   private final ir d;
   private final float e;
   private final boolean f;

   public eps(List<epq> $$0, ir $$1, boolean $$2) {
      this.a = $$0;
      this.d = $$1;
      this.e = $$0.isEmpty() ? Float.MAX_VALUE : this.a.get(this.a.size() - 1).c(this.d);
      this.f = $$2;
   }

   public void a() {
      this.c++;
   }

   public boolean b() {
      return this.c <= 0;
   }

   public boolean c() {
      return this.c >= this.a.size();
   }

   @Nullable
   public epq d() {
      return !this.a.isEmpty() ? this.a.get(this.a.size() - 1) : null;
   }

   public epq a(int $$0) {
      return this.a.get($$0);
   }

   public void b(int $$0) {
      if (this.a.size() > $$0) {
         this.a.subList($$0, this.a.size()).clear();
      }
   }

   public void a(int $$0, epq $$1) {
      this.a.set($$0, $$1);
   }

   public int e() {
      return this.a.size();
   }

   public int f() {
      return this.c;
   }

   public void c(int $$0) {
      this.c = $$0;
   }

   public ewu a(brv $$0, int $$1) {
      epq $$2 = this.a.get($$1);
      double $$3 = (double)$$2.a + (double)((int)($$0.do() + 1.0F)) * 0.5;
      double $$4 = (double)$$2.b;
      double $$5 = (double)$$2.c + (double)((int)($$0.do() + 1.0F)) * 0.5;
      return new ewu($$3, $$4, $$5);
   }

   public ir d(int $$0) {
      return this.a.get($$0).a();
   }

   public ewu a(brv $$0) {
      return this.a($$0, this.c);
   }

   public ir g() {
      return this.a.get(this.c).a();
   }

   public epq h() {
      return this.a.get(this.c);
   }

   @Nullable
   public epq i() {
      return this.c > 0 ? this.a.get(this.c - 1) : null;
   }

   public boolean a(@Nullable eps $$0) {
      if ($$0 == null) {
         return false;
      } else if ($$0.a.size() != this.a.size()) {
         return false;
      } else {
         for (int $$1 = 0; $$1 < this.a.size(); $$1++) {
            epq $$2 = this.a.get($$1);
            epq $$3 = $$0.a.get($$1);
            if ($$2.a != $$3.a || $$2.b != $$3.b || $$2.c != $$3.c) {
               return false;
            }
         }

         return true;
      }
   }

   public boolean j() {
      return this.f;
   }

   @azp
   void a(epq[] $$0, epq[] $$1, Set<epz> $$2) {
      this.b = new eps.a($$0, $$1, $$2);
   }

   @Nullable
   public eps.a k() {
      return this.b;
   }

   public void a(we $$0) {
      if (this.b != null && !this.b.c.isEmpty()) {
         $$0.a(this.f);
         $$0.p(this.c);
         $$0.a(this.d);
         $$0.a(this.a, ($$0x, $$1) -> $$1.a($$0x));
         this.b.a($$0);
      }
   }

   public static eps b(we $$0) {
      boolean $$1 = $$0.readBoolean();
      int $$2 = $$0.readInt();
      ir $$3 = $$0.e();
      List<epq> $$4 = $$0.a(epq::b);
      eps.a $$5 = eps.a.b($$0);
      eps $$6 = new eps($$4, $$3, $$1);
      $$6.b = $$5;
      $$6.c = $$2;
      return $$6;
   }

   @Override
   public String toString() {
      return "Path(length=" + this.a.size() + ")";
   }

   public ir l() {
      return this.d;
   }

   public float m() {
      return this.e;
   }

   static epq[] c(we $$0) {
      epq[] $$1 = new epq[$$0.l()];

      for (int $$2 = 0; $$2 < $$1.length; $$2++) {
         $$1[$$2] = epq.b($$0);
      }

      return $$1;
   }

   static void a(we $$0, epq[] $$1) {
      $$0.c($$1.length);

      for (epq $$2 : $$1) {
         $$2.a($$0);
      }
   }

   public eps n() {
      eps $$0 = new eps(this.a, this.d, this.f);
      $$0.b = this.b;
      $$0.c = this.c;
      return $$0;
   }

   public static record a(epq[] a, epq[] b, Set<epz> c) {

      public void a(we $$0) {
         $$0.a(this.c, ($$0x, $$1) -> $$1.a($$0x));
         eps.a($$0, this.a);
         eps.a($$0, this.b);
      }

      public static eps.a b(we $$0) {
         HashSet<epz> $$1 = $$0.a(HashSet::new, epz::c);
         epq[] $$2 = eps.c($$0);
         epq[] $$3 = eps.c($$0);
         return new eps.a($$2, $$3, $$1);
      }
   }
}
