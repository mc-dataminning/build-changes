import com.mojang.serialization.MapCodec;

public class dqd extends djt {
   public static final MapCodec<dqd> d = b(dqd::new);
   public static final dyk<dyq> e = dyd.ai;
   public static final dye f = dyd.x;

   @Override
   public MapCodec<dqd> a() {
      return d;
   }

   protected dqd(dxm.d $$0) {
      super(true, $$0);
      this.l(this.F.b().b(e, dyq.a).b(f, Boolean.valueOf(false)).b(c, Boolean.valueOf(false)));
   }

   protected boolean a(dha $$0, jh $$1, dxn $$2, boolean $$3, int $$4) {
      if ($$4 >= 8) {
         return false;
      } else {
         int $$5 = $$1.u();
         int $$6 = $$1.v();
         int $$7 = $$1.w();
         boolean $$8 = true;
         dyq $$9 = $$2.c(e);
         switch ($$9) {
            case a:
               if ($$3) {
                  $$7++;
               } else {
                  $$7--;
               }
               break;
            case b:
               if ($$3) {
                  $$5--;
               } else {
                  $$5++;
               }
               break;
            case c:
               if ($$3) {
                  $$5--;
               } else {
                  $$5++;
                  $$6++;
                  $$8 = false;
               }

               $$9 = dyq.b;
               break;
            case d:
               if ($$3) {
                  $$5--;
                  $$6++;
                  $$8 = false;
               } else {
                  $$5++;
               }

               $$9 = dyq.b;
               break;
            case e:
               if ($$3) {
                  $$7++;
               } else {
                  $$7--;
                  $$6++;
                  $$8 = false;
               }

               $$9 = dyq.a;
               break;
            case f:
               if ($$3) {
                  $$7++;
                  $$6++;
                  $$8 = false;
               } else {
                  $$7--;
               }

               $$9 = dyq.a;
         }

         return this.a($$0, new jh($$5, $$6, $$7), $$3, $$4, $$9) ? true : $$8 && this.a($$0, new jh($$5, $$6 - 1, $$7), $$3, $$4, $$9);
      }
   }

   protected boolean a(dha $$0, jh $$1, boolean $$2, int $$3, dyq $$4) {
      dxn $$5 = $$0.a_($$1);
      if (!$$5.a(this)) {
         return false;
      } else {
         dyq $$6 = $$5.c(e);
         if ($$4 != dyq.b || $$6 != dyq.a && $$6 != dyq.e && $$6 != dyq.f) {
            if ($$4 != dyq.a || $$6 != dyq.b && $$6 != dyq.c && $$6 != dyq.d) {
               if (!$$5.c(f)) {
                  return false;
               } else {
                  return $$0.C($$1) ? true : this.a($$0, $$1, $$5, $$2, $$3 + 1);
               }
            } else {
               return false;
            }
         } else {
            return false;
         }
      }
   }

   @Override
   protected void a(dxn $$0, dha $$1, jh $$2, dke $$3) {
      boolean $$4 = $$0.c(f);
      boolean $$5 = $$1.C($$2) || this.a($$1, $$2, $$0, true, 0) || this.a($$1, $$2, $$0, false, 0);
      if ($$5 != $$4) {
         $$1.a($$2, $$0.b(f, Boolean.valueOf($$5)), 3);
         $$1.a($$2.e(), this);
         if ($$0.c(e).b()) {
            $$1.a($$2.d(), this);
         }
      }
   }

   @Override
   public dyp<dyq> c() {
      return e;
   }

   @Override
   protected dxn a(dxn $$0, dqu $$1) {
      switch ($$1) {
         case c:
            switch ((dyq)$$0.c(e)) {
               case c:
                  return $$0.b(e, dyq.d);
               case d:
                  return $$0.b(e, dyq.c);
               case e:
                  return $$0.b(e, dyq.f);
               case f:
                  return $$0.b(e, dyq.e);
               case g:
                  return $$0.b(e, dyq.i);
               case h:
                  return $$0.b(e, dyq.j);
               case i:
                  return $$0.b(e, dyq.g);
               case j:
                  return $$0.b(e, dyq.h);
            }
         case d:
            switch ((dyq)$$0.c(e)) {
               case a:
                  return $$0.b(e, dyq.b);
               case b:
                  return $$0.b(e, dyq.a);
               case c:
                  return $$0.b(e, dyq.e);
               case d:
                  return $$0.b(e, dyq.f);
               case e:
                  return $$0.b(e, dyq.d);
               case f:
                  return $$0.b(e, dyq.c);
               case g:
                  return $$0.b(e, dyq.j);
               case h:
                  return $$0.b(e, dyq.g);
               case i:
                  return $$0.b(e, dyq.h);
               case j:
                  return $$0.b(e, dyq.i);
            }
         case b:
            switch ((dyq)$$0.c(e)) {
               case a:
                  return $$0.b(e, dyq.b);
               case b:
                  return $$0.b(e, dyq.a);
               case c:
                  return $$0.b(e, dyq.f);
               case d:
                  return $$0.b(e, dyq.e);
               case e:
                  return $$0.b(e, dyq.c);
               case f:
                  return $$0.b(e, dyq.d);
               case g:
                  return $$0.b(e, dyq.h);
               case h:
                  return $$0.b(e, dyq.i);
               case i:
                  return $$0.b(e, dyq.j);
               case j:
                  return $$0.b(e, dyq.g);
            }
         default:
            return $$0;
      }
   }

   @Override
   protected dxn a(dxn $$0, dpd $$1) {
      dyq $$2 = $$0.c(e);
      switch ($$1) {
         case b:
            switch ($$2) {
               case e:
                  return $$0.b(e, dyq.f);
               case f:
                  return $$0.b(e, dyq.e);
               case g:
                  return $$0.b(e, dyq.j);
               case h:
                  return $$0.b(e, dyq.i);
               case i:
                  return $$0.b(e, dyq.h);
               case j:
                  return $$0.b(e, dyq.g);
               default:
                  return super.a($$0, $$1);
            }
         case c:
            switch ($$2) {
               case c:
                  return $$0.b(e, dyq.d);
               case d:
                  return $$0.b(e, dyq.c);
               case e:
               case f:
               default:
                  break;
               case g:
                  return $$0.b(e, dyq.h);
               case h:
                  return $$0.b(e, dyq.g);
               case i:
                  return $$0.b(e, dyq.j);
               case j:
                  return $$0.b(e, dyq.i);
            }
      }

      return super.a($$0, $$1);
   }

   @Override
   protected void a(dxo.a<dke, dxn> $$0) {
      $$0.a(e, f, c);
   }
}
