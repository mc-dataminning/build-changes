import it.unimi.dsi.fastutil.longs.Long2ObjectMap;
import it.unimi.dsi.fastutil.longs.Long2ObjectOpenHashMap;
import java.util.List;
import javax.annotation.Nullable;

public class erp extends esa {
   private final Long2ObjectMap<erv> l = new Long2ObjectOpenHashMap();
   private static final float m = 1.0F;
   private static final float n = 1.1F;
   private static final int o = 10;

   @Override
   public void a(dfi $$0, bux $$1) {
      super.a($$0, $$1);
      this.l.clear();
      $$1.G();
   }

   @Override
   public void b() {
      this.b.I();
      this.l.clear();
      super.b();
   }

   @Override
   public erq a() {
      int $$0;
      if (this.f() && this.b.bk()) {
         $$0 = this.b.dD();
         jg.a $$1 = new jg.a(this.b.dC(), (double)$$0, this.b.dI());

         for (dvd $$2 = this.a.a($$1); $$2.a(dia.G); $$2 = this.a.a($$1)) {
            $$1.b(this.b.dC(), (double)(++$$0), this.b.dI());
         }
      } else {
         $$0 = azj.a(this.b.dE() + 0.5);
      }

      jg $$4 = jg.a(this.b.dC(), (double)$$0, this.b.dI());
      if (!this.a($$4)) {
         for (jg $$5 : this.a(this.b)) {
            if (this.a($$5)) {
               return super.c($$5);
            }
         }
      }

      return super.c($$4);
   }

   @Override
   protected boolean a(jg $$0) {
      erv $$1 = this.b($$0.u(), $$0.v(), $$0.w());
      return this.b.a($$1) >= 0.0F;
   }

   @Override
   public erz a(double $$0, double $$1, double $$2) {
      return this.b($$0, $$1, $$2);
   }

   @Override
   public int a(erq[] $$0, erq $$1) {
      int $$2 = 0;
      erq $$3 = this.a($$1.a, $$1.b, $$1.c + 1);
      if (this.c($$3)) {
         $$0[$$2++] = $$3;
      }

      erq $$4 = this.a($$1.a - 1, $$1.b, $$1.c);
      if (this.c($$4)) {
         $$0[$$2++] = $$4;
      }

      erq $$5 = this.a($$1.a + 1, $$1.b, $$1.c);
      if (this.c($$5)) {
         $$0[$$2++] = $$5;
      }

      erq $$6 = this.a($$1.a, $$1.b, $$1.c - 1);
      if (this.c($$6)) {
         $$0[$$2++] = $$6;
      }

      erq $$7 = this.a($$1.a, $$1.b + 1, $$1.c);
      if (this.c($$7)) {
         $$0[$$2++] = $$7;
      }

      erq $$8 = this.a($$1.a, $$1.b - 1, $$1.c);
      if (this.c($$8)) {
         $$0[$$2++] = $$8;
      }

      erq $$9 = this.a($$1.a, $$1.b + 1, $$1.c + 1);
      if (this.c($$9) && this.b($$3) && this.b($$7)) {
         $$0[$$2++] = $$9;
      }

      erq $$10 = this.a($$1.a - 1, $$1.b + 1, $$1.c);
      if (this.c($$10) && this.b($$4) && this.b($$7)) {
         $$0[$$2++] = $$10;
      }

      erq $$11 = this.a($$1.a + 1, $$1.b + 1, $$1.c);
      if (this.c($$11) && this.b($$5) && this.b($$7)) {
         $$0[$$2++] = $$11;
      }

      erq $$12 = this.a($$1.a, $$1.b + 1, $$1.c - 1);
      if (this.c($$12) && this.b($$6) && this.b($$7)) {
         $$0[$$2++] = $$12;
      }

      erq $$13 = this.a($$1.a, $$1.b - 1, $$1.c + 1);
      if (this.c($$13) && this.b($$3) && this.b($$8)) {
         $$0[$$2++] = $$13;
      }

      erq $$14 = this.a($$1.a - 1, $$1.b - 1, $$1.c);
      if (this.c($$14) && this.b($$4) && this.b($$8)) {
         $$0[$$2++] = $$14;
      }

      erq $$15 = this.a($$1.a + 1, $$1.b - 1, $$1.c);
      if (this.c($$15) && this.b($$5) && this.b($$8)) {
         $$0[$$2++] = $$15;
      }

      erq $$16 = this.a($$1.a, $$1.b - 1, $$1.c - 1);
      if (this.c($$16) && this.b($$6) && this.b($$8)) {
         $$0[$$2++] = $$16;
      }

      erq $$17 = this.a($$1.a + 1, $$1.b, $$1.c - 1);
      if (this.c($$17) && this.b($$6) && this.b($$5)) {
         $$0[$$2++] = $$17;
      }

      erq $$18 = this.a($$1.a + 1, $$1.b, $$1.c + 1);
      if (this.c($$18) && this.b($$3) && this.b($$5)) {
         $$0[$$2++] = $$18;
      }

      erq $$19 = this.a($$1.a - 1, $$1.b, $$1.c - 1);
      if (this.c($$19) && this.b($$6) && this.b($$4)) {
         $$0[$$2++] = $$19;
      }

      erq $$20 = this.a($$1.a - 1, $$1.b, $$1.c + 1);
      if (this.c($$20) && this.b($$3) && this.b($$4)) {
         $$0[$$2++] = $$20;
      }

      erq $$21 = this.a($$1.a + 1, $$1.b + 1, $$1.c - 1);
      if (this.c($$21) && this.b($$17) && this.b($$6) && this.b($$5) && this.b($$7) && this.b($$12) && this.b($$11)) {
         $$0[$$2++] = $$21;
      }

      erq $$22 = this.a($$1.a + 1, $$1.b + 1, $$1.c + 1);
      if (this.c($$22) && this.b($$18) && this.b($$3) && this.b($$5) && this.b($$7) && this.b($$9) && this.b($$11)) {
         $$0[$$2++] = $$22;
      }

      erq $$23 = this.a($$1.a - 1, $$1.b + 1, $$1.c - 1);
      if (this.c($$23) && this.b($$19) && this.b($$6) && this.b($$4) && this.b($$7) && this.b($$12) && this.b($$10)) {
         $$0[$$2++] = $$23;
      }

      erq $$24 = this.a($$1.a - 1, $$1.b + 1, $$1.c + 1);
      if (this.c($$24) && this.b($$20) && this.b($$3) && this.b($$4) && this.b($$7) && this.b($$9) && this.b($$10)) {
         $$0[$$2++] = $$24;
      }

      erq $$25 = this.a($$1.a + 1, $$1.b - 1, $$1.c - 1);
      if (this.c($$25) && this.b($$17) && this.b($$6) && this.b($$5) && this.b($$8) && this.b($$16) && this.b($$15)) {
         $$0[$$2++] = $$25;
      }

      erq $$26 = this.a($$1.a + 1, $$1.b - 1, $$1.c + 1);
      if (this.c($$26) && this.b($$18) && this.b($$3) && this.b($$5) && this.b($$8) && this.b($$13) && this.b($$15)) {
         $$0[$$2++] = $$26;
      }

      erq $$27 = this.a($$1.a - 1, $$1.b - 1, $$1.c - 1);
      if (this.c($$27) && this.b($$19) && this.b($$6) && this.b($$4) && this.b($$8) && this.b($$16) && this.b($$14)) {
         $$0[$$2++] = $$27;
      }

      erq $$28 = this.a($$1.a - 1, $$1.b - 1, $$1.c + 1);
      if (this.c($$28) && this.b($$20) && this.b($$3) && this.b($$4) && this.b($$8) && this.b($$13) && this.b($$14)) {
         $$0[$$2++] = $$28;
      }

      return $$2;
   }

   private boolean b(@Nullable erq $$0) {
      return $$0 != null && $$0.k >= 0.0F;
   }

   private boolean c(@Nullable erq $$0) {
      return $$0 != null && !$$0.i;
   }

   @Nullable
   @Override
   protected erq a(int $$0, int $$1, int $$2) {
      erq $$3 = null;
      erv $$4 = this.b($$0, $$1, $$2);
      float $$5 = this.b.a($$4);
      if ($$5 >= 0.0F) {
         $$3 = this.c($$0, $$1, $$2);
         $$3.l = $$4;
         $$3.k = Math.max($$3.k, $$5);
         if ($$4 == erv.c) {
            $$3.k++;
         }
      }

      return $$3;
   }

   @Override
   protected erv b(int $$0, int $$1, int $$2) {
      return (erv)this.l.computeIfAbsent(jg.a($$0, $$1, $$2), $$3 -> this.a(this.a, $$0, $$1, $$2, this.b));
   }

   @Override
   public erv a(erx $$0, int $$1, int $$2, int $$3) {
      erv $$4 = $$0.a($$1, $$2, $$3);
      if ($$4 == erv.b && $$2 >= $$0.a().I_() + 1) {
         jg $$5 = new jg($$1, $$2 - 1, $$3);
         erv $$6 = $$0.a($$5.u(), $$5.v(), $$5.w());
         if ($$6 == erv.o || $$6 == erv.i) {
            $$4 = erv.o;
         } else if ($$6 == erv.q) {
            $$4 = erv.q;
         } else if ($$6 == erv.x) {
            $$4 = erv.x;
         } else if ($$6 == erv.h) {
            if (!$$5.equals($$0.b())) {
               $$4 = erv.h;
            }
         } else {
            $$4 = $$6 != erv.c && $$6 != erv.b && $$6 != erv.j ? erv.c : erv.b;
         }
      }

      if ($$4 == erv.c || $$4 == erv.b) {
         $$4 = a($$0, $$1, $$2, $$3, $$4);
      }

      return $$4;
   }

   private Iterable<jg> a(bux $$0) {
      ezc $$1 = $$0.cS();
      boolean $$2 = $$1.a() < 1.0;
      if (!$$2) {
         return List.of(
            jg.a($$1.a, (double)$$0.dD(), $$1.c),
            jg.a($$1.a, (double)$$0.dD(), $$1.f),
            jg.a($$1.d, (double)$$0.dD(), $$1.c),
            jg.a($$1.d, (double)$$0.dD(), $$1.f)
         );
      } else {
         double $$3 = Math.max(0.0, 1.1F - $$1.d());
         double $$4 = Math.max(0.0, 1.1F - $$1.b());
         double $$5 = Math.max(0.0, 1.1F - $$1.c());
         ezc $$6 = $$1.c($$4, $$5, $$3);
         return jg.a($$0.ea(), 10, azj.a($$6.a), azj.a($$6.b), azj.a($$6.c), azj.a($$6.d), azj.a($$6.e), azj.a($$6.f));
      }
   }
}
