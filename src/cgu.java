import javax.annotation.Nullable;

public abstract class cgu extends cgt implements cgx {
   private iq<cmr> c = iq.a(36, cmr.f);
   @Nullable
   private ahd d;
   private long e;

   protected cgu(blt<?> $$0, cti $$1) {
      super($$0, $$1);
   }

   protected cgu(blt<?> $$0, double $$1, double $$2, double $$3, cti $$4) {
      super($$0, $$4, $$1, $$2, $$3);
   }

   @Override
   public void a(bkn $$0) {
      super.a($$0);
      this.a($$0, this.dM(), this);
   }

   @Override
   public cmr a(int $$0) {
      return this.f_($$0);
   }

   @Override
   public cmr a(int $$0, int $$1) {
      return this.b($$0, $$1);
   }

   @Override
   public cmr b(int $$0) {
      return this.e_($$0);
   }

   @Override
   public void a(int $$0, cmr $$1) {
      this.c($$0, $$1);
   }

   @Override
   public bmx a_(int $$0) {
      return this.g_($$0);
   }

   @Override
   public void e() {
   }

   @Override
   public boolean a(cfb $$0) {
      return this.g($$0);
   }

   @Override
   public void a(blp.c $$0) {
      if (!this.dM().B && $$0.a()) {
         bjr.a(this.dM(), this, this);
      }

      super.a($$0);
   }

   @Override
   protected void b(sl $$0) {
      super.b($$0);
      this.c($$0);
   }

   @Override
   protected void a(sl $$0) {
      super.a($$0);
      this.b_($$0);
   }

   @Override
   public bjv a(cfb $$0, bju $$1) {
      return this.c_($$0);
   }

   @Override
   protected void u() {
      float $$0 = 0.98F;
      if (this.d == null) {
         int $$1 = 15 - chu.b(this);
         $$0 += (float)$$1 * 0.001F;
      }

      if (this.aZ()) {
         $$0 *= 0.95F;
      }

      this.g(this.dp().d((double)$$0, 0.0, (double)$$0));
   }

   @Override
   public void a() {
      this.f();
   }

   public void a(ahd $$0, long $$1) {
      this.d = $$0;
      this.e = $$1;
   }

   @Nullable
   @Override
   public chu createMenu(int $$0, cfa $$1, cfb $$2) {
      if (this.d != null && $$2.P_()) {
         return null;
      } else {
         this.f($$1.m);
         return this.a($$0, $$1);
      }
   }

   protected abstract chu a(int var1, cfa var2);

   @Nullable
   @Override
   public ahd C() {
      return this.d;
   }

   @Override
   public void a(@Nullable ahd $$0) {
      this.d = $$0;
   }

   @Override
   public long D() {
      return this.e;
   }

   @Override
   public void a(long $$0) {
      this.e = $$0;
   }

   @Override
   public iq<cmr> E() {
      return this.c;
   }

   @Override
   public void G() {
      this.c = iq.a(this.b(), cmr.f);
   }
}
