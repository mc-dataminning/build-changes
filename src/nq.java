...
public class nq implements ma {
   ...
   public CompletableFuture<?> a(ly $$0) {
      ...
      Map<akr, Supplier<JsonElement>> $$3 = Maps.newHashMap();
      Set<cul> $$4 = Sets.newHashSet();
      BiConsumer<akr, Supplier<JsonElement>> $$5 = ($$1x, $$2x) -> {
         Supplier<JsonElement> $$3x = $$3.put($$1x, $$2x);
         if ($$3x != null) {
            throw new IllegalStateException("Duplicate model definition for " + $$1x);
         }
      };
      Consumer<cul> $$6 = $$4::add;
      new no($$2, $$5, $$6).a();
      new np($$5).a();
      List<dfy> $$7 = lt.e.h().stream().filter($$0x -> true).map(Entry::getValue).filter($$1x -> !$$1.containsKey($$1x)).toList();
      if (!$$7.isEmpty()) {
