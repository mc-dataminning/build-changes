import com.mojang.serialization.Codec;
import com.mojang.serialization.MapCodec;
import com.mojang.serialization.codecs.RecordCodecBuilder;
import java.util.stream.Stream;

public class dks extends dko {
   public static final MapCodec<dks> b = RecordCodecBuilder.mapCodec(
      $$0 -> $$0.group(dkk.d.fieldOf("biomes").forGetter($$0x -> $$0x.c), Codec.intRange(0, 62).fieldOf("scale").orElse(2).forGetter($$0x -> $$0x.e))
            .apply($$0, dks::new)
   );
   private final jj<dkk> c;
   private final int d;
   private final int e;

   public dks(jj<dkk> $$0, int $$1) {
      this.c = $$0;
      this.d = $$1 + 2;
      this.e = $$1;
   }

   @Override
   protected Stream<jf<dkk>> b() {
      return this.c.a();
   }

   @Override
   protected MapCodec<? extends dko> a() {
      return b;
   }

   @Override
   public jf<dkk> getNoiseBiome(int $$0, int $$1, int $$2, dkt.f $$3) {
      return this.c.a(Math.floorMod(($$0 >> this.d) + ($$2 >> this.d), this.c.b()));
   }
}
