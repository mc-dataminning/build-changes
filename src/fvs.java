public class fvs implements fvh<dhx> {
   private final fjf a;

   public fvs(fvi.a $$0) {
      this.a = new fjf($$0.a(fmw.m));
   }

   public void a(dhx $$0, float $$1, eqb $$2, fth $$3, int $$4, int $$5) {
      djh $$6 = $$0.r();
      if ($$6.c(dba.d)) {
         $$2.a();
         $$2.a(0.5F, 1.0625F, 0.5F);
         float $$7 = $$6.c(dba.b).h().p();
         $$2.a(a.d.rotationDegrees(-$$7));
         $$2.a(a.f.rotationDegrees(67.5F));
         $$2.a(0.0F, -0.125F, 0.0F);
         this.a.a(0.0F, 0.1F, 0.9F, 1.2F);
         eqf $$8 = fvq.a.a($$3, ftp::c);
         this.a.b($$2, $$8, $$4, $$5, 1.0F, 1.0F, 1.0F, 1.0F);
         $$2.b();
      }
   }
}
