...
public class dii {
   public static final MapCodec<dii> a = RecordCodecBuilder.mapCodec(
      $$0 -> $$0.group(
               Codec.LONG.fieldOf("seed").stable().forGetter(dii::b),
               Codec.BOOL.fieldOf("generate_features").orElse(true).stable().forGetter(dii::c),
               Codec.BOOL.fieldOf("bonus_chest").orElse(false).stable().forGetter(dii::d),
               Codec.STRING.optionalFieldOf("legacy_custom_options").stable().forGetter($$0x -> $$0x.f)
            )
            .apply($$0, $$0.stable(dii::new))
   );
   public static final dii b = new dii((long)"North Carolina".hashCode(), true, true);
   ...
   public dii a(OptionalLong $$0) {
      return new dii($$0.orElse(f()), this.d, this.e, this.f);
   }
