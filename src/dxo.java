import com.mojang.serialization.Codec;
import com.mojang.serialization.codecs.RecordCodecBuilder;

public class dxo extends dxk {
   public static final Codec<dxo> c = RecordCodecBuilder.create($$0 -> a($$0).apply($$0, dxo::new));

   public dxo(blb $$0, blb $$1, int $$2) {
      super($$0, $$1, $$2);
   }

   @Override
   protected dxq<?> a() {
      return dxq.f;
   }

   @Override
   protected void a(cvx $$0, dxp.b $$1, awp $$2, dwz $$3, int $$4, dxp.a $$5, int $$6, int $$7, int $$8) {
      for (int $$9 = $$8; $$9 >= $$8 - $$6; $$9--) {
         int $$10 = $$7 + ($$9 != $$8 && $$9 != $$8 - $$6 ? 1 : 0);
         this.a($$0, $$1, $$2, $$3, $$5.a(), $$10, $$9, $$5.c());
      }
   }

   @Override
   protected boolean a(awp $$0, int $$1, int $$2, int $$3, int $$4, boolean $$5) {
      return awi.k((float)$$1 + 0.5F) + awi.k((float)$$3 + 0.5F) > (float)($$4 * $$4);
   }
}
