import com.mojang.logging.LogUtils;
import javax.annotation.Nullable;
import org.slf4j.Logger;

public class dtn extends dty implements bsm {
   private static final Logger b = LogUtils.getLogger();
   public static final int a = 6;
   private static final String c = "patterns";
   @Nullable
   private wp d;
   private final cvl e;
   private dtp f = dtp.a;

   public dtn(ji $$0, dww $$1) {
      this($$0, $$1, ((dic)$$1.b()).b());
   }

   public dtn(ji $$0, dww $$1, cvl $$2) {
      super(dua.u, $$0, $$1);
      this.e = $$2;
   }

   @Override
   public wp al() {
      return (wp)(this.d != null ? this.d : wp.c("block.minecraft.banner"));
   }

   @Nullable
   @Override
   public wp an() {
      return this.d;
   }

   @Override
   protected void b(tq $$0, jt.a $$1) {
      super.b($$0, $$1);
      if (!this.f.equals(dtp.a)) {
         $$0.a("patterns", (un)dtp.b.encodeStart($$1.a(ue.a), this.f).getOrThrow());
      }

      if (this.d != null) {
         $$0.a("CustomName", wp.a.a(this.d, $$1));
      }
   }

   @Override
   protected void a(tq $$0, jt.a $$1) {
      super.a($$0, $$1);
      if ($$0.b("CustomName", 8)) {
         this.d = a($$0.l("CustomName"), $$1);
      }

      if ($$0.e("patterns")) {
         dtp.b
            .parse($$1.a(ue.a), $$0.c("patterns"))
            .resultOrPartial($$0x -> b.error("Failed to parse banner patterns: '{}'", $$0x))
            .ifPresent($$0x -> this.f = $$0x);
      }
   }

   public abs a() {
      return abs.a(this);
   }

   @Override
   public tq a(jt.a $$0) {
      return this.d($$0);
   }

   public dtp b() {
      return this.f;
   }

   public cwo c() {
      cwo $$0 = new cwo(diq.a(this.e));
      $$0.b(this.q());
      return $$0;
   }

   public cvl f() {
      return this.e;
   }

   @Override
   protected void a(dty.b $$0) {
      super.a($$0);
      this.f = $$0.a(kv.ai, dtp.a);
      this.d = $$0.a(kv.g);
   }

   @Override
   protected void a(kr.a $$0) {
      super.a($$0);
      $$0.a(kv.ai, this.f);
      $$0.a(kv.g, this.d);
   }

   @Override
   public void a(tq $$0) {
      $$0.r("patterns");
      $$0.r("CustomName");
   }
}
