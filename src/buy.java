import javax.annotation.Nullable;
import org.joml.Vector3f;

public class buy extends but {
   private static final cmi cb = cmi.a(cjk.oH, cjk.rk, cjk.rj, cjk.ur, cjk.uo, cjk.up);
   public float bT;
   public float bU;
   public float bW;
   public float bX;
   public float bY = 1.0F;
   private float cc = 1.0F;
   public int bZ = this.ag.a(6000) + 6000;
   public boolean ca;

   public buy(biw<? extends buy> $$0, cpx $$1) {
      super($$0, $$1);
      this.a(eaz.j, 0.0F);
   }

   @Override
   protected void z() {
      this.bO.a(0, new bpx(this));
      this.bO.a(1, new bqw(this, 1.4));
      this.bO.a(2, new bpp(this, 1.0));
      this.bO.a(3, new brl(this, 1.0, cb, false));
      this.bO.a(4, new bqc(this, 1.1));
      this.bO.a(5, new brq(this, 1.0));
      this.bO.a(6, new bql(this, cbw.class, 6.0F));
      this.bO.a(7, new bqy(this));
   }

   @Override
   protected float b(bju $$0, bit $$1) {
      return this.m_() ? $$1.b * 0.85F : $$1.b * 0.92F;
   }

   public static bkn.a s() {
      return bjk.A().a(bko.a, 4.0).a(bko.d, 0.25);
   }

   @Override
   public void b_() {
      super.b_();
      this.bX = this.bT;
      this.bW = this.bU;
      this.bU = this.bU + (this.aA() ? -1.0F : 4.0F) * 0.3F;
      this.bU = ary.a(this.bU, 0.0F, 1.0F);
      if (!this.aA() && this.bY < 1.0F) {
         this.bY = 1.0F;
      }

      this.bY *= 0.9F;
      ehp $$0 = this.do();
      if (!this.aA() && $$0.d < 0.0) {
         this.f($$0.d(1.0, 0.6, 1.0));
      }

      this.bT = this.bT + this.bY * 2.0F;
      if (!this.dL().B && this.bv() && !this.m_() && !this.t() && --this.bZ <= 0) {
         this.a(apf.em, 1.0F, (this.ag.i() - this.ag.i()) * 0.2F + 1.0F);
         this.a(cjk.qd);
         this.a(djv.t);
         this.bZ = this.ag.a(6000) + 6000;
      }
   }

   @Override
   protected boolean aR() {
      return this.aa > this.cc;
   }

   @Override
   protected void aQ() {
      this.cc = this.aa + this.bU / 2.0F;
   }

   @Override
   protected ape w() {
      return apf.ek;
   }

   @Override
   protected ape d(bhq $$0) {
      return apf.en;
   }

   @Override
   protected ape l_() {
      return apf.el;
   }

   @Override
   protected void b(gw $$0, dfl $$1) {
      this.a(apf.eo, 0.15F, 1.0F);
   }

   @Nullable
   public buy b(aks $$0, bin $$1) {
      return biw.q.a((cpx)$$0);
   }

   @Override
   public boolean m(cjh $$0) {
      return cb.a($$0);
   }

   @Override
   public int ed() {
      return this.t() ? 10 : super.ed();
   }

   @Override
   public void a(qy $$0) {
      super.a($$0);
      this.ca = $$0.q("IsChickenJockey");
      if ($$0.e("EggLayTime")) {
         this.bZ = $$0.h("EggLayTime");
      }
   }

   @Override
   public void b(qy $$0) {
      super.b($$0);
      $$0.a("IsChickenJockey", this.ca);
      $$0.a("EggLayTime", this.bZ);
   }

   @Override
   public boolean h(double $$0) {
      return this.t();
   }

   @Override
   protected void a(bis $$0, bis.a $$1) {
      super.a($$0, $$1);
      if ($$0 instanceof bji) {
         ((bji)$$0).aU = this.aU;
      }
   }

   @Override
   protected Vector3f a(bis $$0, bit $$1, float $$2) {
      return new Vector3f(0.0F, $$1.b, -0.1F * $$2);
   }

   public boolean t() {
      return this.ca;
   }

   public void w(boolean $$0) {
      this.ca = $$0;
   }
}
