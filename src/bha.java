import com.google.common.annotations.VisibleForTesting;
import com.google.common.collect.ImmutableList;
import com.google.common.collect.ImmutableSet;
import com.google.common.collect.Maps;
import com.google.common.collect.Sets;
import com.google.common.collect.UnmodifiableIterator;
import com.google.common.collect.ImmutableList.Builder;
import com.mojang.datafixers.util.Pair;
import com.mojang.logging.LogUtils;
import com.mojang.serialization.Codec;
import com.mojang.serialization.DataResult;
import com.mojang.serialization.Dynamic;
import com.mojang.serialization.DynamicOps;
import com.mojang.serialization.MapCodec;
import com.mojang.serialization.MapLike;
import com.mojang.serialization.RecordBuilder;
import it.unimi.dsi.fastutil.objects.ObjectArrayList;
import java.util.Collection;
import java.util.List;
import java.util.Map;
import java.util.Optional;
import java.util.Set;
import java.util.Map.Entry;
import java.util.function.Supplier;
import java.util.stream.Stream;
import javax.annotation.Nullable;
import org.apache.commons.lang3.mutable.MutableObject;
import org.slf4j.Logger;

public class bha<E extends bfz> {
   static final Logger a = LogUtils.getLogger();
   private final Supplier<Codec<bha<E>>> b;
   private static final int c = 20;
   private final Map<bpb<?>, Optional<? extends bpa<?>>> d = Maps.newHashMap();
   private final Map<bqg<? extends bqf<? super E>>, bqf<? super E>> e = Maps.newLinkedHashMap();
   private final Map<Integer, Map<bzz, Set<bhs<? super E>>>> f = Maps.newTreeMap();
   private cab g = cab.c;
   private final Map<bzz, Set<Pair<bpb<?>, bpc>>> h = Maps.newHashMap();
   private final Map<bzz, Set<bpb<?>>> i = Maps.newHashMap();
   private Set<bzz> j = Sets.newHashSet();
   private final Set<bzz> k = Sets.newHashSet();
   private bzz l = bzz.b;
   private long m = -9999L;

   public static <E extends bfz> bha.b<E> a(Collection<? extends bpb<?>> $$0, Collection<? extends bqg<? extends bqf<? super E>>> $$1) {
      return new bha.b<>($$0, $$1);
   }

   public static <E extends bfz> Codec<bha<E>> b(final Collection<? extends bpb<?>> $$0, final Collection<? extends bqg<? extends bqf<? super E>>> $$1) {
      final MutableObject<Codec<bha<E>>> $$2 = new MutableObject();
      $$2.setValue(
         (new MapCodec<bha<E>>() {
               public <T> Stream<T> keys(DynamicOps<T> $$0x) {
                  return $$0.stream().flatMap($$0xx -> $$0xx.a().map($$1xxx -> jb.C.b($$0xx)).stream()).map($$1xx -> (T)$$0.createString($$1xx.toString()));
               }

               public <T> DataResult<bha<E>> decode(DynamicOps<T> $$0x, MapLike<T> $$1x) {
                  MutableObject<DataResult<Builder<bha.a<?>>>> $$2 = new MutableObject(DataResult.success(ImmutableList.builder()));
                  $$1.entries().forEach($$2xxx -> {
                     DataResult<bpb<?>> $$3x = jb.C.q().parse($$0, $$2xxx.getFirst());
                     DataResult<? extends bha.a<?>> $$4 = $$3x.flatMap($$2xxxxx -> this.a($$2xxxxx, $$0, (T)$$2xxx.getSecond()));
                     $$2.setValue(((DataResult)$$2.getValue()).apply2(Builder::add, $$4));
                  });
                  ImmutableList<bha.a<?>> $$3 = ((DataResult)$$2.getValue())
                     .resultOrPartial(bha.a::error)
                     .<ImmutableList<bha.a<?>>>map(Builder::build)
                     .orElseGet(ImmutableList::of);
                  return DataResult.success(new bha<>($$0, $$1, $$3, $$2::getValue));
               }

               private <T, U> DataResult<bha.a<U>> a(bpb<U> $$0x, DynamicOps<T> $$1x, T $$2x) {
                  return $$0.a()
                     .<DataResult>map(DataResult::success)
                     .orElseGet(() -> DataResult.error(() -> "No codec for memory: " + $$0))
                     .flatMap($$2xxx -> $$2xxx.parse($$1, $$2))
                     .map($$1xxx -> new bha.a<>($$0, Optional.of($$1xxx)));
               }

               public <T> RecordBuilder<T> a(bha<E> $$0x, DynamicOps<T> $$1x, RecordBuilder<T> $$2x) {
                  $$0.j().forEach($$2xxx -> $$2xxx.a($$1, $$2));
                  return $$2;
               }
            })
            .fieldOf("memories")
            .codec()
      );
      return (Codec<bha<E>>)$$2.getValue();
   }

   public bha(
      Collection<? extends bpb<?>> $$0, Collection<? extends bqg<? extends bqf<? super E>>> $$1, ImmutableList<bha.a<?>> $$2, Supplier<Codec<bha<E>>> $$3
   ) {
      this.b = $$3;

      for (bpb<?> $$4 : $$0) {
         this.d.put($$4, Optional.empty());
      }

      for (bqg<? extends bqf<? super E>> $$5 : $$1) {
         this.e.put($$5, (bqf<? super E>)$$5.a());
      }

      for (bqf<? super E> $$6 : this.e.values()) {
         for (bpb<?> $$7 : $$6.a()) {
            this.d.put($$7, Optional.empty());
         }
      }

      UnmodifiableIterator var11 = $$2.iterator();

      while (var11.hasNext()) {
         bha.a<?> $$8 = (bha.a<?>)var11.next();
         $$8.a(this);
      }
   }

   public <T> DataResult<T> a(DynamicOps<T> $$0) {
      return this.b.get().encodeStart($$0, this);
   }

   Stream<bha.a<?>> j() {
      return this.d.entrySet().stream().map($$0 -> bha.a.a($$0.getKey(), $$0.getValue()));
   }

   public boolean a(bpb<?> $$0) {
      return this.a($$0, bpc.a);
   }

   public void a() {
      this.d.keySet().forEach($$0 -> this.d.put((bpb<?>)$$0, Optional.empty()));
   }

   public <U> void b(bpb<U> $$0) {
      this.a($$0, Optional.empty());
   }

   public <U> void a(bpb<U> $$0, @Nullable U $$1) {
      this.a($$0, Optional.ofNullable($$1));
   }

   public <U> void a(bpb<U> $$0, U $$1, long $$2) {
      this.b($$0, Optional.of(bpa.a($$1, $$2)));
   }

   public <U> void a(bpb<U> $$0, Optional<? extends U> $$1) {
      this.b($$0, $$1.map(bpa::a));
   }

   <U> void b(bpb<U> $$0, Optional<? extends bpa<?>> $$1) {
      if (this.d.containsKey($$0)) {
         if ($$1.isPresent() && this.a($$1.get().c())) {
            this.b($$0);
         } else {
            this.d.put($$0, $$1);
         }
      }
   }

   public <U> Optional<U> c(bpb<U> $$0) {
      Optional<? extends bpa<?>> $$1 = this.d.get($$0);
      if ($$1 == null) {
         throw new IllegalStateException("Unregistered memory fetched: " + $$0);
      } else {
         return $$1.map(bpa::c);
      }
   }

   @Nullable
   public <U> Optional<U> d(bpb<U> $$0) {
      Optional<? extends bpa<?>> $$1 = this.d.get($$0);
      return $$1 == null ? null : $$1.map(bpa::c);
   }

   public <U> long e(bpb<U> $$0) {
      Optional<? extends bpa<?>> $$1 = this.d.get($$0);
      return $$1.<Long>map(bpa::b).orElse(0L);
   }

   @Deprecated
   @aqa
   public Map<bpb<?>, Optional<? extends bpa<?>>> b() {
      return this.d;
   }

   public <U> boolean b(bpb<U> $$0, U $$1) {
      return !this.a($$0) ? false : this.c($$0).filter($$1x -> $$1x.equals($$1)).isPresent();
   }

   public boolean a(bpb<?> $$0, bpc $$1) {
      Optional<? extends bpa<?>> $$2 = this.d.get($$0);
      return $$2 == null ? false : $$1 == bpc.c || $$1 == bpc.a && $$2.isPresent() || $$1 == bpc.b && !$$2.isPresent();
   }

   public cab c() {
      return this.g;
   }

   public void a(cab $$0) {
      this.g = $$0;
   }

   public void a(Set<bzz> $$0) {
      this.j = $$0;
   }

   @Deprecated
   @aqa
   public Set<bzz> d() {
      return this.k;
   }

   @Deprecated
   @aqa
   public List<bhs<? super E>> e() {
      List<bhs<? super E>> $$0 = new ObjectArrayList();

      for (Map<bzz, Set<bhs<? super E>>> $$1 : this.f.values()) {
         for (Set<bhs<? super E>> $$2 : $$1.values()) {
            for (bhs<? super E> $$3 : $$2) {
               if ($$3.a() == bhr.a.b) {
                  $$0.add($$3);
               }
            }
         }
      }

      return $$0;
   }

   public void f() {
      this.d(this.l);
   }

   public Optional<bzz> g() {
      for (bzz $$0 : this.k) {
         if (!this.j.contains($$0)) {
            return Optional.of($$0);
         }
      }

      return Optional.empty();
   }

   public void a(bzz $$0) {
      if (this.f($$0)) {
         this.d($$0);
      } else {
         this.f();
      }
   }

   private void d(bzz $$0) {
      if (!this.c($$0)) {
         this.e($$0);
         this.k.clear();
         this.k.addAll(this.j);
         this.k.add($$0);
      }
   }

   private void e(bzz $$0) {
      for (bzz $$1 : this.k) {
         if ($$1 != $$0) {
            Set<bpb<?>> $$2 = this.i.get($$1);
            if ($$2 != null) {
               for (bpb<?> $$3 : $$2) {
                  this.b($$3);
               }
            }
         }
      }
   }

   public void a(long $$0, long $$1) {
      if ($$1 - this.m > 20L) {
         this.m = $$1;
         bzz $$2 = this.c().a((int)($$0 % 24000L));
         if (!this.k.contains($$2)) {
            this.a($$2);
         }
      }
   }

   public void a(List<bzz> $$0) {
      for (bzz $$1 : $$0) {
         if (this.f($$1)) {
            this.d($$1);
            break;
         }
      }
   }

   public void b(bzz $$0) {
      this.l = $$0;
   }

   public void a(bzz $$0, int $$1, ImmutableList<? extends bhs<? super E>> $$2) {
      this.a($$0, this.a($$1, $$2));
   }

   public void a(bzz $$0, int $$1, ImmutableList<? extends bhs<? super E>> $$2, bpb<?> $$3) {
      Set<Pair<bpb<?>, bpc>> $$4 = ImmutableSet.of(Pair.of($$3, bpc.a));
      Set<bpb<?>> $$5 = ImmutableSet.of($$3);
      this.a($$0, this.a($$1, $$2), $$4, $$5);
   }

   public void a(bzz $$0, ImmutableList<? extends Pair<Integer, ? extends bhs<? super E>>> $$1) {
      this.a($$0, $$1, ImmutableSet.of(), Sets.newHashSet());
   }

   public void a(bzz $$0, ImmutableList<? extends Pair<Integer, ? extends bhs<? super E>>> $$1, Set<Pair<bpb<?>, bpc>> $$2) {
      this.a($$0, $$1, $$2, Sets.newHashSet());
   }

   public void a(bzz $$0, ImmutableList<? extends Pair<Integer, ? extends bhs<? super E>>> $$1, Set<Pair<bpb<?>, bpc>> $$2, Set<bpb<?>> $$3) {
      this.h.put($$0, $$2);
      if (!$$3.isEmpty()) {
         this.i.put($$0, $$3);
      }

      UnmodifiableIterator var5 = $$1.iterator();

      while (var5.hasNext()) {
         Pair<Integer, ? extends bhs<? super E>> $$4 = (Pair<Integer, ? extends bhs<? super E>>)var5.next();
         this.f
            .computeIfAbsent((Integer)$$4.getFirst(), $$0x -> Maps.newHashMap())
            .computeIfAbsent($$0, $$0x -> Sets.newLinkedHashSet())
            .add((bhs<? super E>)$$4.getSecond());
      }
   }

   @VisibleForTesting
   public void h() {
      this.f.clear();
   }

   public boolean c(bzz $$0) {
      return this.k.contains($$0);
   }

   public bha<E> i() {
      bha<E> $$0 = new bha<>(this.d.keySet(), this.e.keySet(), ImmutableList.of(), this.b);

      for (Entry<bpb<?>, Optional<? extends bpa<?>>> $$1 : this.d.entrySet()) {
         bpb<?> $$2 = $$1.getKey();
         if ($$1.getValue().isPresent()) {
            $$0.d.put($$2, $$1.getValue());
         }
      }

      return $$0;
   }

   public void a(aif $$0, E $$1) {
      this.k();
      this.c($$0, $$1);
      this.d($$0, $$1);
      this.e($$0, $$1);
   }

   private void c(aif $$0, E $$1) {
      for (bqf<? super E> $$2 : this.e.values()) {
         $$2.b($$0, $$1);
      }
   }

   private void k() {
      for (Entry<bpb<?>, Optional<? extends bpa<?>>> $$0 : this.d.entrySet()) {
         if ($$0.getValue().isPresent()) {
            bpa<?> $$1 = (bpa<?>)$$0.getValue().get();
            if ($$1.d()) {
               this.b($$0.getKey());
            }

            $$1.a();
         }
      }
   }

   public void b(aif $$0, E $$1) {
      long $$2 = $$1.dI().V();

      for (bhs<? super E> $$3 : this.e()) {
         $$3.g($$0, $$1, $$2);
      }
   }

   private void d(aif $$0, E $$1) {
      long $$2 = $$0.V();

      for (Map<bzz, Set<bhs<? super E>>> $$3 : this.f.values()) {
         for (Entry<bzz, Set<bhs<? super E>>> $$4 : $$3.entrySet()) {
            bzz $$5 = $$4.getKey();
            if (this.k.contains($$5)) {
               for (bhs<? super E> $$7 : $$4.getValue()) {
                  if ($$7.a() == bhr.a.a) {
                     $$7.e($$0, $$1, $$2);
                  }
               }
            }
         }
      }
   }

   private void e(aif $$0, E $$1) {
      long $$2 = $$0.V();

      for (bhs<? super E> $$3 : this.e()) {
         $$3.f($$0, $$1, $$2);
      }
   }

   private boolean f(bzz $$0) {
      if (!this.h.containsKey($$0)) {
         return false;
      } else {
         for (Pair<bpb<?>, bpc> $$1 : this.h.get($$0)) {
            bpb<?> $$2 = (bpb<?>)$$1.getFirst();
            bpc $$3 = (bpc)$$1.getSecond();
            if (!this.a($$2, $$3)) {
               return false;
            }
         }

         return true;
      }
   }

   private boolean a(Object $$0) {
      return $$0 instanceof Collection && ((Collection)$$0).isEmpty();
   }

   ImmutableList<? extends Pair<Integer, ? extends bhs<? super E>>> a(int $$0, ImmutableList<? extends bhs<? super E>> $$1) {
      int $$2 = $$0;
      Builder<Pair<Integer, ? extends bhs<? super E>>> $$3 = ImmutableList.builder();
      UnmodifiableIterator var5 = $$1.iterator();

      while (var5.hasNext()) {
         bhs<? super E> $$4 = (bhs<? super E>)var5.next();
         $$3.add(Pair.of($$2++, $$4));
      }

      return $$3.build();
   }

   static final class a<U> {
      private final bpb<U> a;
      private final Optional<? extends bpa<U>> b;

      static <U> bha.a<U> a(bpb<U> $$0, Optional<? extends bpa<?>> $$1) {
         return new bha.a<>($$0, (Optional<? extends bpa<U>>)$$1);
      }

      a(bpb<U> $$0, Optional<? extends bpa<U>> $$1) {
         this.a = $$0;
         this.b = $$1;
      }

      void a(bha<?> $$0) {
         $$0.b(this.a, this.b);
      }

      public <T> void a(DynamicOps<T> $$0, RecordBuilder<T> $$1) {
         this.a.a().ifPresent($$2 -> this.b.ifPresent($$3 -> $$1.add(jb.C.q().encodeStart($$0, this.a), $$2.encodeStart($$0, $$3))));
      }
   }

   public static final class b<E extends bfz> {
      private final Collection<? extends bpb<?>> a;
      private final Collection<? extends bqg<? extends bqf<? super E>>> b;
      private final Codec<bha<E>> c;

      b(Collection<? extends bpb<?>> $$0, Collection<? extends bqg<? extends bqf<? super E>>> $$1) {
         this.a = $$0;
         this.b = $$1;
         this.c = bha.b($$0, $$1);
      }

      public bha<E> a(Dynamic<?> $$0) {
         return this.c.parse($$0).resultOrPartial(bha.a::error).orElseGet(() -> new bha<>(this.a, this.b, ImmutableList.of(), () -> this.c));
      }
   }
}
