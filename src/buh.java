import com.mojang.datafixers.util.Pair;
import com.mojang.logging.LogUtils;
import com.mojang.serialization.Codec;
import it.unimi.dsi.fastutil.ints.IntSet;
import java.util.List;
import java.util.Optional;
import java.util.function.IntFunction;
import javax.annotation.Nullable;
import org.joml.Quaternionf;
import org.joml.Vector3f;
import org.slf4j.Logger;

public abstract class buh extends buj {
   static final Logger o = LogUtils.getLogger();
   public static final int a = -1;
   private static final ajx<Integer> p = akb.a(buh.class, ajz.b);
   private static final ajx<Integer> q = akb.a(buh.class, ajz.b);
   private static final ajx<Integer> r = akb.a(buh.class, ajz.b);
   private static final ajx<Vector3f> s = akb.a(buh.class, ajz.D);
   private static final ajx<Vector3f> t = akb.a(buh.class, ajz.D);
   private static final ajx<Quaternionf> u = akb.a(buh.class, ajz.E);
   private static final ajx<Quaternionf> ay = akb.a(buh.class, ajz.E);
   private static final ajx<Byte> az = akb.a(buh.class, ajz.a);
   private static final ajx<Integer> aA = akb.a(buh.class, ajz.b);
   private static final ajx<Float> aB = akb.a(buh.class, ajz.d);
   private static final ajx<Float> aC = akb.a(buh.class, ajz.d);
   private static final ajx<Float> aD = akb.a(buh.class, ajz.d);
   private static final ajx<Float> aE = akb.a(buh.class, ajz.d);
   private static final ajx<Float> aF = akb.a(buh.class, ajz.d);
   private static final ajx<Integer> aG = akb.a(buh.class, ajz.b);
   private static final IntSet aH = IntSet.of(new int[]{s.a(), t.a(), u.a(), ay.a(), az.a(), aA.a(), aC.a(), aD.a()});
   private static final float aI = 0.0F;
   private static final float aJ = 1.0F;
   private static final int aK = -1;
   public static final String b = "teleport_duration";
   public static final String c = "interpolation_duration";
   public static final String d = "start_interpolation";
   public static final String e = "transformation";
   public static final String f = "billboard";
   public static final String g = "brightness";
   public static final String h = "view_range";
   public static final String i = "shadow_radius";
   public static final String j = "shadow_strength";
   public static final String k = "width";
   public static final String l = "height";
   public static final String m = "glow_color_override";
   private long aL = -2147483648L;
   private int aM;
   private float aN;
   private fat aO;
   private boolean aP = true;
   protected boolean n;
   private boolean aQ;
   private boolean aR;
   @Nullable
   private buh.k aS;
   @Nullable
   private buh.j aT;

   public buh(buq<?> $$0, dgg $$1) {
      super($$0, $$1);
      this.ad = true;
      this.aO = this.cR();
   }

   @Override
   public void a(ajx<?> $$0) {
      super.a($$0);
      if (aF.equals($$0) || aE.equals($$0)) {
         this.E();
      }

      if (p.equals($$0)) {
         this.aQ = true;
      }

      if (q.equals($$0)) {
         this.aR = true;
      }

      if (aH.contains($$0.a())) {
         this.n = true;
      }
   }

   @Override
   public final boolean a(arc $$0, bsz $$1, float $$2) {
      return false;
   }

   private static j a(akb $$0) {
      Vector3f $$1 = $$0.a(s);
      Quaternionf $$2 = $$0.a(u);
      Vector3f $$3 = $$0.a(t);
      Quaternionf $$4 = $$0.a(ay);
      return new j($$1, $$2, $$3, $$4);
   }

   @Override
   public void h() {
      buj $$0 = this.dl();
      if ($$0 != null && $$0.dR()) {
         this.ae();
      }

      if (this.dW().C) {
         if (this.aQ) {
            this.aQ = false;
            int $$1 = this.t();
            this.aL = (long)(this.af + $$1);
         }

         if (this.aR) {
            this.aR = false;
            this.aM = this.s();
         }

         if (this.n) {
            this.n = false;
            boolean $$2 = this.aM != 0;
            if ($$2 && this.aS != null) {
               this.aS = this.a(this.aS, this.aN);
            } else {
               this.aS = this.F();
            }

            this.a($$2, this.aN);
         }

         if (this.aT != null) {
            if (this.aT.a == 0) {
               this.aT.a(this);
               this.bz();
               this.aT = null;
            } else {
               this.aT.b(this);
               this.aT.a--;
               if (this.aT.a == 0) {
                  this.aT = null;
               }
            }
         }
      }
   }

   protected abstract void a(boolean var1, float var2);

   @Override
   protected void a(akb.a $$0) {
      $$0.a(r, 0);
      $$0.a(p, 0);
      $$0.a(q, 0);
      $$0.a(s, new Vector3f());
      $$0.a(t, new Vector3f(1.0F, 1.0F, 1.0F));
      $$0.a(ay, new Quaternionf());
      $$0.a(u, new Quaternionf());
      $$0.a(az, buh.a.a.a());
      $$0.a(aA, -1);
      $$0.a(aB, 1.0F);
      $$0.a(aC, 0.0F);
      $$0.a(aD, 1.0F);
      $$0.a(aE, 0.0F);
      $$0.a(aF, 0.0F);
      $$0.a(aG, -1);
   }

   @Override
   protected void a(tq $$0) {
      if ($$0.e("transformation")) {
         j.b.decode(ue.a, $$0.c("transformation")).resultOrPartial(af.a("Display entity", o::error)).ifPresent($$0x -> this.a((j)$$0x.getFirst()));
      }

      if ($$0.b("interpolation_duration", 99)) {
         int $$1 = $$0.h("interpolation_duration");
         this.b($$1);
      }

      if ($$0.b("start_interpolation", 99)) {
         int $$2 = $$0.h("start_interpolation");
         this.c($$2);
      }

      if ($$0.b("teleport_duration", 99)) {
         int $$3 = $$0.h("teleport_duration");
         this.d(ayy.a($$3, 0, 59));
      }

      if ($$0.b("billboard", 8)) {
         buh.a.e.decode(ue.a, $$0.c("billboard")).resultOrPartial(af.a("Display entity", o::error)).ifPresent($$0x -> this.a((buh.a)$$0x.getFirst()));
      }

      if ($$0.b("view_range", 99)) {
         this.b($$0.j("view_range"));
      }

      if ($$0.b("shadow_radius", 99)) {
         this.c($$0.j("shadow_radius"));
      }

      if ($$0.b("shadow_strength", 99)) {
         this.x($$0.j("shadow_strength"));
      }

      if ($$0.b("width", 99)) {
         this.y($$0.j("width"));
      }

      if ($$0.b("height", 99)) {
         this.z($$0.j("height"));
      }

      if ($$0.b("glow_color_override", 99)) {
         this.l($$0.h("glow_color_override"));
      }

      if ($$0.b("brightness", 10)) {
         axo.b.decode(ue.a, $$0.c("brightness")).resultOrPartial(af.a("Display entity", o::error)).ifPresent($$0x -> this.a((axo)$$0x.getFirst()));
      } else {
         this.a(null);
      }
   }

   private void a(j $$0) {
      this.al.a(s, $$0.d());
      this.al.a(u, $$0.e());
      this.al.a(t, $$0.f());
      this.al.a(ay, $$0.g());
   }

   @Override
   protected void b(tq $$0) {
      j.b.encodeStart(ue.a, a(this.al)).ifSuccess($$1x -> $$0.a("transformation", $$1x));
      buh.a.e.encodeStart(ue.a, this.v()).ifSuccess($$1x -> $$0.a("billboard", $$1x));
      $$0.a("interpolation_duration", this.s());
      $$0.a("teleport_duration", this.u());
      $$0.a("view_range", this.y());
      $$0.a("shadow_radius", this.z());
      $$0.a("shadow_strength", this.A());
      $$0.a("width", this.B());
      $$0.a("height", this.D());
      $$0.a("glow_color_override", this.C());
      axo $$1 = this.w();
      if ($$1 != null) {
         axo.b.encodeStart(ue.a, $$1).ifSuccess($$1x -> $$0.a("brightness", $$1x));
      }
   }

   @Override
   public void f_() {
      this.aT = null;
   }

   @Override
   public void a(double $$0, double $$1, double $$2, float $$3, float $$4, int $$5) {
      int $$6 = this.u();
      this.aT = new buh.j($$6, $$0, $$1, $$2, (double)$$3, (double)$$4);
   }

   @Override
   public double R_() {
      return this.aT != null ? this.aT.b : this.dB();
   }

   @Override
   public double S_() {
      return this.aT != null ? this.aT.c : this.dD();
   }

   @Override
   public double g_() {
      return this.aT != null ? this.aT.d : this.dH();
   }

   @Override
   public float r_() {
      return this.aT != null ? (float)this.aT.f : this.dO();
   }

   @Override
   public float h_() {
      return this.aT != null ? (float)this.aT.e : this.dM();
   }

   public fat l() {
      return this.aO;
   }

   public boolean m() {
      return !this.aP;
   }

   @Override
   public etc n_() {
      return etc.d;
   }

   @Override
   public boolean s_() {
      return true;
   }

   @Nullable
   public buh.k p() {
      return this.aS;
   }

   private void b(int $$0) {
      this.al.a(q, $$0);
   }

   private int s() {
      return this.al.a(q);
   }

   private void c(int $$0) {
      this.al.a(p, $$0, true);
   }

   private int t() {
      return this.al.a(p);
   }

   private void d(int $$0) {
      this.al.a(r, $$0);
   }

   private int u() {
      return this.al.a(r);
   }

   private void a(buh.a $$0) {
      this.al.a(az, $$0.a());
   }

   private buh.a v() {
      return buh.a.f.apply(this.al.a(az));
   }

   private void a(@Nullable axo $$0) {
      this.al.a(aA, $$0 != null ? $$0.a() : -1);
   }

   @Nullable
   private axo w() {
      int $$0 = this.al.a(aA);
      return $$0 != -1 ? axo.a($$0) : null;
   }

   private int x() {
      return this.al.a(aA);
   }

   private void b(float $$0) {
      this.al.a(aB, $$0);
   }

   private float y() {
      return this.al.a(aB);
   }

   private void c(float $$0) {
      this.al.a(aC, $$0);
   }

   private float z() {
      return this.al.a(aC);
   }

   private void x(float $$0) {
      this.al.a(aD, $$0);
   }

   private float A() {
      return this.al.a(aD);
   }

   private void y(float $$0) {
      this.al.a(aE, $$0);
   }

   private float B() {
      return this.al.a(aE);
   }

   private void z(float $$0) {
      this.al.a(aF, $$0);
   }

   private int C() {
      return this.al.a(aG);
   }

   private void l(int $$0) {
      this.al.a(aG, $$0);
   }

   public float a(float $$0) {
      int $$1 = this.aM;
      if ($$1 <= 0) {
         return 1.0F;
      } else {
         float $$2 = (float)((long)this.af - this.aL);
         float $$3 = $$2 + $$0;
         float $$4 = ayy.a(ayy.f($$3, 0.0F, (float)$$1), 0.0F, 1.0F);
         this.aN = $$4;
         return $$4;
      }
   }

   private float D() {
      return this.al.a(aF);
   }

   @Override
   public void a_(double $$0, double $$1, double $$2) {
      super.a_($$0, $$1, $$2);
      this.E();
   }

   private void E() {
      float $$0 = this.B();
      float $$1 = this.D();
      this.aP = $$0 == 0.0F || $$1 == 0.0F;
      float $$2 = $$0 / 2.0F;
      double $$3 = this.dB();
      double $$4 = this.dD();
      double $$5 = this.dH();
      this.aO = new fat($$3 - (double)$$2, $$4, $$5 - (double)$$2, $$3 + (double)$$2, $$4 + (double)$$1, $$5 + (double)$$2);
   }

   @Override
   public boolean a(double $$0) {
      return $$0 < ayy.k((double)this.y() * 64.0 * cK());
   }

   @Override
   public int k_() {
      int $$0 = this.C();
      return $$0 != -1 ? $$0 : super.k_();
   }

   private buh.k F() {
      return new buh.k(buh.e.constant(a(this.al)), this.v(), this.x(), buh.d.constant(this.z()), buh.d.constant(this.A()), this.C());
   }

   private buh.k a(buh.k $$0, float $$1) {
      j $$2 = $$0.a.get($$1);
      float $$3 = $$0.d.get($$1);
      float $$4 = $$0.e.get($$1);
      return new buh.k(new buh.m($$2, a(this.al)), this.v(), this.x(), new buh.h($$3, this.z()), new buh.h($$4, this.A()), this.C());
   }

   public static enum a implements azu {
      a((byte)0, "fixed"),
      b((byte)1, "vertical"),
      c((byte)2, "horizontal"),
      d((byte)3, "center");

      public static final Codec<buh.a> e = azu.a(buh.a::values);
      public static final IntFunction<buh.a> f = axp.a(buh.a::a, values(), axp.a.a);
      private final byte g;
      private final String h;

      private a(final byte $$0, final String $$1) {
         this.h = $$1;
         this.g = $$0;
      }

      @Override
      public String c() {
         return this.h;
      }

      byte a() {
         return this.g;
      }
   }

   public static class b extends buh {
      public static final String o = "block_state";
      private static final ajx<dwv> p = akb.a(buh.b.class, ajz.i);
      @Nullable
      private buh.b.a q;

      public b(buq<?> $$0, dgg $$1) {
         super($$0, $$1);
      }

      @Override
      protected void a(akb.a $$0) {
         super.a($$0);
         $$0.a(p, djm.a.m());
      }

      @Override
      public void a(ajx<?> $$0) {
         super.a($$0);
         if ($$0.equals(p)) {
            this.n = true;
         }
      }

      private dwv t() {
         return this.al.a(p);
      }

      private void c(dwv $$0) {
         this.al.a(p, $$0);
      }

      @Override
      protected void a(tq $$0) {
         super.a($$0);
         this.c(uf.a(this.dW().a(mc.f), $$0.p("block_state")));
      }

      @Override
      protected void b(tq $$0) {
         super.b($$0);
         $$0.a("block_state", uf.a(this.t()));
      }

      @Nullable
      public buh.b.a s() {
         return this.q;
      }

      @Override
      protected void a(boolean $$0, float $$1) {
         this.q = new buh.b.a(this.t());
      }

      public static record a(dwv a) {
      }
   }

   static record c(int a, int b) implements buh.f {
      @Override
      public int get(float $$0) {
         return axj.a($$0, this.a, this.b);
      }
   }

   @FunctionalInterface
   public interface d {
      static buh.d constant(float $$0) {
         return $$1 -> $$0;
      }

      float get(float var1);
   }

   @FunctionalInterface
   public interface e<T> {
      static <T> buh.e<T> constant(T $$0) {
         return $$1 -> $$0;
      }

      T get(float var1);
   }

   @FunctionalInterface
   public interface f {
      static buh.f constant(int $$0) {
         return $$1 -> $$0;
      }

      int get(float var1);
   }

   public static class g extends buh {
      private static final String o = "item";
      private static final String p = "item_display";
      private static final ajx<cwn> q = akb.a(buh.g.class, ajz.h);
      private static final ajx<Byte> r = akb.a(buh.g.class, ajz.a);
      private final bvx s = bvx.a(this::t, this::a);
      @Nullable
      private buh.g.a t;

      public g(buq<?> $$0, dgg $$1) {
         super($$0, $$1);
      }

      @Override
      protected void a(akb.a $$0) {
         super.a($$0);
         $$0.a(q, cwn.j);
         $$0.a(r, cwl.a.a());
      }

      @Override
      public void a(ajx<?> $$0) {
         super.a($$0);
         if (q.equals($$0) || r.equals($$0)) {
            this.n = true;
         }
      }

      private cwn t() {
         return this.al.a(q);
      }

      private void a(cwn $$0) {
         this.al.a(q, $$0);
      }

      private void a(cwl $$0) {
         this.al.a(r, $$0.a());
      }

      private cwl u() {
         return cwl.k.apply(this.al.a(r));
      }

      @Override
      protected void a(tq $$0) {
         super.a($$0);
         if ($$0.e("item")) {
            this.a(cwn.a(this.dY(), (un)$$0.p("item")).orElse(cwn.j));
         } else {
            this.a(cwn.j);
         }

         if ($$0.b("item_display", 8)) {
            cwl.j.decode(ue.a, $$0.c("item_display")).resultOrPartial(af.a("Display entity", buh.o::error)).ifPresent($$0x -> this.a((cwl)$$0x.getFirst()));
         }
      }

      @Override
      protected void b(tq $$0) {
         super.b($$0);
         if (!this.t().f()) {
            $$0.a("item", this.t().a(this.dY()));
         }

         cwl.j.encodeStart(ue.a, this.u()).ifSuccess($$1 -> $$0.a("item_display", $$1));
      }

      @Override
      public bvx a_(int $$0) {
         return $$0 == 0 ? this.s : bvx.a;
      }

      @Nullable
      public buh.g.a s() {
         return this.t;
      }

      @Override
      protected void a(boolean $$0, float $$1) {
         cwn $$2 = this.t();
         $$2.a(this);
         this.t = new buh.g.a($$2, this.u());
      }

      public static record a(cwn a, cwl b) {
      }
   }

   static record h(float a, float b) implements buh.d {
      @Override
      public float get(float $$0) {
         return ayy.h($$0, this.a, this.b);
      }
   }

   static record i(int a, int b) implements buh.f {
      @Override
      public int get(float $$0) {
         return ayy.a($$0, this.a, this.b);
      }
   }

   static class j {
      int a;
      final double b;
      final double c;
      final double d;
      final double e;
      final double f;

      j(int $$0, double $$1, double $$2, double $$3, double $$4, double $$5) {
         this.a = $$0;
         this.b = $$1;
         this.c = $$2;
         this.d = $$3;
         this.e = $$4;
         this.f = $$5;
      }

      void a(buj $$0) {
         $$0.a_(this.b, this.c, this.d);
         $$0.b((float)this.e, (float)this.f);
      }

      void b(buj $$0) {
         $$0.a(this.a, this.b, this.c, this.d, this.e, this.f);
      }
   }

   public static record k(buh.e<j> a, buh.a b, int c, buh.d d, buh.d e, int f) {
   }

   public static class l extends buh {
      public static final String o = "text";
      private static final String ay = "line_width";
      private static final String az = "text_opacity";
      private static final String aA = "background";
      private static final String aB = "shadow";
      private static final String aC = "see_through";
      private static final String aD = "default_background";
      private static final String aE = "alignment";
      public static final byte p = 1;
      public static final byte q = 2;
      public static final byte r = 4;
      public static final byte s = 8;
      public static final byte t = 16;
      private static final byte aF = -1;
      public static final int u = 1073741824;
      private static final ajx<wo> aG = akb.a(buh.l.class, ajz.f);
      private static final ajx<Integer> aH = akb.a(buh.l.class, ajz.b);
      private static final ajx<Integer> aI = akb.a(buh.l.class, ajz.b);
      private static final ajx<Byte> aJ = akb.a(buh.l.class, ajz.a);
      private static final ajx<Byte> aK = akb.a(buh.l.class, ajz.a);
      private static final IntSet aL = IntSet.of(new int[]{aG.a(), aH.a(), aI.a(), aJ.a(), aK.a()});
      @Nullable
      private buh.l.b aM;
      @Nullable
      private buh.l.e aN;

      public l(buq<?> $$0, dgg $$1) {
         super($$0, $$1);
      }

      @Override
      protected void a(akb.a $$0) {
         super.a($$0);
         $$0.a(aG, wo.i());
         $$0.a(aH, 200);
         $$0.a(aI, 1073741824);
         $$0.a(aJ, (byte)-1);
         $$0.a(aK, (byte)0);
      }

      @Override
      public void a(ajx<?> $$0) {
         super.a($$0);
         if (aL.contains($$0.a())) {
            this.n = true;
         }
      }

      private wo t() {
         return this.al.a(aG);
      }

      private void a(wo $$0) {
         this.al.a(aG, $$0);
      }

      private int u() {
         return this.al.a(aH);
      }

      private void b(int $$0) {
         this.al.a(aH, $$0);
      }

      private byte v() {
         return this.al.a(aJ);
      }

      private void c(byte $$0) {
         this.al.a(aJ, $$0);
      }

      private int w() {
         return this.al.a(aI);
      }

      private void c(int $$0) {
         this.al.a(aI, $$0);
      }

      private byte x() {
         return this.al.a(aK);
      }

      private void d(byte $$0) {
         this.al.a(aK, $$0);
      }

      private static byte a(byte $$0, tq $$1, String $$2, byte $$3) {
         return $$1.q($$2) ? (byte)($$0 | $$3) : $$0;
      }

      @Override
      protected void a(tq $$0) {
         super.a($$0);
         if ($$0.b("line_width", 99)) {
            this.b($$0.h("line_width"));
         }

         if ($$0.b("text_opacity", 99)) {
            this.c($$0.f("text_opacity"));
         }

         if ($$0.b("background", 99)) {
            this.c($$0.h("background"));
         }

         byte $$1 = a((byte)0, $$0, "shadow", (byte)1);
         $$1 = a($$1, $$0, "see_through", (byte)2);
         $$1 = a($$1, $$0, "default_background", (byte)4);
         Optional<buh.l.a> $$2 = buh.l.a.d.decode(ue.a, $$0.c("alignment")).resultOrPartial(af.a("Display entity", buh.o::error)).map(Pair::getFirst);
         if ($$2.isPresent()) {
            $$1 = switch ((buh.l.a)$$2.get()) {
               case a -> $$1;
               case b -> (byte)($$1 | 8);
               case c -> (byte)($$1 | 16);
            };
         }

         this.d($$1);
         if ($$0.b("text", 8)) {
            String $$3 = $$0.l("text");

            try {
               wo $$4 = wo.a.a($$3, this.dY());
               if ($$4 != null && this.dW() instanceof arc $$5) {
                  ex $$6 = this.d($$5).a(2);
                  wo $$7 = wr.a($$6, $$4, this, 0);
                  this.a($$7);
               } else {
                  this.a(wo.i());
               }
            } catch (Exception var9) {
               buh.o.warn("Failed to parse display entity text {}", $$3, var9);
            }
         }
      }

      private static void b(byte $$0, tq $$1, String $$2, byte $$3) {
         $$1.a($$2, ($$0 & $$3) != 0);
      }

      @Override
      protected void b(tq $$0) {
         super.b($$0);
         $$0.a("text", wo.a.a(this.t(), this.dY()));
         $$0.a("line_width", this.u());
         $$0.a("background", this.w());
         $$0.a("text_opacity", this.v());
         byte $$1 = this.x();
         b($$1, $$0, "shadow", (byte)1);
         b($$1, $$0, "see_through", (byte)2);
         b($$1, $$0, "default_background", (byte)4);
         buh.l.a.d.encodeStart(ue.a, a($$1)).ifSuccess($$1x -> $$0.a("alignment", $$1x));
      }

      @Override
      protected void a(boolean $$0, float $$1) {
         if ($$0 && this.aN != null) {
            this.aN = this.a(this.aN, $$1);
         } else {
            this.aN = this.y();
         }

         this.aM = null;
      }

      @Nullable
      public buh.l.e s() {
         return this.aN;
      }

      private buh.l.e y() {
         return new buh.l.e(this.t(), this.u(), buh.f.constant(this.v()), buh.f.constant(this.w()), this.x());
      }

      private buh.l.e a(buh.l.e $$0, float $$1) {
         int $$2 = $$0.d.get($$1);
         int $$3 = $$0.c.get($$1);
         return new buh.l.e(this.t(), this.u(), new buh.i($$3, this.v()), new buh.c($$2, this.w()), this.x());
      }

      public buh.l.b a(buh.l.d $$0) {
         if (this.aM == null) {
            if (this.aN != null) {
               this.aM = $$0.split(this.aN.a(), this.aN.b());
            } else {
               this.aM = new buh.l.b(List.of(), 0);
            }
         }

         return this.aM;
      }

      public static buh.l.a a(byte $$0) {
         if (($$0 & 8) != 0) {
            return buh.l.a.b;
         } else {
            return ($$0 & 16) != 0 ? buh.l.a.c : buh.l.a.a;
         }
      }

      public static enum a implements azu {
         a("center"),
         b("left"),
         c("right");

         public static final Codec<buh.l.a> d = azu.a(buh.l.a::values);
         private final String e;

         private a(final String $$0) {
            this.e = $$0;
         }

         @Override
         public String c() {
            return this.e;
         }
      }

      public static record b(List<buh.l.c> a, int b) {
      }

      public static record c(ayk a, int b) {
      }

      @FunctionalInterface
      public interface d {
         buh.l.b split(wo var1, int var2);
      }

      public static record e(wo a, int b, buh.f c, buh.f d, byte e) {
      }
   }

   static record m(j a, j b) implements buh.e<j> {
      public j a(float $$0) {
         return (double)$$0 >= 1.0 ? this.b : this.a.a(this.b, $$0);
      }
   }
}
