public class ggo implements ggd<dqs> {
   private final ftw a;

   public ggo(gge.a $$0) {
      this.a = new ftw($$0.a(fxn.r));
   }

   public void a(dqs $$0, float $$1, fam $$2, ged $$3, int $$4, int $$5) {
      dsk $$6 = $$0.n();
      if ($$6.c(djs.d)) {
         $$2.a();
         $$2.a(0.5F, 1.0625F, 0.5F);
         float $$7 = $$6.c(djs.b).h().p();
         $$2.a(a.d.rotationDegrees(-$$7));
         $$2.a(a.f.rotationDegrees(67.5F));
         $$2.a(0.0F, -0.125F, 0.0F);
         this.a.a(0.0F, 0.1F, 0.9F, 1.2F);
         faq $$8 = ggm.a.a($$3, gel::c);
         this.a.b($$2, $$8, $$4, $$5, 1.0F, 1.0F, 1.0F, 1.0F);
         $$2.b();
      }
   }
}
