...
public class axp extends DataFix {
   ...
   private static <T> Dynamic<T> a(Dynamic<T> $$0) {
      ...
      Optional<Dynamic<T>> $$1 = $$0.get("HiddenEffect").result().map(axp::a);
      return a($$0, "HiddenEffect", "hidden_effect", $$1);
   }

   private static <T> Dynamic<T> c(Dynamic<T> $$0, String $$1, String $$2) {
      Optional<Dynamic<T>> $$3 = $$0.get($$1).asStreamOpt().result().map($$1x -> $$0.createList($$1x.map(axp::a)));
      return a($$0, $$1, $$2, $$3);
   }
   ...
   private TypeRewriteRule a() {
      Type<?> $$0 = this.getInputSchema().getType(ayp.s);
      return this.fixTypeEverywhereTyped("BlockEntityMobEffectIdFix", $$0, $$0x -> this.a($$0x, ayp.s, "minecraft:beacon", $$0xx -> {
   ...
   private TypeRewriteRule b() {
      Type<?> $$0 = this.getInputSchema().getType(ayp.x);
      return this.fixTypeEverywhereTyped("EntityMobEffectIdFix", $$0, $$0x -> {
         $$0x = this.a($$0x, ayp.x, "minecraft:mooshroom", axp::c);
         $$0x = this.a($$0x, ayp.x, "minecraft:arrow", axp::d);
         $$0x = this.a($$0x, ayp.x, "minecraft:area_effect_cloud", axp::e);
         return $$0x.update(DSL.remainderFinder(), axp::f);
      });
   }

   private static <T> Dynamic<T> g(Dynamic<T> $$0) {
      Optional<Dynamic<T>> $$1 = $$0.get("Effects").asStreamOpt().result().map($$1x -> $$0.createList($$1x.map(axp::b)));
      return a($$0, "Effects", "effects", $$1);
   }

   private TypeRewriteRule c() {
      OpticFinder<Pair<String, String>> $$0 = DSL.fieldFinder("id", DSL.named(ayp.z.typeName(), azx.a()));
      Type<?> $$1 = this.getInputSchema().getType(ayp.t);
      ...
      return this.fixTypeEverywhereTyped("ItemStackMobEffectIdFix", $$1, $$2x -> {
         ...
         if ($$3.isPresent()) {
            ...
            if ($$4.equals("minecraft:suspicious_stew")) {
               return $$2x.updateTyped($$2, $$0xx -> $$0xx.update(DSL.remainderFinder(), axp::g));
            }
   ...
   protected TypeRewriteRule makeRule() {
      return TypeRewriteRule.seq(this.a(), new TypeRewriteRule[]{this.b(), this.c()});
   }
}
