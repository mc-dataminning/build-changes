...
public record ard<T>(T a, Optional<T> b) {
   public static <T> Codec<ard<T>> a(Codec<T> $$0) {
      Codec<ard<T>> $$1 = RecordCodecBuilder.create(
         $$1x -> $$1x.group($$0.fieldOf("raw").forGetter(ard::a), $$0.optionalFieldOf("filtered").forGetter(ard::b)).apply($$1x, ard::new)
      );
      Codec<ard<T>> $$2 = $$0.xmap(ard::a, ard::a);
      return Codec.withAlternative($$1, $$2);
   }

   public static <B extends ByteBuf, T> ys<B, ard<T>> a(ys<B, T> $$0) {
      return ys.a($$0, ard::a, $$0.a(yq::a), ard::b, ard::new);
   }
