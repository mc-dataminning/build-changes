...
public class bdv {
   ...
   private JsonElement a(List<Pair<dgz, beg<bdx>>> $$0) {
      JsonObject $$1 = new JsonObject();
      $$1.addProperty("durationNanosTotal", $$0.stream().mapToDouble($$0x -> (double)((beg)$$0x.getSecond()).f().toNanos()).sum());
      JsonArray $$2 = ac.a(new JsonArray(), $$1x -> $$1.add("status", $$1x));

      for (Pair<dgz, beg<bdx>> $$3 : $$0) {
         beg<bdx> $$4 = (beg<bdx>)$$3.getSecond();
         JsonObject $$5 = ac.a(new JsonObject(), $$2::add);
         $$5.addProperty("state", ((dgz)$$3.getFirst()).toString());
   ...
   private JsonElement b(List<bed> $$0) {
      if ($$0.isEmpty()) {
         ...
         Map<Integer, Double> $$4 = bdl.a($$2);
         $$4.forEach(($$1x, $$2x) -> $$1.addProperty("p" + $$1x, $$2x));
         return $$1;
      }
   }

   private JsonElement b(bdt $$0) {
      JsonObject $$1 = new JsonObject();
   ...
   private JsonElement a(beb $$0) {
      ...
      $$0.e().forEach($$1x -> {
         JsonObject $$2x = new JsonObject();
         $$2.add($$2x);
         beb.b $$3 = (beb.b)$$1x.getFirst();
         beb.a $$4 = (beb.a)$$1x.getSecond();
   ...
   private JsonElement c(List<bdy> $$0) {
      JsonObject $$1 = new JsonObject();
      BiFunction<List<bdy>, ToDoubleFunction<bdy>, JsonObject> $$2 = ($$0x, $$1x) -> {
         JsonObject $$2x = new JsonObject();
      ...
      $$1.add("jvm", (JsonElement)$$2.apply($$0, bdy::a));
      $$1.add("userJvm", (JsonElement)$$2.apply($$0, bdy::b));
      $$1.add("system", (JsonElement)$$2.apply($$0, bdy::c));
      return $$1;
   }
}
