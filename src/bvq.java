import com.google.common.collect.Lists;
import java.util.List;
import java.util.Objects;
import javax.annotation.Nullable;

public class bvq {
   public static final int a = 100;
   public static final int b = 300;
   private static final yd c = yd.a.a(new xe.e(ayt.w)).a(new xm.e(xg.b("MCPE-28723")));
   private final List<bvo> d = Lists.newArrayList();
   private final byf e;
   private int f;
   private int g;
   private int h;
   private boolean i;
   private boolean j;

   public bvq(byf $$0) {
      this.e = $$0;
   }

   public void a(bvt $$0, float $$1) {
      this.c();
      bvy $$2 = bvy.a(this.e);
      bvo $$3 = new bvo($$0, $$1, $$2, (float)this.e.Z);
      this.d.add($$3);
      this.f = this.e.af;
      this.j = true;
      if (!this.i && this.e.bJ() && a($$0)) {
         this.i = true;
         this.g = this.e.af;
         this.h = this.g;
         this.e.f_();
      }
   }

   private static boolean a(bvt $$0) {
      return $$0.d() instanceof byf;
   }

   private xg a(bxe $$0, xg $$1, String $$2, String $$3) {
      dak $$5 = $$0 instanceof byf $$4 ? $$4.fb() : dak.l;
      return !$$5.f() && $$5.c(kl.g) ? xg.a($$2, this.e.P_(), $$1, $$5.K()) : xg.a($$3, this.e.P_(), $$1);
   }

   private xg a(bvo $$0, @Nullable bxe $$1) {
      bvt $$2 = $$0.a();
      if (!$$2.a(axo.m) && !$$2.a(axo.s)) {
         xg $$4 = a($$1);
         bxe $$5 = $$2.d();
         xg $$6 = a($$5);
         if ($$6 != null && !$$6.equals($$4)) {
            return this.a($$5, $$6, "death.fell.assist.item", "death.fell.assist");
         } else {
            return (xg)($$4 != null ? this.a($$1, $$4, "death.fell.finish.item", "death.fell.finish") : xg.a("death.fell.killer", this.e.P_()));
         }
      } else {
         bvy $$3 = Objects.requireNonNullElse($$0.c(), bvy.a);
         return xg.a($$3.a(), this.e.P_());
      }
   }

   @Nullable
   private static xg a(@Nullable bxe $$0) {
      return $$0 == null ? null : $$0.P_();
   }

   public xg a() {
      if (this.d.isEmpty()) {
         return xg.a("death.attack.generic", this.e.P_());
      } else {
         bvo $$0 = this.d.get(this.d.size() - 1);
         bvt $$1 = $$0.a();
         bvo $$2 = this.d();
         bvx $$3 = $$1.k().e();
         if ($$3 == bvx.b && $$2 != null) {
            return this.a($$2, $$1.d());
         } else if ($$3 == bvx.c) {
            String $$4 = "death.attack." + $$1.f();
            xg $$5 = xj.a((xg)xg.c($$4 + ".link")).c(c);
            return xg.a($$4 + ".message", this.e.P_(), $$5);
         } else {
            return $$1.a(this.e);
         }
      }
   }

   @Nullable
   private bvo d() {
      bvo $$0 = null;
      bvo $$1 = null;
      float $$2 = 0.0F;
      float $$3 = 0.0F;

      for (int $$4 = 0; $$4 < this.d.size(); $$4++) {
         bvo $$5 = this.d.get($$4);
         bvo $$6 = $$4 > 0 ? this.d.get($$4 - 1) : null;
         bvt $$7 = $$5.a();
         boolean $$8 = $$7.a(axo.s);
         float $$9 = $$8 ? Float.MAX_VALUE : $$5.d();
         if (($$7.a(axo.m) || $$8) && $$9 > 0.0F && ($$0 == null || $$9 > $$3)) {
            if ($$4 > 0) {
               $$0 = $$6;
            } else {
               $$0 = $$5;
            }

            $$3 = $$9;
         }

         if ($$5.c() != null && ($$1 == null || $$5.b() > $$2)) {
            $$1 = $$5;
            $$2 = $$5.b();
         }
      }

      if ($$3 > 5.0F && $$0 != null) {
         return $$0;
      } else {
         return $$2 > 5.0F && $$1 != null ? $$1 : null;
      }
   }

   public int b() {
      return this.i ? this.e.af - this.g : this.h - this.g;
   }

   public void c() {
      int $$0 = this.i ? 300 : 100;
      if (this.j && (!this.e.bJ() || this.e.af - this.f > $$0)) {
         boolean $$1 = this.i;
         this.j = false;
         this.i = false;
         this.h = this.e.af;
         if ($$1) {
            this.e.O_();
         }

         this.d.clear();
      }
   }
}
