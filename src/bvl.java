import javax.annotation.Nullable;
import org.joml.Vector3f;

public class bvl extends bux {
   public static final double bT = 0.6;
   public static final double bU = 0.8;
   public static final double bW = 1.33;
   private static final cmm bX = cmm.a(cjo.ql, cjo.qm);
   private static final aef<Boolean> bY = aei.a(bvl.class, aeh.k);
   @Nullable
   private bvl.a<cca> bZ;
   @Nullable
   private bvl.b ca;

   public bvl(bja<? extends bvl> $$0, cqb $$1) {
      super($$0, $$1);
      this.t();
   }

   boolean y() {
      return this.an.b(bY);
   }

   private void w(boolean $$0) {
      this.an.b(bY, $$0);
      this.t();
   }

   @Override
   public void b(qw $$0) {
      super.b($$0);
      $$0.a("Trusting", this.y());
   }

   @Override
   public void a(qw $$0) {
      super.a($$0);
      this.w($$0.q("Trusting"));
   }

   @Override
   protected void a_() {
      super.a_();
      this.an.a(bY, false);
   }

   @Override
   protected void z() {
      this.ca = new bvl.b(this, 0.6, bX, true);
      this.bO.a(1, new bqb(this));
      this.bO.a(3, this.ca);
      this.bO.a(7, new bqn(this, 0.3F));
      this.bO.a(8, new bqx(this));
      this.bO.a(9, new bpt(this, 0.8));
      this.bO.a(10, new bru(this, 0.8, 1.0000001E-5F));
      this.bO.a(11, new bqp(this, cca.class, 10.0F));
      this.bP.a(1, new bsa<>(this, bvc.class, false));
      this.bP.a(1, new bsa<>(this, bvy.class, 10, false, false, bvy.bU));
   }

   @Override
   public void X() {
      if (this.I().b()) {
         double $$0 = this.I().c();
         if ($$0 == 0.6) {
            this.b(bjy.f);
            this.g(false);
         } else if ($$0 == 1.33) {
            this.b(bjy.a);
            this.g(true);
         } else {
            this.b(bjy.a);
            this.g(false);
         }
      } else {
         this.b(bjy.a);
         this.g(false);
      }
   }

   @Override
   public boolean h(double $$0) {
      return !this.y() && this.ah > 2400;
   }

   public static bkr.a s() {
      return bjo.A().a(bks.l, 10.0).a(bks.m, 0.3F).a(bks.c, 3.0);
   }

   @Nullable
   @Override
   protected apf w() {
      return apg.qG;
   }

   @Override
   public int O() {
      return 900;
   }

   @Override
   protected apf d(bhu $$0) {
      return apg.qF;
   }

   @Override
   protected apf l_() {
      return apg.qH;
   }

   private float ge() {
      return (float)this.b(bks.c);
   }

   @Override
   public boolean C(biw $$0) {
      return $$0.a(this.dM().b((bjm)this), this.ge());
   }

   @Override
   public bhe b(cca $$0, bhd $$1) {
      cjl $$2 = $$0.b($$1);
      if ((this.ca == null || this.ca.i()) && !this.y() && this.m($$2) && $$0.f(this) < 9.0) {
         this.a($$0, $$1, $$2);
         if (!this.dL().B) {
            if (this.ag.a(3) == 0) {
               this.w(true);
               this.x(true);
               this.dL().a(this, (byte)41);
            } else {
               this.x(false);
               this.dL().a(this, (byte)40);
            }
         }

         return bhe.a(this.dL().B);
      } else {
         return super.b($$0, $$1);
      }
   }

   @Override
   public void b(byte $$0) {
      if ($$0 == 41) {
         this.x(true);
      } else if ($$0 == 40) {
         this.x(false);
      } else {
         super.b($$0);
      }
   }

   private void x(boolean $$0) {
      it $$1 = iv.M;
      if (!$$0) {
         $$1 = iv.Z;
      }

      for (int $$2 = 0; $$2 < 7; $$2++) {
         double $$3 = this.ag.k() * 0.02;
         double $$4 = this.ag.k() * 0.02;
         double $$5 = this.ag.k() * 0.02;
         this.dL().a($$1, this.d(1.0), this.dt() + 0.5, this.g(1.0), $$3, $$4, $$5);
      }
   }

   protected void t() {
      if (this.bZ == null) {
         this.bZ = new bvl.a<>(this, cca.class, 16.0F, 0.8, 1.33);
      }

      this.bO.a(this.bZ);
      if (!this.y()) {
         this.bO.a(4, this.bZ);
      }
   }

   @Nullable
   public bvl b(akt $$0, bir $$1) {
      return bja.aq.a((cqb)$$0);
   }

   @Override
   public boolean m(cjl $$0) {
      return bX.a($$0);
   }

   public static boolean c(bja<bvl> $$0, cqc $$1, bjq $$2, gw $$3, ash $$4) {
      return $$4.a(3) != 0;
   }

   @Override
   public boolean a(cqe $$0) {
      if ($$0.f(this) && !$$0.d(this.cG())) {
         gw $$1 = this.dl();
         if ($$1.v() < $$0.y_()) {
            return false;
         }

         dfd $$2 = $$0.a_($$1.d());
         if ($$2.a(cte.i) || $$2.a(apv.O)) {
            return true;
         }
      }

      return false;
   }

   @Nullable
   @Override
   public bkf a(cqq $$0, bhc $$1, bjq $$2, @Nullable bkf $$3, @Nullable qw $$4) {
      if ($$3 == null) {
         $$3 = new bir.a(1.0F);
      }

      return super.a($$0, $$1, $$2, $$3, $$4);
   }

   @Override
   public ehh cI() {
      return new ehh(0.0, (double)(0.5F * this.cH()), (double)(this.df() * 0.4F));
   }

   @Override
   public boolean bS() {
      return this.bW() || super.bS();
   }

   @Override
   protected Vector3f a(biw $$0, bix $$1, float $$2) {
      return new Vector3f(0.0F, $$1.b - 0.0625F * $$2, 0.0F);
   }

   static class a<T extends bjm> extends bpo<T> {
      private final bvl i;

      public a(bvl $$0, Class<T> $$1, float $$2, double $$3, double $$4) {
         super($$0, $$1, $$2, $$3, $$4, biz.e::test);
         this.i = $$0;
      }

      @Override
      public boolean a() {
         return !this.i.y() && super.a();
      }

      @Override
      public boolean b() {
         return !this.i.y() && super.b();
      }
   }

   static class b extends brp {
      private final bvl c;

      public b(bvl $$0, double $$1, cmm $$2, boolean $$3) {
         super($$0, $$1, $$2, $$3);
         this.c = $$0;
      }

      @Override
      protected boolean h() {
         return super.h() && !this.c.y();
      }
   }
}
