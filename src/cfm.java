import javax.annotation.Nullable;

public abstract class cfm extends cfl implements cfp {
   private il<clj> c = il.a(36, clj.b);
   @Nullable
   private agi d;
   private long e;

   protected cfm(bku<?> $$0, csa $$1) {
      super($$0, $$1);
   }

   protected cfm(bku<?> $$0, double $$1, double $$2, double $$3, csa $$4) {
      super($$0, $$4, $$1, $$2, $$3);
   }

   @Override
   public void a(bjo $$0) {
      super.a($$0);
      this.a($$0, this.dN(), this);
   }

   @Override
   public clj a(int $$0) {
      return this.f_($$0);
   }

   @Override
   public clj a(int $$0, int $$1) {
      return this.b($$0, $$1);
   }

   @Override
   public clj b(int $$0) {
      return this.e_($$0);
   }

   @Override
   public void a(int $$0, clj $$1) {
      this.c($$0, $$1);
   }

   @Override
   public bly a_(int $$0) {
      return this.g_($$0);
   }

   @Override
   public void e() {
   }

   @Override
   public boolean a(cdu $$0) {
      return this.g($$0);
   }

   @Override
   public void a(bkq.c $$0) {
      if (!this.dN().B && $$0.a()) {
         bit.a(this.dN(), this, this);
      }

      super.a($$0);
   }

   @Override
   protected void b(rz $$0) {
      super.b($$0);
      this.c($$0);
   }

   @Override
   protected void a(rz $$0) {
      super.a($$0);
      this.b_($$0);
   }

   @Override
   public bix a(cdu $$0, biw $$1) {
      return this.c_($$0);
   }

   @Override
   protected void u() {
      float $$0 = 0.98F;
      if (this.d == null) {
         int $$1 = 15 - cgm.b(this);
         $$0 += (float)$$1 * 0.001F;
      }

      if (this.aZ()) {
         $$0 *= 0.95F;
      }

      this.g(this.dq().d((double)$$0, 0.0, (double)$$0));
   }

   @Override
   public void a() {
      this.f();
   }

   public void a(agi $$0, long $$1) {
      this.d = $$0;
      this.e = $$1;
   }

   @Nullable
   @Override
   public cgm createMenu(int $$0, cdt $$1, cdu $$2) {
      if (this.d != null && $$2.O_()) {
         return null;
      } else {
         this.f($$1.m);
         return this.a($$0, $$1);
      }
   }

   protected abstract cgm a(int var1, cdt var2);

   @Nullable
   @Override
   public agi C() {
      return this.d;
   }

   @Override
   public void a(@Nullable agi $$0) {
      this.d = $$0;
   }

   @Override
   public long D() {
      return this.e;
   }

   @Override
   public void a(long $$0) {
      this.e = $$0;
   }

   @Override
   public il<clj> E() {
      return this.c;
   }

   @Override
   public void G() {
      this.c = il.a(this.b(), clj.b);
   }
}
