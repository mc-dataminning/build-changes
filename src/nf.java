...
public class nf implements lk {
   private final lm.a d;
   private final lm.a e;

   public nf(lm $$0) {
      this.d = $$0.a(lm.b.b, "blockstates");
      this.e = $$0.a(lm.b.b, "models");
   }

   @Override
   public CompletableFuture<?> a(li $$0) {
      Map<dde, ng> $$1 = Maps.newHashMap();
      Consumer<ng> $$2 = $$1x -> {
         dde $$2x = $$1x.a();
         ng $$3x = $$1.put($$2x, $$1x);
         if ($$3x != null) {
      ...
      Map<akh, Supplier<JsonElement>> $$3 = Maps.newHashMap();
      Set<csu> $$4 = Sets.newHashSet();
      BiConsumer<akh, Supplier<JsonElement>> $$5 = ($$1x, $$2x) -> {
         Supplier<JsonElement> $$3x = $$3.put($$1x, $$2x);
         if ($$3x != null) {
            throw new IllegalStateException("Duplicate model definition for " + $$1x);
         }
      };
      Consumer<csu> $$6 = $$4::add;
      new nd($$2, $$5, $$6).a();
      new ne($$5).a();
      List<dde> $$7 = ld.e.g().stream().filter($$0x -> true).map(Entry::getValue).filter($$1x -> !$$1.containsKey($$1x)).toList();
      if (!$$7.isEmpty()) {
   ...
   private <T> CompletableFuture<?> a(li $$0, Map<T, ? extends Supplier<JsonElement>> $$1, Function<T, Path> $$2) {
