import javax.annotation.Nullable;

public class gxl extends gtf<gkt, gzw, gdg> {
   public gxl(gse.a $$0, boolean $$1) {
      super($$0, new gdg($$0.a($$1 ? gfc.ck : gfc.ce), $$1), 0.5F);
      this.a(new gwi<>(this, new gcn($$0.a($$1 ? gfc.cl : gfc.ci)), new gcn($$0.a($$1 ? gfc.cm : gfc.cj)), $$0.h()));
      this.a(new gws<>(this));
      this.a(new gvo<>(this, $$0));
      this.a(new gvy(this, $$0.f()));
      this.a(new gvs(this, $$0.f(), $$0.g()));
      this.a(new gvx<>(this, $$0.f()));
      this.a(new gxe<>(this, $$0.f(), $$0.h()));
      this.a(new gwq(this, $$0.f()));
      this.a(new gxa(this, $$0.f()));
      this.a(new gvp<>(this, $$0));
   }

   protected boolean a(gzw $$0) {
      return !$$0.m;
   }

   public fba b(gzw $$0) {
      fba $$1 = super.a($$0);
      return $$0.N ? $$1.b(0.0, (double)($$0.af * -2.0F) / 16.0, 0.0) : $$1;
   }

   private static gco.a a(gkt $$0, bvb $$1) {
      cwp $$2 = $$0.b(bsj.a);
      cwp $$3 = $$0.b(bsj.b);
      gco.a $$4 = a($$0, $$2, bsj.a);
      gco.a $$5 = a($$0, $$3, bsj.b);
      if ($$4.a()) {
         $$5 = $$3.f() ? gco.a.a : gco.a.b;
      }

      return $$0.fy() == $$1 ? $$4 : $$5;
   }

   private static gco.a a(cox $$0, cwp $$1, bsj $$2) {
      if ($$1.f()) {
         return gco.a.a;
      } else {
         if ($$0.fA() == $$2 && $$0.fC() > 0) {
            cwr $$3 = $$1.w();
            if ($$3 == cwr.d) {
               return gco.a.c;
            }

            if ($$3 == cwr.e) {
               return gco.a.d;
            }

            if ($$3 == cwr.f) {
               return gco.a.e;
            }

            if ($$3 == cwr.g) {
               return gco.a.f;
            }

            if ($$3 == cwr.h) {
               return gco.a.h;
            }

            if ($$3 == cwr.i) {
               return gco.a.i;
            }

            if ($$3 == cwr.j) {
               return gco.a.j;
            }
         } else if (!$$0.aI && $$1.a(cwt.wV) && cvg.g($$1)) {
            return gco.a.g;
         }

         return gco.a.b;
      }
   }

   public akv c(gzw $$0) {
      return $$0.a.a();
   }

   protected void a(gzw $$0, ffu $$1) {
      float $$2 = 0.9375F;
      $$1.b(0.9375F, 0.9375F, 0.9375F);
   }

   protected void a(gzw $$0, wp $$1, ffu $$2, gly $$3, int $$4) {
      $$2.a();
      if ($$0.aD != null) {
         super.a($$0, $$0.aD, $$2, $$3, $$4);
         $$2.a(0.0F, 9.0F * 1.15F * 0.025F, 0.0F);
      }

      super.a($$0, $$1, $$2, $$3, $$4);
      $$2.b();
   }

   public gzw b() {
      return new gzw();
   }

   public void a(gkt $$0, gzw $$1, float $$2) {
      super.a($$0, $$1, $$2);
      gsv.a($$0, $$1, $$2, this.i);
      $$1.h = a($$0, bvb.a);
      $$1.f = a($$0, bvb.b);
      $$1.a = $$0.d();
      $$1.j = $$0.eU();
      $$1.k = $$0.eV();
      $$1.l = $$0.fC();
      $$1.aC = $$0.aI;
      $$1.m = $$0.Z_();
      $$1.n = $$0.a(coy.g);
      $$1.o = $$0.a(coy.b);
      $$1.p = $$0.a(coy.e);
      $$1.q = $$0.a(coy.f);
      $$1.aw = $$0.a(coy.c);
      $$1.ax = $$0.a(coy.d);
      $$1.ay = $$0.a(coy.a);
      b($$0, $$1, $$2);
      c($$0, $$1, $$2);
      if ($$1.y < 100.0) {
         fcf $$3 = $$0.gy();
         fbx $$4 = $$3.a(fbw.c);
         if ($$4 != null) {
            fcb $$5 = $$3.d($$0, $$4);
            wp $$6 = fcb.a($$5, $$4.a(yi.b));
            $$1.aD = wp.i().b($$6).b(wo.v).b($$4.d());
         } else {
            $$1.aD = null;
         }
      } else {
         $$1.aD = null;
      }

      $$1.aE = a($$0, true);
      $$1.aF = a($$0, false);
      $$1.aG = $$0.ar();
      $$1.aH = $$0.gh().getName();
      $$1.aI.b();
      if ($$1.R) {
         cwp $$7 = $$0.b($$1.M);
         if ($$7.a(cwt.rU)) {
            this.i.a($$1.aI, $$7, cwn.f, false, $$0);
         }
      }
   }

   private static void b(gkt $$0, gzw $$1, float $$2) {
      $$1.az = (float)$$0.fK() + $$2;
      fba $$3 = $$0.g($$2);
      fba $$4 = $$0.I($$2);
      double $$5 = $$4.j();
      double $$6 = $$3.j();
      if ($$5 > 0.0 && $$6 > 0.0) {
         $$1.aA = true;
         double $$7 = Math.min(1.0, ($$4.d * $$3.d + $$4.f * $$3.f) / Math.sqrt($$5 * $$6));
         double $$8 = $$4.d * $$3.f - $$4.f * $$3.d;
         $$1.aB = (float)(Math.signum($$8) * Math.acos($$7));
      } else {
         $$1.aA = false;
         $$1.aB = 0.0F;
      }
   }

   private static void c(gkt $$0, gzw $$1, float $$2) {
      double $$3 = ayz.d((double)$$2, $$0.ck, $$0.cn) - ayz.d((double)$$2, $$0.K, $$0.dA());
      double $$4 = ayz.d((double)$$2, $$0.cl, $$0.co) - ayz.d((double)$$2, $$0.L, $$0.dC());
      double $$5 = ayz.d((double)$$2, $$0.cm, $$0.cp) - ayz.d((double)$$2, $$0.M, $$0.dG());
      float $$6 = ayz.i($$2, $$0.aY, $$0.aX);
      double $$7 = (double)ayz.a($$6 * (float) (Math.PI / 180.0));
      double $$8 = (double)(-ayz.b($$6 * (float) (Math.PI / 180.0)));
      $$1.b = (float)$$4 * 10.0F;
      $$1.b = ayz.a($$1.b, -6.0F, 32.0F);
      $$1.c = (float)($$3 * $$7 + $$5 * $$8) * 100.0F;
      $$1.c = $$1.c * (1.0F - $$1.b());
      $$1.c = ayz.a($$1.c, 0.0F, 150.0F);
      $$1.d = (float)($$3 * $$8 - $$5 * $$7) * 100.0F;
      $$1.d = ayz.a($$1.d, -20.0F, 20.0F);
      float $$9 = ayz.h($$2, $$0.ch, $$0.ci);
      float $$10 = ayz.h($$2, $$0.f, $$0.g);
      $$1.b = $$1.b + ayz.a($$10 * 6.0F) * 32.0F * $$9;
   }

   @Nullable
   private static cho.b a(gkt $$0, boolean $$1) {
      tq $$2 = $$1 ? $$0.gA() : $$0.gB();
      return bus.a($$2.l("id")).filter($$0x -> $$0x == bus.aO).isPresent() ? cho.b.a($$2.h("Variant")) : null;
   }

   public void a(ffu $$0, gly $$1, int $$2, akv $$3, boolean $$4) {
      this.a($$0, $$1, $$2, $$3, this.h.r, $$4);
   }

   public void b(ffu $$0, gly $$1, int $$2, akv $$3, boolean $$4) {
      this.a($$0, $$1, $$2, $$3, this.h.s, $$4);
   }

   private void a(ffu $$0, gly $$1, int $$2, akv $$3, gfd $$4, boolean $$5) {
      gdg $$6 = this.c();
      $$4.c();
      $$4.k = true;
      $$6.a.k = $$5;
      $$6.b.k = $$5;
      $$6.s.g = -0.1F;
      $$6.r.g = 0.1F;
      $$4.a($$0, $$1.getBuffer(gmi.j($$3)), $$2, hei.d);
   }

   protected void a(gzw $$0, ffu $$1, float $$2, float $$3) {
      float $$4 = $$0.G;
      float $$5 = $$0.ab;
      if ($$0.O) {
         super.a($$0, $$1, $$2, $$3);
         float $$6 = $$0.b();
         if (!$$0.al) {
            $$1.a(a.b.rotationDegrees($$6 * (-90.0F - $$5)));
         }

         if ($$0.aA) {
            $$1.a(a.d.rotation($$0.aB));
         }
      } else if ($$4 > 0.0F) {
         super.a($$0, $$1, $$2, $$3);
         float $$7 = $$0.ak ? -90.0F - $$5 : -90.0F;
         float $$8 = ayz.h($$4, 0.0F, $$7);
         $$1.a(a.b.rotationDegrees($$8));
         if ($$0.P) {
            $$1.a(0.0F, -1.0F, 0.3F);
         }
      } else {
         super.a($$0, $$1, $$2, $$3);
      }
   }
}
