import com.google.common.collect.Lists;
import com.google.common.collect.Maps;
import com.google.common.collect.Sets;
import com.mojang.authlib.GameProfile;
import com.mojang.logging.LogUtils;
import com.mojang.serialization.Dynamic;
import java.io.File;
import java.net.SocketAddress;
import java.nio.file.Path;
import java.text.SimpleDateFormat;
import java.time.Instant;
import java.util.EnumSet;
import java.util.List;
import java.util.Map;
import java.util.Optional;
import java.util.Set;
import java.util.UUID;
import java.util.function.Function;
import java.util.function.Predicate;
import javax.annotation.Nullable;
import net.minecraft.server.MinecraftServer;
import org.slf4j.Logger;

public abstract class avr {
   public static final File a = new File("banned-players.json");
   public static final File b = new File("banned-ips.json");
   public static final File c = new File("ops.json");
   public static final File d = new File("whitelist.json");
   public static final xl e = xl.c("chat.filtered_full");
   public static final xl f = xl.c("multiplayer.disconnect.duplicate_login");
   private static final Logger h = LogUtils.getLogger();
   private static final int i = 600;
   private static final SimpleDateFormat j = new SimpleDateFormat("yyyy-MM-dd 'at' HH:mm:ss z");
   private final MinecraftServer k;
   private final List<arr> l = Lists.newArrayList();
   private final Map<UUID, arr> m = Maps.newHashMap();
   private final avx n = new avx(a);
   private final avo o = new avo(b);
   private final avs p = new avs(c);
   private final avz q = new avz(d);
   private final Map<UUID, awu> r = Maps.newHashMap();
   private final Map<UUID, alt> s = Maps.newHashMap();
   private final etx t;
   private boolean u;
   private final jx<alu> v;
   protected final int g;
   private int w;
   private int x;
   private boolean y;
   private static final boolean z = false;
   private int A;

   public avr(MinecraftServer $$0, jx<alu> $$1, etx $$2, int $$3) {
      this.k = $$0;
      this.v = $$1;
      this.g = $$3;
      this.t = $$2;
   }

   public void a(wf $$0, arr $$1, asg $$2) {
      GameProfile $$3 = $$1.gk();
      avn $$4 = this.k.at();
      String $$6;
      if ($$4 != null) {
         Optional<GameProfile> $$5 = $$4.a($$3.getId());
         $$6 = $$5.<String>map(GameProfile::getName).orElse($$3.getName());
         $$4.a($$3);
      } else {
         $$6 = $$3.getName();
      }

      Optional<un> $$8 = this.a($$1);
      alk<dff> $$9 = $$8.<alk<dff>>flatMap($$0x -> dzj.a(new Dynamic(vb.a, $$0x.c("Dimension"))).resultOrPartial(h::error)).orElse(dff.i);
      arq $$10 = this.k.a($$9);
      arq $$11;
      if ($$10 == null) {
         h.warn("Unknown respawn dimension {}, defaulting to overworld", $$9);
         $$11 = this.k.J();
      } else {
         $$11 = $$10;
      }

      $$1.a($$11);
      String $$13 = $$0.a(this.k.bl());
      h.info("{}[{}] logged in with entity id {} at ({}, {}, {})", new Object[]{$$1.am().getString(), $$13, $$1.as(), $$1.dD(), $$1.dF(), $$1.dJ()});
      etq $$14 = $$11.A_();
      $$1.c($$8.orElse(null));
      ast $$15 = new ast(this.k, $$0, $$1, $$2);
      $$0.a(agx.a.a(ww.a(this.k.ba())), $$15);
      dfb $$16 = $$11.ac();
      boolean $$17 = $$16.b(dfb.D);
      boolean $$18 = $$16.b(dfb.q);
      boolean $$19 = $$16.b(dfb.w);
      $$15.b(new ady($$1.as(), $$14.l(), this.k.K(), this.n(), this.w, this.x, $$18, !$$17, $$19, $$1.b($$11), this.k.aA()));
      $$15.b(new acu($$14.q(), $$14.r()));
      $$15.b(new aei($$1.gm()));
      $$15.b(new aft($$1.gl().j));
      $$15.b(new ags(this.k.aI().e()));
      this.e($$1);
      $$1.J().c();
      $$1.K().a($$1);
      this.a($$11.f(), $$1);
      this.k.av();
      xz $$20;
      if ($$1.gk().getName().equalsIgnoreCase($$6)) {
         $$20 = xl.a("multiplayer.player.joined", $$1.S_());
      } else {
         $$20 = xl.a("multiplayer.player.joined.renamed", $$1.S_(), $$6);
      }

      this.a($$20.a(n.o), false);
      $$15.a($$1.dD(), $$1.dF(), $$1.dJ(), $$1.dO(), $$1.dQ());
      aki $$22 = this.k.au();
      if ($$22 != null && !$$2.d()) {
         $$1.a($$22);
      }

      $$1.g.b(aeo.a(this.l));
      this.l.add($$1);
      this.m.put($$1.cI(), $$1);
      this.a(aeo.a(List.of($$1)));
      this.a($$1, $$11);
      $$11.a($$1);
      this.k.aM().a($$1);
      this.d($$1);
      $$1.b($$8);
      $$1.a($$8);
      $$1.h();
   }

   protected void a(ame $$0, arr $$1) {
      Set<fao> $$2 = Sets.newHashSet();

      for (far $$3 : $$0.g()) {
         $$1.g.b(afx.a($$3, true));
      }

      for (fan $$4 : fan.values()) {
         fao $$5 = $$0.a($$4);
         if ($$5 != null && !$$2.contains($$5)) {
            for (zs<?> $$7 : $$0.d($$5)) {
               $$1.g.b($$7);
            }

            $$2.add($$5);
         }
      }
   }

   public void a(arq $$0) {
      $$0.C_().a(new dxc() {
         @Override
         public void a(dxe $$0, double $$1) {
            avr.this.a(new afe($$0));
         }

         @Override
         public void a(dxe $$0, double $$1, double $$2, long $$3) {
            avr.this.a(new afd($$0));
         }

         @Override
         public void a(dxe $$0, double $$1, double $$2) {
            avr.this.a(new afc($$0));
         }

         @Override
         public void a(dxe $$0, int $$1) {
            avr.this.a(new aff($$0));
         }

         @Override
         public void b(dxe $$0, int $$1) {
            avr.this.a(new afg($$0));
         }

         @Override
         public void b(dxe $$0, double $$1) {
         }

         @Override
         public void c(dxe $$0, double $$1) {
         }
      });
   }

   public Optional<un> a(arr $$0) {
      un $$1 = this.k.aZ().w();
      Optional<un> $$2;
      if (this.k.a($$0.gk()) && $$1 != null) {
         $$2 = Optional.of($$1);
         $$0.g($$1);
         h.debug("loading single player");
      } else {
         $$2 = this.t.b($$0);
      }

      return $$2;
   }

   protected void b(arr $$0) {
      this.t.a($$0);
      awu $$1 = this.r.get($$0.cI());
      if ($$1 != null) {
         $$1.a();
      }

      alt $$2 = this.s.get($$0.cI());
      if ($$2 != null) {
         $$2.b();
      }
   }

   public void c(arr $$0) {
      arq $$1 = $$0.B();
      $$0.a(awy.j);
      this.b($$0);
      if ($$0.cb()) {
         bui $$2 = $$0.di();
         if ($$2.dh()) {
            h.debug("Removing player mount");
            $$0.af();
            $$2.de().forEach($$0x -> $$0x.b(bui.c.d));
         }
      }

      $$0.ap();

      for (cpu $$3 : $$0.al()) {
         $$3.b(bui.c.d);
      }

      $$1.a($$0, bui.c.d);
      $$0.T().a();
      this.l.remove($$0);
      this.k.aM().b($$0);
      UUID $$4 = $$0.cI();
      arr $$5 = this.m.get($$4);
      if ($$5 == $$0) {
         this.m.remove($$4);
         this.r.remove($$4);
         this.s.remove($$4);
      }

      this.a(new aen(List.of($$0.cI())));
   }

   @Nullable
   public xl a(SocketAddress $$0, GameProfile $$1) {
      if (this.n.a($$1)) {
         avy $$2 = this.n.b($$1);
         xz $$3 = xl.a("multiplayer.disconnect.banned.reason", $$2.d());
         if ($$2.c() != null) {
            $$3.b(xl.a("multiplayer.disconnect.banned.expiration", j.format($$2.c())));
         }

         return $$3;
      } else if (!this.c($$1)) {
         return xl.c("multiplayer.disconnect.not_whitelisted");
      } else if (this.o.a($$0)) {
         avp $$4 = this.o.b($$0);
         xz $$5 = xl.a("multiplayer.disconnect.banned_ip.reason", $$4.d());
         if ($$4.c() != null) {
            $$5.b(xl.a("multiplayer.disconnect.banned_ip.expiration", j.format($$4.c())));
         }

         return $$5;
      } else {
         return this.l.size() >= this.g && !this.d($$1) ? xl.c("multiplayer.disconnect.server_full") : null;
      }
   }

   public arr a(GameProfile $$0, arc $$1) {
      return new arr(this.k, this.k.J(), $$0, $$1);
   }

   public boolean e(GameProfile $$0) {
      UUID $$1 = $$0.getId();
      Set<arr> $$2 = Sets.newIdentityHashSet();

      for (arr $$3 : this.l) {
         if ($$3.cI().equals($$1)) {
            $$2.add($$3);
         }
      }

      arr $$4 = this.m.get($$0.getId());
      if ($$4 != null) {
         $$2.add($$4);
      }

      for (arr $$5 : $$2) {
         $$5.g.a(f);
      }

      return !$$2.isEmpty();
   }

   public arr a(arr $$0, boolean $$1, bui.c $$2) {
      this.l.remove($$0);
      $$0.B().a($$0, $$2);
      esm $$3 = $$0.a(!$$1, esm.a);
      arq $$4 = $$3.a();
      arr $$5 = new arr(this.k, $$4, $$0.gk(), $$0.D());
      $$5.g = $$0.g;
      $$5.a($$0, $$1);
      $$5.e($$0.as());
      $$5.b($$0.fB());
      if (!$$3.f()) {
         $$5.b($$0);
      }

      for (String $$6 : $$0.at()) {
         $$5.a($$6);
      }

      ezr $$7 = $$3.b();
      $$5.b($$7.d, $$7.e, $$7.f, $$3.d(), $$3.e());
      if ($$3.f()) {
         $$5.g.b(new ado(ado.b, 0.0F));
      }

      byte $$8 = (byte)($$1 ? 1 : 0);
      arq $$9 = $$5.B();
      etq $$10 = $$9.A_();
      $$5.g.b(new aew($$5.b($$9), $$8));
      $$5.g.a($$5.dD(), $$5.dF(), $$5.dJ(), $$5.dO(), $$5.dQ());
      $$5.g.b(new afl($$4.W(), $$4.X()));
      $$5.g.b(new acu($$10.q(), $$10.r()));
      $$5.g.b(new afr($$5.cs, $$5.cr, $$5.cq));
      this.d($$5);
      this.a($$5, $$4);
      this.e($$5);
      $$4.b($$5);
      this.l.add($$5);
      this.m.put($$5.cI(), $$5);
      $$5.h();
      $$5.x($$5.eH());
      jh $$11 = $$5.U();
      arq $$12 = this.k.a($$5.W());
      if (!$$1 && $$11 != null && $$12 != null) {
         dvo $$13 = $$12.a_($$11);
         if ($$13.a(dil.pl)) {
            $$5.g.b(new agf(awo.vx, awp.e, (double)$$11.u(), (double)$$11.v(), (double)$$11.w(), 1.0F, 1.0F, $$4.E_().g()));
         }
      }

      return $$5;
   }

   public void d(arr $$0) {
      this.a($$0, $$0.g);
   }

   public void a(bve $$0, ast $$1) {
      for (btn $$2 : $$0.eE()) {
         $$1.b(new agr($$0.as(), $$2, false));
      }
   }

   public void e(arr $$0) {
      GameProfile $$1 = $$0.gk();
      int $$2 = this.k.c($$1);
      this.a($$0, $$2);
   }

   public void d() {
      if (++this.A > 600) {
         this.a(new aeo(EnumSet.of(aeo.a.e), this.l));
         this.A = 0;
      }
   }

   public void a(zs<?> $$0) {
      for (arr $$1 : this.l) {
         $$1.g.b($$0);
      }
   }

   public void a(zs<?> $$0, alk<dff> $$1) {
      for (arr $$2 : this.l) {
         if ($$2.dY().ag() == $$1) {
            $$2.g.b($$0);
         }
      }
   }

   public void a(cor $$0, xl $$1) {
      fay $$2 = $$0.ct();
      if ($$2 != null) {
         for (String $$4 : $$2.g()) {
            arr $$5 = this.a($$4);
            if ($$5 != null && $$5 != $$0) {
               $$5.a($$1);
            }
         }
      }
   }

   public void b(cor $$0, xl $$1) {
      fay $$2 = $$0.ct();
      if ($$2 == null) {
         this.a($$1, false);
      } else {
         for (int $$3 = 0; $$3 < this.l.size(); $$3++) {
            arr $$4 = this.l.get($$3);
            if ($$4.ct() != $$2) {
               $$4.a($$1);
            }
         }
      }
   }

   public String[] e() {
      String[] $$0 = new String[this.l.size()];

      for (int $$1 = 0; $$1 < this.l.size(); $$1++) {
         $$0[$$1] = this.l.get($$1).gk().getName();
      }

      return $$0;
   }

   public avx f() {
      return this.n;
   }

   public avo g() {
      return this.o;
   }

   public void a(GameProfile $$0) {
      this.p.a(new avt($$0, this.k.k(), this.p.a($$0)));
      arr $$1 = this.a($$0.getId());
      if ($$1 != null) {
         this.e($$1);
      }
   }

   public void b(GameProfile $$0) {
      this.p.c($$0);
      arr $$1 = this.a($$0.getId());
      if ($$1 != null) {
         this.e($$1);
      }
   }

   private void a(arr $$0, int $$1) {
      if ($$0.g != null) {
         byte $$2;
         if ($$1 <= 0) {
            $$2 = 24;
         } else if ($$1 >= 4) {
            $$2 = 28;
         } else {
            $$2 = (byte)(24 + $$1);
         }

         $$0.g.b(new adl($$0, $$2));
      }

      this.k.aG().a($$0);
   }

   public boolean c(GameProfile $$0) {
      return !this.u || this.p.d($$0) || this.q.d($$0);
   }

   public boolean f(GameProfile $$0) {
      return this.p.d($$0) || this.k.a($$0) && this.k.aZ().m() || this.y;
   }

   @Nullable
   public arr a(String $$0) {
      int $$1 = this.l.size();

      for (int $$2 = 0; $$2 < $$1; $$2++) {
         arr $$3 = this.l.get($$2);
         if ($$3.gk().getName().equalsIgnoreCase($$0)) {
            return $$3;
         }
      }

      return null;
   }

   public void a(@Nullable cor $$0, double $$1, double $$2, double $$3, double $$4, alk<dff> $$5, zs<?> $$6) {
      for (int $$7 = 0; $$7 < this.l.size(); $$7++) {
         arr $$8 = this.l.get($$7);
         if ($$8 != $$0 && $$8.dY().ag() == $$5) {
            double $$9 = $$1 - $$8.dD();
            double $$10 = $$2 - $$8.dF();
            double $$11 = $$3 - $$8.dJ();
            if ($$9 * $$9 + $$10 * $$10 + $$11 * $$11 < $$4 * $$4) {
               $$8.g.b($$6);
            }
         }
      }
   }

   public void h() {
      for (int $$0 = 0; $$0 < this.l.size(); $$0++) {
         this.b(this.l.get($$0));
      }
   }

   public avz i() {
      return this.q;
   }

   public String[] j() {
      return this.q.a();
   }

   public avs k() {
      return this.p;
   }

   public String[] l() {
      return this.p.a();
   }

   public void a() {
   }

   public void a(arr $$0, arq $$1) {
      dxe $$2 = this.k.J().C_();
      $$0.g.b(new adr($$2));
      $$0.g.b(new agb($$1.aa(), $$1.ab(), $$1.ac().b(dfb.l)));
      $$0.g.b(new afl($$1.W(), $$1.X()));
      if ($$1.ae()) {
         $$0.g.b(new ado(ado.c, 0.0F));
         $$0.g.b(new ado(ado.i, $$1.d(1.0F)));
         $$0.g.b(new ado(ado.j, $$1.b(1.0F)));
      }

      $$0.g.b(new ado(ado.o, 0.0F));
      this.k.aP().a($$0);
   }

   public void f(arr $$0) {
      $$0.cc.b();
      $$0.y();
      $$0.g.b(new aft($$0.gl().j));
   }

   public int m() {
      return this.l.size();
   }

   public int n() {
      return this.g;
   }

   public boolean o() {
      return this.u;
   }

   public void a(boolean $$0) {
      this.u = $$0;
   }

   public List<arr> b(String $$0) {
      List<arr> $$1 = Lists.newArrayList();

      for (arr $$2 : this.l) {
         if ($$2.C().equals($$0)) {
            $$1.add($$2);
         }
      }

      return $$1;
   }

   public int p() {
      return this.w;
   }

   public int q() {
      return this.x;
   }

   public MinecraftServer c() {
      return this.k;
   }

   @Nullable
   public un r() {
      return null;
   }

   public void b(boolean $$0) {
      this.y = $$0;
   }

   public void s() {
      for (int $$0 = 0; $$0 < this.l.size(); $$0++) {
         this.l.get($$0).g.a(xl.c("multiplayer.disconnect.server_shutdown"));
      }
   }

   public void a(xl $$0, boolean $$1) {
      this.a($$0, $$1x -> $$0, $$1);
   }

   public void a(xl $$0, Function<arr, xl> $$1, boolean $$2) {
      this.k.a($$0);

      for (arr $$3 : this.l) {
         xl $$4 = $$1.apply($$3);
         if ($$4 != null) {
            $$3.b($$4, $$2);
         }
      }
   }

   public void a(yb $$0, ew $$1, xh.a $$2) {
      this.a($$0, $$1::a, $$1.i(), $$2);
   }

   public void a(yb $$0, arr $$1, xh.a $$2) {
      this.a($$0, $$1::c, $$1, $$2);
   }

   private void a(yb $$0, Predicate<arr> $$1, @Nullable arr $$2, xh.a $$3) {
      boolean $$4 = this.a($$0);
      this.k.a($$0.d(), $$3, $$4 ? null : "Not Secure");
      ya $$5 = ya.a($$0);
      boolean $$6 = false;

      for (arr $$7 : this.l) {
         boolean $$8 = $$1.test($$7);
         $$7.a($$5, $$8, $$3);
         $$6 |= $$8 && $$0.j();
      }

      if ($$6 && $$2 != null) {
         $$2.a(e);
      }
   }

   private boolean a(yb $$0) {
      return $$0.i() && !$$0.a(Instant.now());
   }

   public awu a(cor $$0) {
      UUID $$1 = $$0.cI();
      awu $$2 = this.r.get($$1);
      if ($$2 == null) {
         File $$3 = this.k.a(ets.b).toFile();
         File $$4 = new File($$3, $$1 + ".json");
         if (!$$4.exists()) {
            File $$5 = new File($$3, $$0.am().getString() + ".json");
            Path $$6 = $$5.toPath();
            if (v.a($$6) && v.b($$6) && $$6.startsWith($$3.getPath()) && $$5.isFile()) {
               $$5.renameTo($$4);
            }
         }

         $$2 = new awu(this.k, $$4);
         this.r.put($$1, $$2);
      }

      return $$2;
   }

   public alt g(arr $$0) {
      UUID $$1 = $$0.cI();
      alt $$2 = this.s.get($$1);
      if ($$2 == null) {
         Path $$3 = this.k.a(ets.a).resolve($$1 + ".json");
         $$2 = new alt(this.k.aC(), this, this.k.aD(), $$3, $$0);
         this.s.put($$1, $$2);
      }

      $$2.a($$0);
      return $$2;
   }

   public void a(int $$0) {
      this.w = $$0;
      this.a(new afj($$0));

      for (arq $$1 : this.k.L()) {
         if ($$1 != null) {
            $$1.l().a($$0);
         }
      }
   }

   public void b(int $$0) {
      this.x = $$0;
      this.a(new afz($$0));

      for (arq $$1 : this.k.L()) {
         if ($$1 != null) {
            $$1.l().b($$0);
         }
      }
   }

   public List<arr> t() {
      return this.l;
   }

   @Nullable
   public arr a(UUID $$0) {
      return this.m.get($$0);
   }

   public boolean d(GameProfile $$0) {
      return false;
   }

   public void u() {
      for (alt $$0 : this.s.values()) {
         $$0.a(this.k.aD());
      }

      this.a(new aaj(axv.a(this.v)));
      ags $$1 = new ags(this.k.aI().e());

      for (arr $$2 : this.l) {
         $$2.g.b($$1);
         $$2.K().a($$2);
      }
   }

   public boolean v() {
      return this.y;
   }
}
