import com.mojang.serialization.Codec;
import com.mojang.serialization.codecs.RecordCodecBuilder;
import java.util.stream.Stream;

public class crb extends cqx {
   public static final Codec<crb> b = RecordCodecBuilder.create(
      $$0 -> $$0.group(cqt.d.fieldOf("biomes").forGetter($$0x -> $$0x.c), Codec.intRange(0, 62).fieldOf("scale").orElse(2).forGetter($$0x -> $$0x.e))
            .apply($$0, crb::new)
   );
   private final hk<cqt> c;
   private final int d;
   private final int e;

   public crb(hk<cqt> $$0, int $$1) {
      this.c = $$0;
      this.d = $$1 + 2;
      this.e = $$1;
   }

   @Override
   protected Stream<hg<cqt>> b() {
      return this.c.a();
   }

   @Override
   protected Codec<? extends cqx> a() {
      return b;
   }

   @Override
   public hg<cqt> getNoiseBiome(int $$0, int $$1, int $$2, crc.f $$3) {
      return this.c.a(Math.floorMod(($$0 >> this.d) + ($$2 >> this.d), this.c.b()));
   }
}
