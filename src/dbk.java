import com.mojang.serialization.Codec;
import com.mojang.serialization.codecs.RecordCodecBuilder;
import java.util.stream.Stream;

public class dbk extends dbg {
   public static final Codec<dbk> b = RecordCodecBuilder.create(
      $$0 -> $$0.group(dbc.d.fieldOf("biomes").forGetter($$0x -> $$0x.c), Codec.intRange(0, 62).fieldOf("scale").orElse(2).forGetter($$0x -> $$0x.e))
            .apply($$0, dbk::new)
   );
   private final ja<dbc> c;
   private final int d;
   private final int e;

   public dbk(ja<dbc> $$0, int $$1) {
      this.c = $$0;
      this.d = $$1 + 2;
      this.e = $$1;
   }

   @Override
   protected Stream<iw<dbc>> b() {
      return this.c.a();
   }

   @Override
   protected Codec<? extends dbg> a() {
      return b;
   }

   @Override
   public iw<dbc> getNoiseBiome(int $$0, int $$1, int $$2, dbl.f $$3) {
      return this.c.a(Math.floorMod(($$0 >> this.d) + ($$2 >> this.d), this.c.b()));
   }
}
