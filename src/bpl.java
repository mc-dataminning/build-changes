public abstract class bpl extends bpu {
   protected bja d;
   protected gu e = gu.b;
   protected boolean f;
   private boolean a;
   private float b;
   private float c;

   public bpl(bja $$0) {
      this.d = $$0;
      if (!btq.a($$0)) {
         throw new IllegalArgumentException("Unsupported mob type for DoorInteractGoal");
      }
   }

   protected boolean h() {
      if (!this.f) {
         return false;
      } else {
         dfa $$0 = this.d.dK().a_(this.e);
         if (!($$0.b() instanceof cuo)) {
            this.f = false;
            return false;
         } else {
            return $$0.c(cuo.b);
         }
      }
   }

   protected void a(boolean $$0) {
      if (this.f) {
         dfa $$1 = this.d.dK().a_(this.e);
         if ($$1.b() instanceof cuo) {
            ((cuo)$$1.b()).a(this.d, this.d.dK(), $$1, this.e, $$0);
         }
      }
   }

   @Override
   public boolean a() {
      if (!btq.a(this.d)) {
         return false;
      } else if (!this.d.P) {
         return false;
      } else {
         bsh $$0 = (bsh)this.d.H();
         eas $$1 = $$0.j();
         if ($$1 != null && !$$1.c() && $$0.f()) {
            for (int $$2 = 0; $$2 < Math.min($$1.f() + 2, $$1.e()); $$2++) {
               eaq $$3 = $$1.a($$2);
               this.e = new gu($$3.a, $$3.b + 1, $$3.c);
               if (!(this.d.i((double)this.e.u(), this.d.dr(), (double)this.e.w()) > 2.25)) {
                  this.f = cuo.a(this.d.dK(), this.e);
                  if (this.f) {
                     return true;
                  }
               }
            }

            this.e = this.d.dk().c();
            this.f = cuo.a(this.d.dK(), this.e);
            return this.f;
         } else {
            return false;
         }
      }
   }

   @Override
   public boolean b() {
      return !this.a;
   }

   @Override
   public void c() {
      this.a = false;
      this.b = (float)((double)this.e.u() + 0.5 - this.d.dp());
      this.c = (float)((double)this.e.w() + 0.5 - this.d.dv());
   }

   @Override
   public boolean K_() {
      return true;
   }

   @Override
   public void e() {
      float $$0 = (float)((double)this.e.u() + 0.5 - this.d.dp());
      float $$1 = (float)((double)this.e.w() + 0.5 - this.d.dv());
      float $$2 = this.b * $$0 + this.c * $$1;
      if ($$2 < 0.0F) {
         this.a = true;
      }
   }
}
