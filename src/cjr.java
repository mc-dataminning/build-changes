import java.util.List;

public class cjr extends cjy {
   public static final float b = bsz.D.l() / bsz.Y.l();
   private static final int e = 1200;
   private static final int bY = 50;
   private static final int bZ = 6000;
   private static final int ca = 2;
   private static final int cb = 1200;

   public cjr(bsz<? extends cjr> $$0, dbx $$1) {
      super($$0, $$1);
      this.fU();
      if (this.d != null) {
         this.d.c(400);
      }
   }

   public static buv.a s() {
      return cjy.gq().a(buw.r, 0.3F).a(buw.c, 8.0).a(buw.q, 80.0);
   }

   @Override
   public int u() {
      return 60;
   }

   @Override
   protected avy v() {
      return this.bh() ? avz.hO : avz.hP;
   }

   @Override
   protected avy d(brm $$0) {
      return this.bh() ? avz.hU : avz.hV;
   }

   @Override
   protected avy o_() {
      return this.bh() ? avz.hR : avz.hS;
   }

   @Override
   protected avy y() {
      return avz.hT;
   }

   @Override
   protected void Z() {
      super.Z();
      if ((this.ai + this.al()) % 1200 == 0) {
         bsb $$0 = new bsb(bsd.d, 6000, 2);
         List<arf> $$1 = bsc.a((are)this.dP(), this, this.dn(), 50.0, $$0, 1200);
         $$1.forEach($$0x -> $$0x.c.b(new ado(ado.l, this.aW() ? 0.0F : 1.0F)));
      }

      if (!this.gb()) {
         this.a(this.dp(), 16);
      }
   }
}
