import com.google.common.collect.Lists;
import com.mojang.brigadier.ResultConsumer;
import com.mojang.brigadier.context.CommandContext;
import com.mojang.brigadier.exceptions.CommandSyntaxException;
import com.mojang.brigadier.exceptions.SimpleCommandExceptionType;
import com.mojang.brigadier.suggestion.Suggestions;
import com.mojang.brigadier.suggestion.SuggestionsBuilder;
import java.util.Collection;
import java.util.Objects;
import java.util.Set;
import java.util.concurrent.CompletableFuture;
import java.util.function.BinaryOperator;
import java.util.function.IntConsumer;
import java.util.function.Supplier;
import java.util.stream.Stream;
import javax.annotation.Nullable;
import net.minecraft.server.MinecraftServer;

public class dr implements du {
   public static final SimpleCommandExceptionType a = new SimpleCommandExceptionType(tf.c("permissions.requires.player"));
   public static final SimpleCommandExceptionType b = new SimpleCommandExceptionType(tf.c("permissions.requires.entity"));
   private final dq c;
   private final ehe d;
   private final akk e;
   private final int f;
   private final String g;
   private final tf h;
   private final MinecraftServer i;
   private final boolean j;
   @Nullable
   private final bii k;
   @Nullable
   private final ResultConsumer<dr> l;
   private final eb.a m;
   private final ehd n;
   private final dp o;
   private final asj p;
   private final IntConsumer q;

   public dr(dq $$0, ehe $$1, ehd $$2, akk $$3, int $$4, String $$5, tf $$6, MinecraftServer $$7, @Nullable bii $$8) {
      this($$0, $$1, $$2, $$3, $$4, $$5, $$6, $$7, $$8, false, ($$0x, $$1x, $$2x) -> {
      }, eb.a.a, dp.a, asj.immediate($$7), $$0x -> {
      });
   }

   protected dr(
      dq $$0,
      ehe $$1,
      ehd $$2,
      akk $$3,
      int $$4,
      String $$5,
      tf $$6,
      MinecraftServer $$7,
      @Nullable bii $$8,
      boolean $$9,
      @Nullable ResultConsumer<dr> $$10,
      eb.a $$11,
      dp $$12,
      asj $$13,
      IntConsumer $$14
   ) {
      this.c = $$0;
      this.d = $$1;
      this.e = $$3;
      this.j = $$9;
      this.k = $$8;
      this.f = $$4;
      this.g = $$5;
      this.h = $$6;
      this.i = $$7;
      this.l = $$10;
      this.m = $$11;
      this.n = $$2;
      this.o = $$12;
      this.p = $$13;
      this.q = $$14;
   }

   public dr a(dq $$0) {
      return this.c == $$0 ? this : new dr($$0, this.d, this.n, this.e, this.f, this.g, this.h, this.i, this.k, this.j, this.l, this.m, this.o, this.p, this.q);
   }

   public dr a(bii $$0) {
      return this.k == $$0
         ? this
         : new dr(this.c, this.d, this.n, this.e, this.f, $$0.ab().getString(), $$0.H_(), this.i, $$0, this.j, this.l, this.m, this.o, this.p, this.q);
   }

   public dr a(ehe $$0) {
      return this.d.equals($$0)
         ? this
         : new dr(this.c, $$0, this.n, this.e, this.f, this.g, this.h, this.i, this.k, this.j, this.l, this.m, this.o, this.p, this.q);
   }

   public dr a(ehd $$0) {
      return this.n.c($$0) ? this : new dr(this.c, this.d, $$0, this.e, this.f, this.g, this.h, this.i, this.k, this.j, this.l, this.m, this.o, this.p, this.q);
   }

   public dr a(ResultConsumer<dr> $$0) {
      return Objects.equals(this.l, $$0)
         ? this
         : new dr(this.c, this.d, this.n, this.e, this.f, this.g, this.h, this.i, this.k, this.j, $$0, this.m, this.o, this.p, this.q);
   }

   public dr a(ResultConsumer<dr> $$0, BinaryOperator<ResultConsumer<dr>> $$1) {
      ResultConsumer<dr> $$2 = $$1.apply(this.l, $$0);
      return this.a($$2);
   }

   public dr a() {
      return !this.j && !this.c.g_()
         ? new dr(this.c, this.d, this.n, this.e, this.f, this.g, this.h, this.i, this.k, true, this.l, this.m, this.o, this.p, this.q)
         : this;
   }

   public dr a(int $$0) {
      return $$0 == this.f ? this : new dr(this.c, this.d, this.n, this.e, $$0, this.g, this.h, this.i, this.k, this.j, this.l, this.m, this.o, this.p, this.q);
   }

   public dr b(int $$0) {
      return $$0 <= this.f ? this : new dr(this.c, this.d, this.n, this.e, $$0, this.g, this.h, this.i, this.k, this.j, this.l, this.m, this.o, this.p, this.q);
   }

   public dr a(eb.a $$0) {
      return $$0 == this.m ? this : new dr(this.c, this.d, this.n, this.e, this.f, this.g, this.h, this.i, this.k, this.j, this.l, $$0, this.o, this.p, this.q);
   }

   public dr a(akk $$0) {
      if ($$0 == this.e) {
         return this;
      } else {
         double $$1 = dij.a(this.e.x_(), $$0.x_());
         ehe $$2 = new ehe(this.d.c * $$1, this.d.d, this.d.e * $$1);
         return new dr(this.c, $$2, this.n, $$0, this.f, this.g, this.h, this.i, this.k, this.j, this.l, this.m, this.o, this.p, this.q);
      }
   }

   public dr a(bii $$0, eb.a $$1) {
      return this.b($$1.a($$0));
   }

   public dr b(ehe $$0) {
      ehe $$1 = this.m.a(this);
      double $$2 = $$0.c - $$1.c;
      double $$3 = $$0.d - $$1.d;
      double $$4 = $$0.e - $$1.e;
      double $$5 = Math.sqrt($$2 * $$2 + $$4 * $$4);
      float $$6 = arp.g((float)(-(arp.d($$3, $$5) * 180.0F / (float)Math.PI)));
      float $$7 = arp.g((float)(arp.d($$4, $$2) * 180.0F / (float)Math.PI) - 90.0F);
      return this.a(new ehd($$6, $$7));
   }

   public dr a(dp $$0) {
      return $$0 == this.o ? this : new dr(this.c, this.d, this.n, this.e, this.f, this.g, this.h, this.i, this.k, this.j, this.l, this.m, $$0, this.p, this.q);
   }

   public dr a(asj $$0) {
      return $$0 == this.p ? this : new dr(this.c, this.d, this.n, this.e, this.f, this.g, this.h, this.i, this.k, this.j, this.l, this.m, this.o, $$0, this.q);
   }

   public dr a(IntConsumer $$0) {
      return $$0 == this.q ? this : new dr(this.c, this.d, this.n, this.e, this.f, this.g, this.h, this.i, this.k, this.j, this.l, this.m, this.o, this.p, $$0);
   }

   public tf b() {
      return this.h;
   }

   public String c() {
      return this.g;
   }

   @Override
   public boolean c(int $$0) {
      return this.f >= $$0;
   }

   public ehe d() {
      return this.d;
   }

   public akk e() {
      return this.e;
   }

   @Nullable
   public bii f() {
      return this.k;
   }

   public bii g() throws CommandSyntaxException {
      if (this.k == null) {
         throw b.create();
      } else {
         return this.k;
      }
   }

   public akl h() throws CommandSyntaxException {
      bii var2 = this.k;
      if (var2 instanceof akl) {
         return (akl)var2;
      } else {
         throw a.create();
      }
   }

   @Nullable
   public akl i() {
      return this.k instanceof akl $$0 ? $$0 : null;
   }

   public boolean j() {
      return this.k instanceof akl;
   }

   public ehd k() {
      return this.n;
   }

   public MinecraftServer l() {
      return this.i;
   }

   public eb.a m() {
      return this.m;
   }

   public dp n() {
      return this.o;
   }

   public asj o() {
      return this.p;
   }

   public IntConsumer p() {
      return this.q;
   }

   public boolean a(akl $$0) {
      akl $$1 = this.i();
      return $$0 == $$1 ? false : $$1 != null && $$1.W() || $$0.W();
   }

   public void a(tt $$0, boolean $$1, tb.a $$2) {
      if (!this.j) {
         akl $$3 = this.i();
         if ($$3 != null) {
            $$3.a($$0, $$1, $$2);
         } else {
            this.c.a($$2.a($$0.a()));
         }
      }
   }

   public void a(tf $$0) {
      if (!this.j) {
         akl $$1 = this.i();
         if ($$1 != null) {
            $$1.a($$0);
         } else {
            this.c.a($$0);
         }
      }
   }

   public void a(Supplier<tf> $$0, boolean $$1) {
      boolean $$2 = this.c.f_() && !this.j;
      boolean $$3 = $$1 && this.c.N_() && !this.j;
      if ($$2 || $$3) {
         tf $$4 = $$0.get();
         if ($$2) {
            this.c.a($$4);
         }

         if ($$3) {
            this.c($$4);
         }
      }
   }

   private void c(tf $$0) {
      tf $$1 = tf.a("chat.type.admin", this.b(), $$0).a(n.h, n.u);
      if (this.i.aI().b(cpi.o)) {
         for (akl $$2 : this.i.ac().t()) {
            if ($$2 != this.c && this.i.ac().g($$2.fP())) {
               $$2.a($$1);
            }
         }
      }

      if (this.c != this.i && this.i.aI().b(cpi.l)) {
         this.i.a($$1);
      }
   }

   public void b(tf $$0) {
      if (this.c.q_() && !this.j) {
         this.c.a(tf.h().b($$0).a(n.m));
      }
   }

   public void a(CommandContext<dr> $$0, boolean $$1, int $$2) {
      if (this.l != null) {
         this.l.onCommandComplete($$0, $$1, $$2);
      }
   }

   @Override
   public Collection<String> q() {
      return Lists.newArrayList(this.i.J());
   }

   @Override
   public Collection<String> r() {
      return this.i.aF().f();
   }

   @Override
   public Stream<aer> s() {
      return jb.c.s().map(aov::a);
   }

   @Override
   public Stream<aer> t() {
      return this.i.aE().d();
   }

   @Override
   public CompletableFuture<Suggestions> a(CommandContext<?> $$0) {
      return Suggestions.empty();
   }

   @Override
   public CompletableFuture<Suggestions> a(aeq<? extends hr<?>> $$0, du.a $$1, SuggestionsBuilder $$2, CommandContext<?> $$3) {
      return this.v().c($$0).map($$2x -> {
         this.a($$2x, $$1, $$2);
         return $$2.buildFuture();
      }).orElseGet(Suggestions::empty);
   }

   @Override
   public Set<aeq<cpm>> u() {
      return this.i.E();
   }

   @Override
   public hs v() {
      return this.i.aV();
   }

   @Override
   public cdv w() {
      return this.e.G();
   }
}
