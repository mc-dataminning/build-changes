...
public class nx implements me {
   private final mg.a d;
   private final mg.a e;

   public nx(mg $$0) {
      this.d = $$0.a(mg.b.b, "blockstates");
      this.e = $$0.a(mg.b.b, "models");
   }

   @Override
   public CompletableFuture<?> a(mc $$0) {
      Map<dhy, ny> $$1 = Maps.newHashMap();
      Consumer<ny> $$2 = $$1x -> {
         dhy $$2x = $$1x.a();
         ny $$3x = $$1.put($$2x, $$1x);
         if ($$3x != null) {
      ...
      Map<alh, Supplier<JsonElement>> $$3 = Maps.newHashMap();
      Set<cvt> $$4 = Sets.newHashSet();
      BiConsumer<alh, Supplier<JsonElement>> $$5 = ($$1x, $$2x) -> {
         Supplier<JsonElement> $$3x = $$3.put($$1x, $$2x);
         if ($$3x != null) {
            throw new IllegalStateException("Duplicate model definition for " + $$1x);
         }
      };
      Consumer<cvt> $$6 = $$4::add;
      new nu($$2, $$5, $$6).a();
      new nw($$5).a();
      List<dhy> $$7 = lx.e.k().stream().filter($$0x -> true).map(Entry::getValue).filter($$1x -> !$$1.containsKey($$1x)).toList();
      if (!$$7.isEmpty()) {
   ...
   private <T> CompletableFuture<?> a(mc $$0, Map<T, ? extends Supplier<JsonElement>> $$1, Function<T, Path> $$2) {
