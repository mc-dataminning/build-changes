import java.util.List;
import java.util.Optional;

public class cud extends csa {
   public static final int m = 0;
   public static final int n = 1;
   private static final int r = 2;
   private static final int s = 29;
   private static final int t = 29;
   private static final int u = 38;
   private final csn v;
   final csu w = csu.a();
   private final dgg x;
   private dbp.b<dcc> y = dbp.b.a();
   private cwn z = cwn.j;
   long A;
   final ctx o;
   final ctx p;
   Runnable B = () -> {
   };
   public final bsb q = new bsp(1) {
      @Override
      public void e() {
         super.e();
         cud.this.a(this);
         cud.this.B.run();
      }
   };
   final cts C = new cts();

   public cud(int $$0, cou $$1) {
      this($$0, $$1, csn.a);
   }

   public cud(int $$0, cou $$1, final csn $$2) {
      super(ctj.y, $$0);
      this.v = $$2;
      this.x = $$1.k.dW();
      this.o = this.a(new ctx(this.q, 0, 20, 33));
      this.p = this.a(new ctx(this.C, 1, 143, 33) {
         @Override
         public boolean a(cwn $$0) {
            return false;
         }

         @Override
         public void a(cov $$0, cwn $$1) {
            $$1.a($$0.dW(), $$0, $$1.M());
            cud.this.C.a($$0, this.j());
            cwn $$2 = cud.this.o.a(1);
            if (!$$2.f()) {
               cud.this.f(cud.this.w.b());
            }

            $$2.a(($$0x, $$1x) -> {
               long $$2xxx = $$0x.ad();
               if (cud.this.A != $$2xxx) {
                  $$0x.a(null, $$1x, avz.Bj, awa.e, 1.0F, 1.0F);
                  cud.this.A = $$2xxx;
               }
            });
            super.a($$0, $$1);
         }

         private List<cwn> j() {
            return List.of(cud.this.o.g());
         }
      });
      this.c($$1, 8, 84);
      this.a(this.w);
   }

   public int l() {
      return this.w.b();
   }

   public dbp.b<dcc> m() {
      return this.y;
   }

   public int n() {
      return this.y.d();
   }

   public boolean o() {
      return this.o.h() && !this.y.c();
   }

   @Override
   public boolean b(cov $$0) {
      return a(this.v, $$0, djm.oE);
   }

   @Override
   public boolean a(cov $$0, int $$1) {
      if (this.w.b() == $$1) {
         return false;
      } else {
         if (this.e($$1)) {
            this.w.a($$1);
            this.f($$1);
         }

         return true;
      }
   }

   private boolean e(int $$0) {
      return $$0 >= 0 && $$0 < this.y.d();
   }

   @Override
   public void a(bsb $$0) {
      cwn $$1 = this.o.g();
      if (!$$1.a(this.z.h())) {
         this.z = $$1.v();
         this.c($$1);
      }
   }

   private void c(cwn $$0) {
      this.w.a(-1);
      this.p.f(cwn.j);
      if (!$$0.f()) {
         this.y = this.x.Q().a().b($$0);
      } else {
         this.y = dbp.b.a();
      }
   }

   void f(int $$0) {
      Optional<dbh<dcc>> $$2;
      if (!this.y.c() && this.e($$0)) {
         dbp.a<dcc> $$1 = this.y.e().get($$0);
         $$2 = $$1.c().c();
      } else {
         $$2 = Optional.empty();
      }

      $$2.ifPresentOrElse($$0x -> {
         this.C.a($$0x);
         this.p.f(((dcc)$$0x.b()).a(new dbv(this.q.a(0)), this.x.K_()));
      }, () -> {
         this.p.f(cwn.j);
         this.C.a(null);
      });
      this.d();
   }

   @Override
   public ctj<?> a() {
      return ctj.y;
   }

   public void a(Runnable $$0) {
      this.B = $$0;
   }

   @Override
   public boolean a(cwn $$0, ctx $$1) {
      return $$1.c != this.C && super.a($$0, $$1);
   }

   @Override
   public cwn b(cov $$0, int $$1) {
      cwn $$2 = cwn.j;
      ctx $$3 = this.k.get($$1);
      if ($$3 != null && $$3.h()) {
         cwn $$4 = $$3.g();
         cwj $$5 = $$4.h();
         $$2 = $$4.v();
         if ($$1 == 1) {
            $$5.a($$4, $$0.dW(), $$0);
            if (!this.a($$4, 2, 38, true)) {
               return cwn.j;
            }

            $$3.b($$4, $$2);
         } else if ($$1 == 0) {
            if (!this.a($$4, 2, 38, false)) {
               return cwn.j;
            }
         } else if (this.x.Q().a().a($$4)) {
            if (!this.a($$4, 0, 1, false)) {
               return cwn.j;
            }
         } else if ($$1 >= 2 && $$1 < 29) {
            if (!this.a($$4, 29, 38, false)) {
               return cwn.j;
            }
         } else if ($$1 >= 29 && $$1 < 38 && !this.a($$4, 2, 29, false)) {
            return cwn.j;
         }

         if ($$4.f()) {
            $$3.e(cwn.j);
         }

         $$3.c();
         if ($$4.M() == $$2.M()) {
            return cwn.j;
         }

         $$3.a($$0, $$4);
         if ($$1 == 1) {
            $$0.a($$4, false);
         }

         this.d();
      }

      return $$2;
   }

   @Override
   public void a(cov $$0) {
      super.a($$0);
      this.C.b(1);
      this.v.a(($$1, $$2) -> this.a($$0, this.q));
   }
}
