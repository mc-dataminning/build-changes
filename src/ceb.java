public abstract class ceb extends cfg implements ceg {
   private static final ajr<Boolean> b = ajv.a(ceb.class, ajt.k);

   public ceb(bsa<? extends ceb> $$0, dax $$1) {
      super($$0, $$1);
      this.bP = new ceb.a(this);
   }

   public static btv.a s() {
      return bsq.A().a(btw.q, 3.0);
   }

   @Override
   public boolean V() {
      return super.V() || this.u();
   }

   @Override
   public boolean h(double $$0) {
      return !this.u() && !this.ag();
   }

   @Override
   public int ae() {
      return 8;
   }

   @Override
   protected void a(ajv.a $$0) {
      super.a($$0);
      $$0.a(b, false);
   }

   @Override
   public boolean u() {
      return this.ao.a(b);
   }

   @Override
   public void w(boolean $$0) {
      this.ao.a(b, $$0);
   }

   @Override
   public void b(ud $$0) {
      super.b($$0);
      $$0.a("FromBucket", this.u());
   }

   @Override
   public void a(ud $$0) {
      super.a($$0);
      this.w($$0.q("FromBucket"));
   }

   @Override
   protected void z() {
      super.z();
      this.bS.a(0, new caf(this, 1.25));
      this.bS.a(2, new byt<>(this, clw.class, 8.0F, 1.6, 1.4, brz.f::test));
      this.bS.a(4, new ceb.b(this));
   }

   @Override
   protected cca b(dax $$0) {
      return new ccc(this, $$0);
   }

   @Override
   public void a(euk $$0) {
      if (this.db() && this.be()) {
         this.a(0.01F, $$0);
         this.a(bst.a, this.ds());
         this.g(this.ds().a(0.9));
         if (this.p() == null) {
            this.g(this.ds().b(0.0, -0.005, 0.0));
         }
      } else {
         super.a($$0);
      }
   }

   @Override
   public void n_() {
      if (!this.be() && this.aE() && this.R) {
         this.g(this.ds().b((double)((this.ah.i() * 2.0F - 1.0F) * 0.05F), 0.4F, (double)((this.ah.i() * 2.0F - 1.0F) * 0.05F)));
         this.d(false);
         this.av = true;
         this.b(this.gq());
      }

      super.n_();
   }

   @Override
   protected bpu b(clw $$0, bpt $$1) {
      return ceg.a($$0, $$1, this).orElse(super.b($$0, $$1));
   }

   @Override
   public void n(cto $$0) {
      ceg.a(this, $$0);
   }

   @Override
   public void c(ud $$0) {
      ceg.a(this, $$0);
   }

   @Override
   public avg y() {
      return avh.dk;
   }

   protected boolean gp() {
      return true;
   }

   protected abstract avg gq();

   @Override
   protected avg aP() {
      return avh.iV;
   }

   @Override
   protected void b(io $$0, drb $$1) {
   }

   static class a extends byp {
      private final ceb l;

      a(ceb $$0) {
         super($$0);
         this.l = $$0;
      }

      @Override
      public void a() {
         if (this.l.a(awb.a)) {
            this.l.g(this.l.ds().b(0.0, 0.005, 0.0));
         }

         if (this.k == byp.a.b && !this.l.K().l()) {
            float $$0 = (float)(this.h * this.l.g(btw.r));
            this.l.y(ayd.i(0.125F, this.l.fn(), $$0));
            double $$1 = this.e - this.l.du();
            double $$2 = this.f - this.l.dw();
            double $$3 = this.g - this.l.dA();
            if ($$2 != 0.0) {
               double $$4 = Math.sqrt($$1 * $$1 + $$2 * $$2 + $$3 * $$3);
               this.l.g(this.l.ds().b(0.0, (double)this.l.fn() * ($$2 / $$4) * 0.1, 0.0));
            }

            if ($$1 != 0.0 || $$3 != 0.0) {
               float $$5 = (float)(ayd.d($$3, $$1) * 180.0F / (float)Math.PI) - 90.0F;
               this.l.r(this.a(this.l.dF(), $$5, 90.0F));
               this.l.aZ = this.l.dF();
            }
         } else {
            this.l.y(0.0F);
         }
      }
   }

   static class b extends cak {
      private final ceb i;

      public b(ceb $$0) {
         super($$0, 1.0, 40);
         this.i = $$0;
      }

      @Override
      public boolean a() {
         return this.i.gp() && super.a();
      }
   }
}
