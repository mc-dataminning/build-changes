...
public class ku implements jk {
   private final jm.a d;
   private final jm.a e;

   public ku(jm $$0) {
      this.d = $$0.a(jm.b.b, "blockstates");
      this.e = $$0.a(jm.b.b, "models");
   }

   @Override
   public CompletableFuture<?> a(ji $$0) {
      Map<csq, kv> $$1 = Maps.newHashMap();
      Consumer<kv> $$2 = $$1x -> {
         csq $$2x = $$1x.a();
         kv $$3x = $$1.put($$2x, $$1x);
         if ($$3x != null) {
      ...
      Map<aeu, Supplier<JsonElement>> $$3 = Maps.newHashMap();
      Set<civ> $$4 = Sets.newHashSet();
      BiConsumer<aeu, Supplier<JsonElement>> $$5 = ($$1x, $$2x) -> {
         Supplier<JsonElement> $$3x = $$3.put($$1x, $$2x);
         if ($$3x != null) {
            throw new IllegalStateException("Duplicate model definition for " + $$1x);
         }
      };
      Consumer<civ> $$6 = $$4::add;
      new ks($$2, $$5, $$6).a();
      new kt($$5).a();
      List<csq> $$7 = jd.f.g().stream().filter($$0x -> true).map(Entry::getValue).filter($$1x -> !$$1.containsKey($$1x)).toList();
      if (!$$7.isEmpty()) {
   ...
   private <T> CompletableFuture<?> a(ji $$0, Map<T, ? extends Supplier<JsonElement>> $$1, Function<T, Path> $$2) {
