import com.mojang.serialization.Codec;
import com.mojang.serialization.codecs.RecordCodecBuilder;
import java.util.stream.Stream;

public class cuv extends cur {
   public static final Codec<cuv> b = RecordCodecBuilder.create(
      $$0 -> $$0.group(cun.d.fieldOf("biomes").forGetter($$0x -> $$0x.c), Codec.intRange(0, 62).fieldOf("scale").orElse(2).forGetter($$0x -> $$0x.e))
            .apply($$0, cuv::new)
   );
   private final il<cun> c;
   private final int d;
   private final int e;

   public cuv(il<cun> $$0, int $$1) {
      this.c = $$0;
      this.d = $$1 + 2;
      this.e = $$1;
   }

   @Override
   protected Stream<ih<cun>> b() {
      return this.c.a();
   }

   @Override
   protected Codec<? extends cur> a() {
      return b;
   }

   @Override
   public ih<cun> getNoiseBiome(int $$0, int $$1, int $$2, cuw.f $$3) {
      return this.c.a(Math.floorMod(($$0 >> this.d) + ($$2 >> this.d), this.c.b()));
   }
}
