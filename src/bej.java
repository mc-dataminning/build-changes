...
public class bej extends DataFix {
   ...
   public TypeRewriteRule makeRule() {
      return this.a(this.getInputSchema().getTypeRaw(bjb.t), this.getOutputSchema().getTypeRaw(bjb.t));
   }

   private <ItemStackOld, ItemStackNew> TypeRewriteRule a(Type<ItemStackOld> $$0, Type<ItemStackNew> $$1) {
      Type<Pair<String, Either<List<ItemStackOld>, Unit>>> $$2 = DSL.named(bjb.A.typeName(), DSL.optional(DSL.field("Equipment", DSL.list($$0))));
      ...
      if (!$$2.equals(this.getInputSchema().getType(bjb.A))) {
         throw new IllegalStateException("Input entity_equipment type does not match expected");
      } else if (!$$3.equals(this.getOutputSchema().getType(bjb.A))) {
         throw new IllegalStateException("Output entity_equipment type does not match expected");
      } else {
         return TypeRewriteRule.seq(
            this.fixTypeEverywhereTyped(
               "EntityEquipmentToArmorAndHandFix - drop chances", this.getInputSchema().getType(bjb.D), $$0x -> $$0x.update(DSL.remainderFinder(), bej::a)
            ),
