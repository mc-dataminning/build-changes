...
public class bfi extends DataFix {
   ...
   protected TypeRewriteRule makeRule() {
      Type<?> $$0 = this.getInputSchema().getTypeRaw(biq.t);
      Type<?> $$1 = this.getOutputSchema().getTypeRaw(biq.t);
   ...
   private <ItemStackOld, ItemStackNew> TypeRewriteRule a(Type<ItemStackOld> $$0, Type<ItemStackNew> $$1, OpticFinder<?> $$2) {
      ...
      if (!$$3.equals(this.getInputSchema().getType(biq.A))) {
         throw new IllegalStateException("Input entity_equipment type does not match expected");
      } else if (!$$4.equals(this.getOutputSchema().getType(biq.A))) {
