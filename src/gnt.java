public class gnt<T extends btc, M extends fuq<T> & ftp> extends goa<T, M> {
   private final geb a;

   public gnt(glk<T, M> $$0, geb $$1) {
      super($$0);
      this.a = $$1;
   }

   public void a(fao $$0, gef $$1, int $$2, T $$3, float $$4, float $$5, float $$6, float $$7, float $$8, float $$9) {
      boolean $$10 = $$3.fs() == bsw.b;
      cud $$11 = $$10 ? $$3.eW() : $$3.eV();
      cud $$12 = $$10 ? $$3.eV() : $$3.eW();
      if (!$$11.e() || !$$12.e()) {
         $$0.a();
         if (this.c().e) {
            float $$13 = 0.5F;
            $$0.a(0.0F, 0.75F, 0.0F);
            $$0.b(0.5F, 0.5F, 0.5F);
         }

         this.a($$3, $$12, cua.c, bsw.b, $$0, $$1, $$2);
         this.a($$3, $$11, cua.b, bsw.a, $$0, $$1, $$2);
         $$0.b();
      }
   }

   protected void a(btc $$0, cud $$1, cua $$2, bsw $$3, fao $$4, gef $$5, int $$6) {
      if (!$$1.e()) {
         $$4.a();
         this.c().a($$3, $$4);
         $$4.a(a.b.rotationDegrees(-90.0F));
         $$4.a(a.d.rotationDegrees(180.0F));
         boolean $$7 = $$3 == bsw.a;
         $$4.a((float)($$7 ? -1 : 1) / 16.0F, 0.125F, -0.625F);
         this.a.a($$0, $$1, $$2, $$7, $$4, $$5, $$6);
         $$4.b();
      }
   }
}
