...
public class bfb extends DataFix {
   ...
   private static <T> Dynamic<T> a(Dynamic<T> $$0) {
      ...
      Optional<Dynamic<T>> $$1 = $$0.get("HiddenEffect").result().map(bfb::a);
      return $$0.replaceField("HiddenEffect", "hidden_effect", $$1);
   }

   private static <T> Dynamic<T> b(Dynamic<T> $$0, String $$1, String $$2) {
      Optional<Dynamic<T>> $$3 = $$0.get($$1).asStreamOpt().result().map($$1x -> $$0.createList($$1x.map(bfb::a)));
   ...
   private static <T> Dynamic<T> g(Dynamic<T> $$0) {
      Optional<Dynamic<T>> $$1 = $$0.get("Effects").asStreamOpt().result().map($$1x -> $$0.createList($$1x.map(bfb::b)));
      ...
      return this.fixTypeEverywhereTyped("ItemStackMobEffectIdFix", $$1, $$2x -> {
         ...
         if ($$3.isPresent()) {
            ...
            if ($$4.equals("minecraft:suspicious_stew")) {
               return $$2x.updateTyped($$2, $$0xx -> $$0xx.update(DSL.remainderFinder(), bfb::g));
            }
