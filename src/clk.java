import java.util.EnumSet;
import javax.annotation.Nullable;

public class clk extends cmp implements cma {
   public static final float a = 0.03F;
   boolean cb;
   protected final cet b;
   protected final ceq c;

   public clk(buq<? extends clk> $$0, dgg $$1) {
      super($$0, $$1);
      this.bP = new clk.d(this);
      this.a(eto.j, 0.0F);
      this.b = new cet(this, $$1);
      this.c = new ceq(this, $$1);
   }

   public static bwm.a m() {
      return cmp.gs().a(bwn.B, 1.0);
   }

   @Override
   protected void p() {
      this.bS.a(1, new clk.c(this, 1.0));
      this.bS.a(2, new clk.f(this, 1.0, 40, 10.0F));
      this.bS.a(2, new clk.a(this, 1.0, false));
      this.bS.a(5, new clk.b(this, 1.0));
      this.bS.a(6, new clk.e(this, 1.0, this.dW().P()));
      this.bS.a(7, new cda(this, 1.0));
      this.bT.a(1, new cdv(this, clk.class).a(cmr.class));
      this.bT.a(2, new cdw<>(this, cov.class, 10, true, false, ($$0, $$1) -> this.j($$0)));
      this.bT.a(3, new cdw<>(this, cob.class, false));
      this.bT.a(3, new cdw<>(this, chi.class, true));
      this.bT.a(3, new cdw<>(this, cii.class, true, false));
      this.bT.a(5, new cdw<>(this, chx.class, 10, true, false, chx.bY));
   }

   @Override
   public bvy a(dgx $$0, bsg $$1, bup $$2, @Nullable bvy $$3) {
      $$3 = super.a($$0, $$1, $$2, $$3);
      if (this.a(bur.b).f() && $$0.H_().i() < 0.03F) {
         this.a(bur.b, new cwn(cwr.wT));
         this.g(bur.b);
      }

      return $$3;
   }

   public static boolean a(buq<clk> $$0, dgx $$1, bup $$2, ji $$3, azg $$4) {
      if (!$$1.b_($$3.e()).a(awu.a) && !bup.a($$2)) {
         return false;
      } else {
         jr<dhi> $$5 = $$1.t($$3);
         boolean $$6 = $$1.am() != bsf.a && (bup.b($$2) || a($$1, $$3, $$4)) && (bup.a($$2) || $$1.b_($$3).a(awu.a));
         if (!$$6 || !bup.a($$2) && $$2 != bup.j) {
            return $$5.a(awn.ap) ? $$4.a(15) == 0 && $$6 : $$4.a(40) == 0 && a($$1, $$3) && $$6;
         } else {
            return true;
         }
      }
   }

   private static boolean a(dgh $$0, ji $$1) {
      return $$1.v() < $$0.P() - 5;
   }

   @Override
   protected avy u() {
      return this.bj() ? avz.hZ : avz.hY;
   }

   @Override
   protected avy e(bsz $$0) {
      return this.bj() ? avz.id : avz.ic;
   }

   @Override
   protected avy o_() {
      return this.bj() ? avz.ib : avz.ia;
   }

   @Override
   protected avy t() {
      return avz.if;
   }

   @Override
   protected avy aV() {
      return avz.ig;
   }

   @Override
   protected boolean x() {
      return true;
   }

   @Override
   protected cwn gp() {
      return cwn.j;
   }

   @Override
   protected void a(azg $$0, bsg $$1) {
      if ((double)$$0.i() > 0.9) {
         int $$2 = $$0.a(16);
         if ($$2 < 10) {
            this.a(bur.a, new cwn(cwr.wS));
         } else {
            this.a(bur.a, new cwn(cwr.rS));
         }
      }
   }

   @Override
   protected boolean a(cwn $$0, cwn $$1, bur $$2) {
      return $$1.a(cwr.wT) ? false : super.a($$0, $$1, $$2);
   }

   @Override
   protected boolean gq() {
      return false;
   }

   @Override
   public boolean a(dgj $$0) {
      return $$0.f(this);
   }

   public boolean j(@Nullable bvf $$0) {
      return $$0 != null ? !this.dW().V() || $$0.bj() : false;
   }

   @Override
   public boolean cJ() {
      return !this.ck();
   }

   boolean gy() {
      if (this.cb) {
         return true;
      } else {
         bvf $$0 = this.O_();
         return $$0 != null && $$0.bj();
      }
   }

   @Override
   public void a_(fay $$0) {
      if (this.di() && this.bo() && this.gy()) {
         this.a(0.01F, $$0);
         this.a(bvj.a, this.dz());
         this.h(this.dz().c(0.9));
      } else {
         super.a_($$0);
      }
   }

   @Override
   public void bp() {
      if (!this.dW().C) {
         if (this.dk() && this.bo() && this.gy()) {
            this.bR = this.b;
            this.i(true);
         } else {
            this.bR = this.c;
            this.i(false);
         }
      }
   }

   @Override
   public boolean cl() {
      return this.ck();
   }

   protected boolean gr() {
      etl $$0 = this.L().i();
      if ($$0 != null) {
         ji $$1 = $$0.l();
         if ($$1 != null) {
            double $$2 = this.i((double)$$1.u(), (double)$$1.v(), (double)$$1.w());
            if ($$2 < 4.0) {
               return true;
            }
         }
      }

      return false;
   }

   @Override
   public void a(bvf $$0, float $$1) {
      cwn $$2 = this.eZ();
      cwn $$3 = $$2.a(cwr.wS) ? $$2 : new cwn(cwr.wS);
      cqb $$4 = new cqb(this.dW(), this, $$3);
      double $$5 = $$0.dB() - this.dB();
      double $$6 = $$0.e(0.3333333333333333) - $$4.dD();
      double $$7 = $$0.dH() - this.dH();
      double $$8 = Math.sqrt($$5 * $$5 + $$7 * $$7);
      if (this.dW() instanceof arc $$9) {
         cpo.a($$4, $$9, $$3, $$5, $$6 + $$8 * 0.2F, $$7, 1.6F, (float)(14 - this.dW().am().a() * 4));
      }

      this.a(avz.ie, 1.0F, 1.0F / (this.dZ().i() * 0.4F + 0.8F));
   }

   @Override
   public axe<cwj> W() {
      return awx.bS;
   }

   public void x(boolean $$0) {
      this.cb = $$0;
   }

   static class a extends cds {
      private final clk b;

      public a(clk $$0, double $$1, boolean $$2) {
         super($$0, $$1, $$2);
         this.b = $$0;
      }

      @Override
      public boolean b() {
         return super.b() && this.b.j(this.b.O_());
      }

      @Override
      public boolean c() {
         return super.c() && this.b.j(this.b.O_());
      }
   }

   static class b extends ccq {
      private final clk g;

      public b(clk $$0, double $$1) {
         super($$0, $$1, 8, 2);
         this.g = $$0;
      }

      @Override
      public boolean b() {
         return super.b() && !this.g.dW().V() && this.g.bj() && this.g.dD() >= (double)(this.g.dW().P() - 3);
      }

      @Override
      public boolean c() {
         return super.c();
      }

      @Override
      protected boolean a(dgj $$0, ji $$1) {
         ji $$2 = $$1.d();
         return $$0.u($$2) && $$0.u($$2.d()) ? $$0.a_($$1).a($$0, $$1, this.g) : false;
      }

      @Override
      public void d() {
         this.g.x(false);
         this.g.bR = this.g.c;
         super.d();
      }

      @Override
      public void e() {
         super.e();
      }
   }

   static class c extends ccd {
      private final bvn a;
      private double b;
      private double c;
      private double d;
      private final double e;
      private final dgg f;

      public c(bvn $$0, double $$1) {
         this.a = $$0;
         this.e = $$1;
         this.f = $$0.dW();
         this.a(EnumSet.of(ccd.a.a));
      }

      @Override
      public boolean b() {
         if (!this.f.V()) {
            return false;
         } else if (this.a.bj()) {
            return false;
         } else {
            fay $$0 = this.h();
            if ($$0 == null) {
               return false;
            } else {
               this.b = $$0.d;
               this.c = $$0.e;
               this.d = $$0.f;
               return true;
            }
         }
      }

      @Override
      public boolean c() {
         return !this.a.L().k();
      }

      @Override
      public void d() {
         this.a.L().a(this.b, this.c, this.d, this.e);
      }

      @Nullable
      private fay h() {
         azg $$0 = this.a.dZ();
         ji $$1 = this.a.dw();

         for (int $$2 = 0; $$2 < 10; $$2++) {
            ji $$3 = $$1.b($$0.a(20) - 10, 2 - $$0.a(8), $$0.a(20) - 10);
            if (this.f.a_($$3).a(djm.J)) {
               return fay.c($$3);
            }
         }

         return null;
      }
   }

   static class d extends cbg {
      private final clk l;

      public d(clk $$0) {
         super($$0);
         this.l = $$0;
      }

      @Override
      public void a() {
         bvf $$0 = this.l.O_();
         if (this.l.gy() && this.l.bj()) {
            if ($$0 != null && $$0.dD() > this.l.dD() || this.l.cb) {
               this.l.h(this.l.dz().b(0.0, 0.002, 0.0));
            }

            if (this.k != cbg.a.b || this.l.L().k()) {
               this.l.C(0.0F);
               return;
            }

            double $$1 = this.e - this.l.dB();
            double $$2 = this.f - this.l.dD();
            double $$3 = this.g - this.l.dH();
            double $$4 = Math.sqrt($$1 * $$1 + $$2 * $$2 + $$3 * $$3);
            $$2 /= $$4;
            float $$5 = (float)(ayy.d($$3, $$1) * 180.0F / (float)Math.PI) - 90.0F;
            this.l.v(this.b(this.l.dM(), $$5, 90.0F));
            this.l.aX = this.l.dM();
            float $$6 = (float)(this.h * this.l.h(bwn.v));
            float $$7 = ayy.h(0.125F, this.l.fp(), $$6);
            this.l.C($$7);
            this.l.h(this.l.dz().b((double)$$7 * $$1 * 0.005, (double)$$7 * $$2 * 0.1, (double)$$7 * $$3 * 0.005));
         } else {
            if (!this.l.aJ()) {
               this.l.h(this.l.dz().b(0.0, -0.008, 0.0));
            }

            super.a();
         }
      }
   }

   static class e extends ccd {
      private final clk a;
      private final double b;
      private final int c;
      private boolean d;

      public e(clk $$0, double $$1, int $$2) {
         this.a = $$0;
         this.b = $$1;
         this.c = $$2;
      }

      @Override
      public boolean b() {
         return !this.a.dW().V() && this.a.bj() && this.a.dD() < (double)(this.c - 2);
      }

      @Override
      public boolean c() {
         return this.b() && !this.d;
      }

      @Override
      public void a() {
         if (this.a.dD() < (double)(this.c - 1) && (this.a.L().k() || this.a.gr())) {
            fay $$0 = cga.a(this.a, 4, 8, new fay(this.a.dB(), (double)(this.c - 1), this.a.dH()), (float) (Math.PI / 2));
            if ($$0 == null) {
               this.d = true;
               return;
            }

            this.a.L().a($$0.d, $$0.e, $$0.f, this.b);
         }
      }

      @Override
      public void d() {
         this.a.x(true);
         this.d = false;
      }

      @Override
      public void e() {
         this.a.x(false);
      }
   }

   static class f extends cdc {
      private final clk a;

      public f(cma $$0, double $$1, int $$2, float $$3) {
         super($$0, $$1, $$2, $$3);
         this.a = (clk)$$0;
      }

      @Override
      public boolean b() {
         return super.b() && this.a.eZ().a(cwr.wS);
      }

      @Override
      public void d() {
         super.d();
         this.a.w(true);
         this.a.c(bsh.a);
      }

      @Override
      public void e() {
         super.e();
         this.a.fF();
         this.a.w(false);
      }
   }
}
