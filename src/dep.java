import com.mojang.serialization.MapCodec;

public abstract class dep extends dfa implements dmb {
   protected static final ewl a = dfa.a(0.0, 0.0, 0.0, 16.0, 2.0, 16.0);
   protected static final ewl b = dfa.a(0.0, 0.0, 0.0, 16.0, 8.0, 16.0);
   public static final dsu c = dst.C;
   private final boolean d;

   public static boolean a(dbz $$0, iz $$1) {
      return g($$0.a_($$1));
   }

   public static boolean g(dsd $$0) {
      return $$0.a(awp.N) && $$0.b() instanceof dep;
   }

   protected dep(boolean $$0, dsc.d $$1) {
      super($$1);
      this.d = $$0;
   }

   @Override
   protected abstract MapCodec<? extends dep> a();

   public boolean b() {
      return this.d;
   }

   @Override
   protected ewl a(dsd $$0, dbf $$1, iz $$2, evx $$3) {
      dth $$4 = $$0.a(this) ? $$0.c(this.c()) : null;
      return $$4 != null && $$4.b() ? b : a;
   }

   @Override
   protected boolean a(dsd $$0, dcc $$1, iz $$2) {
      return c($$1, $$2.d());
   }

   @Override
   protected void b(dsd $$0, dbz $$1, iz $$2, dsd $$3, boolean $$4) {
      if (!$$3.a($$0.b())) {
         this.a($$0, $$1, $$2, $$4);
      }
   }

   protected dsd a(dsd $$0, dbz $$1, iz $$2, boolean $$3) {
      $$0 = this.a($$1, $$2, $$0, true);
      if (this.d) {
         $$1.a($$0, $$2, this, $$2, $$3);
      }

      return $$0;
   }

   @Override
   protected void a(dsd $$0, dbz $$1, iz $$2, dfa $$3, iz $$4, boolean $$5) {
      if (!$$1.B && $$1.a_($$2).a(this)) {
         dth $$6 = $$0.c(this.c());
         if (a($$2, $$1, $$6)) {
            c($$0, $$1, $$2);
            $$1.a($$2, $$5);
         } else {
            this.a($$0, $$1, $$2, $$3);
         }
      }
   }

   private static boolean a(iz $$0, dbz $$1, dth $$2) {
      if (!c($$1, $$0.d())) {
         return true;
      } else {
         switch ($$2) {
            case c:
               return !c($$1, $$0.h());
            case d:
               return !c($$1, $$0.g());
            case e:
               return !c($$1, $$0.e());
            case f:
               return !c($$1, $$0.f());
            default:
               return false;
         }
      }
   }

   protected void a(dsd $$0, dbz $$1, iz $$2, dfa $$3) {
   }

   protected dsd a(dbz $$0, iz $$1, dsd $$2, boolean $$3) {
      if ($$0.B) {
         return $$2;
      } else {
         dth $$4 = $$2.c(this.c());
         return new dla($$0, $$1, $$2).a($$0.C($$1), $$3, $$4).c();
      }
   }

   @Override
   protected void a(dsd $$0, dbz $$1, iz $$2, dsd $$3, boolean $$4) {
      if (!$$4) {
         super.a($$0, $$1, $$2, $$3, $$4);
         if ($$0.c(this.c()).b()) {
            $$1.a($$2.c(), this);
         }

         if (this.d) {
            $$1.a($$2, this);
            $$1.a($$2.d(), this);
         }
      }
   }

   @Override
   public dsd a(cyb $$0) {
      enw $$1 = $$0.q().b_($$0.a());
      boolean $$2 = $$1.a() == enx.c;
      dsd $$3 = super.o();
      je $$4 = $$0.g();
      boolean $$5 = $$4 == je.f || $$4 == je.e;
      return $$3.a(this.c(), $$5 ? dth.b : dth.a).a(c, Boolean.valueOf($$2));
   }

   public abstract dtg<dth> c();

   @Override
   protected dsd a(dsd $$0, je $$1, dsd $$2, dca $$3, iz $$4, iz $$5) {
      if ($$0.c(c)) {
         $$3.a($$4, enx.c, enx.c.a($$3));
      }

      return super.a($$0, $$1, $$2, $$3, $$4, $$5);
   }

   @Override
   protected enw b_(dsd $$0) {
      return $$0.c(c) ? enx.c.a(false) : super.b_($$0);
   }
}
