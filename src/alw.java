import com.google.common.collect.Lists;
import com.google.common.collect.Sets;
import java.util.List;
import java.util.Optional;
import java.util.Set;
import javax.annotation.Nullable;
import net.minecraft.server.MinecraftServer;

public class alw extends ewu {
   private final MinecraftServer b;
   private final Set<ewm> c = Sets.newHashSet();
   private final List<Runnable> d = Lists.newArrayList();

   public alw(MinecraftServer $$0) {
      this.b = $$0;
   }

   @Override
   protected void a(ewt $$0, ewm $$1, ewr $$2) {
      super.a($$0, $$1, $$2);
      if (this.c.contains($$1)) {
         this.b.ah().a(new afv($$0.cB(), $$1.b(), $$2.a(), Optional.ofNullable($$2.d()), Optional.ofNullable($$2.c())));
      }

      this.a();
   }

   @Override
   protected void a(ewt $$0, ewm $$1) {
      super.a($$0, $$1);
      this.a();
   }

   @Override
   public void a(ewt $$0) {
      super.a($$0);
      this.b.ah().a(new aeu($$0.cB(), null));
      this.a();
   }

   @Override
   public void b(ewt $$0, ewm $$1) {
      super.b($$0, $$1);
      if (this.c.contains($$1)) {
         this.b.ah().a(new aeu($$0.cB(), $$1.b()));
      }

      this.a();
   }

   @Override
   public void a(ewl $$0, @Nullable ewm $$1) {
      ewm $$2 = this.a($$0);
      super.a($$0, $$1);
      if ($$2 != $$1 && $$2 != null) {
         if (this.h($$2) > 0) {
            this.b.ah().a(new afl($$0, $$1));
         } else {
            this.g($$2);
         }
      }

      if ($$1 != null) {
         if (this.c.contains($$1)) {
            this.b.ah().a(new afl($$0, $$1));
         } else {
            this.e($$1);
         }
      }

      this.a();
   }

   @Override
   public boolean a(String $$0, ewp $$1) {
      if (super.a($$0, $$1)) {
         this.b.ah().a(afu.a($$1, $$0, afu.a.a));
         this.a();
         return true;
      } else {
         return false;
      }
   }

   @Override
   public void b(String $$0, ewp $$1) {
      super.b($$0, $$1);
      this.b.ah().a(afu.a($$1, $$0, afu.a.b));
      this.a();
   }

   @Override
   public void a(ewm $$0) {
      super.a($$0);
      this.a();
   }

   @Override
   public void b(ewm $$0) {
      super.b($$0);
      if (this.c.contains($$0)) {
         this.b.ah().a(new afs($$0, 2));
      }

      this.a();
   }

   @Override
   public void c(ewm $$0) {
      super.c($$0);
      if (this.c.contains($$0)) {
         this.g($$0);
      }

      this.a();
   }

   @Override
   public void a(ewp $$0) {
      super.a($$0);
      this.b.ah().a(afu.a($$0, true));
      this.a();
   }

   @Override
   public void b(ewp $$0) {
      super.b($$0);
      this.b.ah().a(afu.a($$0, false));
      this.a();
   }

   @Override
   public void c(ewp $$0) {
      super.c($$0);
      this.b.ah().a(afu.a($$0));
      this.a();
   }

   public void a(Runnable $$0) {
      this.d.add($$0);
   }

   protected void a() {
      for (Runnable $$0 : this.d) {
         $$0.run();
      }
   }

   public List<zv<?>> d(ewm $$0) {
      List<zv<?>> $$1 = Lists.newArrayList();
      $$1.add(new afs($$0, 0));

      for (ewl $$2 : ewl.values()) {
         if (this.a($$2) == $$0) {
            $$1.add(new afl($$2, $$0));
         }
      }

      for (ewn $$3 : this.i($$0)) {
         $$1.add(new afv($$3.c(), $$0.b(), $$3.d(), Optional.ofNullable($$3.e()), Optional.ofNullable($$3.f())));
      }

      return $$1;
   }

   public void e(ewm $$0) {
      List<zv<?>> $$1 = this.d($$0);

      for (arf $$2 : this.b.ah().t()) {
         for (zv<?> $$3 : $$1) {
            $$2.c.b($$3);
         }
      }

      this.c.add($$0);
   }

   public List<zv<?>> f(ewm $$0) {
      List<zv<?>> $$1 = Lists.newArrayList();
      $$1.add(new afs($$0, 1));

      for (ewl $$2 : ewl.values()) {
         if (this.a($$2) == $$0) {
            $$1.add(new afl($$2, $$0));
         }
      }

      return $$1;
   }

   public void g(ewm $$0) {
      List<zv<?>> $$1 = this.f($$0);

      for (arf $$2 : this.b.ah().t()) {
         for (zv<?> $$3 : $$1) {
            $$2.c.b($$3);
         }
      }

      this.c.remove($$0);
   }

   public int h(ewm $$0) {
      int $$1 = 0;

      for (ewl $$2 : ewl.values()) {
         if (this.a($$2) == $$0) {
            $$1++;
         }
      }

      return $$1;
   }

   public epb.a<ewv> b() {
      return new epb.a<>(this::h, this::a, baf.n);
   }

   private ewv h() {
      ewv $$0 = new ewv(this);
      this.a($$0::c);
      return $$0;
   }

   private ewv a(ur $$0, jk.a $$1) {
      return this.h().b($$0, $$1);
   }

   public static enum a {
      a,
      b;
   }
}
