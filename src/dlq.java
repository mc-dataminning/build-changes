import com.mojang.serialization.MapCodec;
import java.util.List;
import java.util.Optional;
import java.util.OptionalInt;
import javax.annotation.Nullable;

public class dlq extends djx {
   public static final MapCodec<dlq> a = b(dlq::new);
   private static final int d = 6;
   public static final int b = 3;
   public static final List<dyl> c = List.of(dyk.bq, dyk.br, dyk.bs, dyk.bt, dyk.bu, dyk.bv);

   @Override
   public MapCodec<dlq> a() {
      return a;
   }

   public dlq(dxt.d $$0) {
      super($$0);
      dxu $$1 = this.F.b().b(doj.aF, jm.c);

      for (dyl $$2 : c) {
         $$1 = $$1.b($$2, Boolean.valueOf(false));
      }

      this.l($$1);
   }

   @Override
   protected dqu a_(dxu $$0) {
      return dqu.c;
   }

   @Override
   protected bti a(cxo $$0, dxu $$1, dhh $$2, jh $$3, cpw $$4, bth $$5, fbt $$6) {
      if ($$2.c_($$3) instanceof dvf $$7) {
         if (!$$0.a(ayd.bb)) {
            return bti.f;
         } else {
            OptionalInt $$9 = this.a($$6, $$1);
            if ($$9.isEmpty()) {
               return bti.e;
            } else if ($$1.c(c.get($$9.getAsInt()))) {
               return bti.f;
            } else {
               a($$2, $$3, $$4, $$7, $$0, $$9.getAsInt());
               return bti.a;
            }
         }
      } else {
         return bti.e;
      }
   }

   @Override
   protected bti a(dxu $$0, dhh $$1, jh $$2, cpw $$3, fbt $$4) {
      if ($$1.c_($$2) instanceof dvf $$5) {
         OptionalInt $$7 = this.a($$4, $$0);
         if ($$7.isEmpty()) {
            return bti.e;
         } else if (!$$0.c(c.get($$7.getAsInt()))) {
            return bti.c;
         } else {
            a($$1, $$2, $$3, $$5, $$7.getAsInt());
            return bti.a;
         }
      } else {
         return bti.e;
      }
   }

   private OptionalInt a(fbt $$0, dxu $$1) {
      return a($$0, $$1.c(doj.aF)).map($$0x -> {
         int $$1x = $$0x.j >= 0.5F ? 0 : 1;
         int $$2 = a($$0x.i);
         return OptionalInt.of($$2 + $$1x * 3);
      }).orElseGet(OptionalInt::empty);
   }

   private static Optional<fbw> a(fbt $$0, jm $$1) {
      jm $$2 = $$0.c();
      if ($$1 != $$2) {
         return Optional.empty();
      } else {
         jh $$3 = $$0.b().a($$2);
         fbx $$4 = $$0.g().a((double)$$3.u(), (double)$$3.v(), (double)$$3.w());
         double $$5 = $$4.a();
         double $$6 = $$4.b();
         double $$7 = $$4.c();

         return switch ($$2) {
            case c -> Optional.of(new fbw((float)(1.0 - $$5), (float)$$6));
            case d -> Optional.of(new fbw((float)$$5, (float)$$6));
            case e -> Optional.of(new fbw((float)$$7, (float)$$6));
            case f -> Optional.of(new fbw((float)(1.0 - $$7), (float)$$6));
            case a, b -> Optional.empty();
         };
      }
   }

   private static int a(float $$0) {
      float $$1 = 0.0625F;
      float $$2 = 0.375F;
      if ($$0 < 0.375F) {
         return 0;
      } else {
         float $$3 = 0.6875F;
         return $$0 < 0.6875F ? 1 : 2;
      }
   }

   private static void a(dhh $$0, jh $$1, cpw $$2, dvf $$3, cxo $$4, int $$5) {
      if (!$$0.C) {
         $$2.b(axp.c.b($$4.h()));
         axe $$6 = $$4.a(cxs.vm) ? axf.fc : axf.fb;
         $$3.a($$5, $$4.b(1, $$2));
         $$0.a(null, $$1, $$6, axg.e, 1.0F, 1.0F);
      }
   }

   private static void a(dhh $$0, jh $$1, cpw $$2, dvf $$3, int $$4) {
      if (!$$0.C) {
         cxo $$5 = $$3.a($$4, 1);
         axe $$6 = $$5.a(cxs.vm) ? axf.ff : axf.fe;
         $$0.a(null, $$1, $$6, axg.e, 1.0F, 1.0F);
         if (!$$2.gi().f($$5)) {
            $$2.a($$5, false);
         }

         $$0.a($$2, ecq.c, $$1);
      }
   }

   @Nullable
   @Override
   public duw a(jh $$0, dxu $$1) {
      return new dvf($$0, $$1);
   }

   @Override
   protected void a(dxv.a<dkl, dxu> $$0) {
      $$0.a(doj.aF);
      c.forEach($$1 -> $$0.a($$1));
   }

   @Override
   protected void a(dxu $$0, dhh $$1, jh $$2, dxu $$3, boolean $$4) {
      if (!$$0.a($$3.b())) {
         boolean $$9;
         label32: {
            if ($$1.c_($$2) instanceof dvf $$6 && !$$6.c()) {
               for (int $$7 = 0; $$7 < 6; $$7++) {
                  cxo $$8 = $$6.a($$7);
                  if (!$$8.f()) {
                     bte.a($$1, (double)$$2.u(), (double)$$2.v(), (double)$$2.w(), $$8);
                  }
               }

               $$6.a();
               $$9 = true;
               break label32;
            }

            $$9 = false;
         }

         super.a($$0, $$1, $$2, $$3, $$4);
         if ($$9) {
            $$1.c($$2, this);
         }
      }
   }

   @Override
   public dxu a(dbf $$0) {
      return this.m().b(doj.aF, $$0.g().g());
   }

   @Override
   public dxu a(dxu $$0, drb $$1) {
      return $$0.b(doj.aF, $$1.a($$0.c(doj.aF)));
   }

   @Override
   public dxu a(dxu $$0, dpk $$1) {
      return $$0.a($$1.a($$0.c(doj.aF)));
   }

   @Override
   protected boolean c_(dxu $$0) {
      return true;
   }

   @Override
   protected int a(dxu $$0, dhh $$1, jh $$2) {
      if ($$1.B_()) {
         return 0;
      } else {
         return $$1.c_($$2) instanceof dvf $$3 ? $$3.j() + 1 : 0;
      }
   }
}
