...
public class lr implements kf {
   ...
   public CompletableFuture<?> a(kd $$0) {
      ...
      Map<afw, Supplier<JsonElement>> $$3 = Maps.newHashMap();
      Set<cke> $$4 = Sets.newHashSet();
      BiConsumer<afw, Supplier<JsonElement>> $$5 = ($$1x, $$2x) -> {
         Supplier<JsonElement> $$3x = $$3.put($$1x, $$2x);
         if ($$3x != null) {
            throw new IllegalStateException("Duplicate model definition for " + $$1x);
         }
      };
      Consumer<cke> $$6 = $$4::add;
      new lp($$2, $$5, $$6).a();
      new lq($$5).a();
      List<cua> $$7 = jy.f.g().stream().filter($$0x -> true).map(Entry::getValue).filter($$1x -> !$$1.containsKey($$1x)).toList();
      if (!$$7.isEmpty()) {
