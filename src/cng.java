import java.util.EnumSet;
import java.util.function.IntFunction;
import javax.annotation.Nullable;

public abstract class cng extends cmd {
   private static final alc<Byte> a = alg.a(cng.class, ale.a);
   protected int b;
   private cng.a bZ = cng.a.a;

   protected cng(bvq<? extends cng> $$0, dhh $$1) {
      super($$0, $$1);
   }

   @Override
   protected void a(alg.a $$0) {
      super.a($$0);
      $$0.a(a, (byte)0);
   }

   @Override
   public void a(ux $$0) {
      super.a($$0);
      this.b = $$0.h("SpellTicks");
   }

   @Override
   public void b(ux $$0) {
      super.b($$0);
      $$0.a("SpellTicks", this.b);
   }

   @Override
   public cmd.a m() {
      if (this.gx()) {
         return cmd.a.c;
      } else {
         return this.gG() ? cmd.a.g : cmd.a.a;
      }
   }

   public boolean gx() {
      return this.dW().C ? this.al.a(a) > 0 : this.b > 0;
   }

   public void a(cng.a $$0) {
      this.bZ = $$0;
      this.al.a(a, (byte)$$0.h);
   }

   protected cng.a gy() {
      return !this.dW().C ? this.bZ : cng.a.a(this.al.a(a));
   }

   @Override
   protected void a(ash $$0) {
      super.a($$0);
      if (this.b > 0) {
         this.b--;
      }
   }

   @Override
   public void h() {
      super.h();
      if (this.dW().C && this.gx()) {
         cng.a $$0 = this.gy();
         float $$1 = (float)$$0.i[0];
         float $$2 = (float)$$0.i[1];
         float $$3 = (float)$$0.i[2];
         float $$4 = this.aX * (float) (Math.PI / 180.0) + bae.b((float)this.af * 0.6662F) * 0.25F;
         float $$5 = bae.b($$4);
         float $$6 = bae.a($$4);
         double $$7 = 0.6 * (double)this.ek();
         double $$8 = 1.8 * (double)this.ek();
         this.dW().a(ll.a(ls.u, $$1, $$2, $$3), this.dB() + (double)$$5 * $$7, this.dD() + $$8, this.dH() + (double)$$6 * $$7, 0.0, 0.0, 0.0);
         this.dW().a(ll.a(ls.u, $$1, $$2, $$3), this.dB() - (double)$$5 * $$7, this.dD() + $$8, this.dH() - (double)$$6 * $$7, 0.0, 0.0, 0.0);
      }
   }

   protected int gz() {
      return this.b;
   }

   protected abstract axe x();

   protected static enum a {
      a(0, 0.0, 0.0, 0.0),
      b(1, 0.7, 0.7, 0.8),
      c(2, 0.4, 0.3, 0.35),
      d(3, 0.7, 0.5, 0.2),
      e(4, 0.3, 0.3, 0.8),
      f(5, 0.1, 0.1, 0.2);

      private static final IntFunction<cng.a> g = ayv.a($$0 -> $$0.h, values(), ayv.a.a);
      final int h;
      final double[] i;

      private a(final int $$0, final double $$1, final double $$2, final double $$3) {
         this.h = $$0;
         this.i = new double[]{$$1, $$2, $$3};
      }

      public static cng.a a(int $$0) {
         return g.apply($$0);
      }
   }

   protected class b extends cdd {
      public b() {
         this.a(EnumSet.of(cdd.a.a, cdd.a.b));
      }

      @Override
      public boolean b() {
         return cng.this.gz() > 0;
      }

      @Override
      public void d() {
         super.d();
         cng.this.bS.o();
      }

      @Override
      public void e() {
         super.e();
         cng.this.a(cng.a.a);
      }

      @Override
      public void a() {
         if (cng.this.O_() != null) {
            cng.this.H().a(cng.this.O_(), (float)cng.this.aa(), (float)cng.this.Z());
         }
      }
   }

   protected abstract class c extends cdd {
      protected int b;
      protected int c;

      @Override
      public boolean b() {
         bwf $$0 = cng.this.O_();
         if ($$0 == null || !$$0.bL()) {
            return false;
         } else {
            return cng.this.gx() ? false : cng.this.af >= this.c;
         }
      }

      @Override
      public boolean c() {
         bwf $$0 = cng.this.O_();
         return $$0 != null && $$0.bL() && this.b > 0;
      }

      @Override
      public void d() {
         this.b = this.a(this.n());
         cng.this.b = this.h();
         this.c = cng.this.af + this.i();
         axe $$0 = this.l();
         if ($$0 != null) {
            cng.this.a($$0, 1.0F, 1.0F);
         }

         cng.this.a(this.m());
      }

      @Override
      public void a() {
         this.b--;
         if (this.b == 0) {
            this.k();
            cng.this.a(cng.this.x(), 1.0F, 1.0F);
         }
      }

      protected abstract void k();

      protected int n() {
         return 20;
      }

      protected abstract int h();

      protected abstract int i();

      @Nullable
      protected abstract axe l();

      protected abstract cng.a m();
   }
}
