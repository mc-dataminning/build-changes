import com.mojang.datafixers.kinds.App;
import java.util.Optional;
import java.util.function.Function;

public class bxl {
   public static bvi<btw> a(ccs<jd> $$0, float $$1, int $$2, boolean $$3) {
      return a($$0, $$1, $$2, $$3, exc::c);
   }

   public static bws<btw> b(ccs<? extends bsr> $$0, float $$1, int $$2, boolean $$3) {
      return a($$0, $$1, $$2, $$3, bsr::dm);
   }

   private static <T> bws<btw> a(ccs<T> $$0, float $$1, int $$2, boolean $$3, Function<T, exc> $$4) {
      return byu.a(
         (Function<byu.b<btw>, ? extends App<byu.c<btw>, byx<btw>>>)($$5 -> $$5.group($$5.a(ccs.m), $$5.b($$0)).apply($$5, ($$5x, $$6) -> ($$7, $$8, $$9) -> {
                  Optional<ccv> $$10 = $$5.a($$5x);
                  if ($$10.isPresent() && !$$3) {
                     return false;
                  } else {
                     exc $$11 = $$8.dm();
                     exc $$12 = $$4.apply($$5.b($$6));
                     if (!$$11.a((jw)$$12, (double)$$2)) {
                        return false;
                     } else {
                        if ($$10.isPresent() && $$10.get().b() == $$1) {
                           exc $$13 = $$10.get().a().a().d($$11);
                           exc $$14 = $$12.d($$11);
                           if ($$13.b($$14) < 0.0) {
                              return false;
                           }
                        }

                        for (int $$15 = 0; $$15 < 10; $$15++) {
                           exc $$16 = cem.b($$8, 16, 7, $$12);
                           if ($$16 != null) {
                              $$5x.a(new ccv($$16, $$1, 0));
                              break;
                           }
                        }

                        return true;
                     }
                  }
               }))
      );
   }
}
