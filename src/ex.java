...
public class ex<T extends Enum<T> & apr> implements ArgumentType<T> {
   ...
   public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> $$0, SuggestionsBuilder $$1) {
      return du.b(Arrays.<Enum>stream((Enum[])this.c.get()).map($$0x -> ((apr)$$0x).c()).map(this::a).collect(Collectors.toList()), $$1);
   }

   public Collection<String> getExamples() {
      return Arrays.<Enum>stream((Enum[])this.c.get()).map($$0 -> ((apr)$$0).c()).map(this::a).limit(2L).collect(Collectors.toList());
   }
