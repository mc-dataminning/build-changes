...
public class bec extends DataFix {
   ...
   public TypeRewriteRule makeRule() {
      return this.a(this.getInputSchema().getTypeRaw(biq.t), this.getOutputSchema().getTypeRaw(biq.t));
   }

   private <ItemStackOld, ItemStackNew> TypeRewriteRule a(Type<ItemStackOld> $$0, Type<ItemStackNew> $$1) {
      Type<Pair<String, Either<List<ItemStackOld>, Unit>>> $$2 = DSL.named(biq.A.typeName(), DSL.optional(DSL.field("Equipment", DSL.list($$0))));
      ...
      if (!$$2.equals(this.getInputSchema().getType(biq.A))) {
         throw new IllegalStateException("Input entity_equipment type does not match expected");
      } else if (!$$3.equals(this.getOutputSchema().getType(biq.A))) {
         throw new IllegalStateException("Output entity_equipment type does not match expected");
      } else {
         return TypeRewriteRule.seq(
            this.fixTypeEverywhereTyped(
               "EntityEquipmentToArmorAndHandFix - drop chances", this.getInputSchema().getType(biq.D), $$0x -> $$0x.update(DSL.remainderFinder(), bec::a)
            ),
