import com.mojang.serialization.Codec;
import com.mojang.serialization.MapCodec;
import com.mojang.serialization.codecs.RecordCodecBuilder;
import java.util.stream.Stream;

public class dfb extends dex {
   public static final MapCodec<dfb> b = RecordCodecBuilder.mapCodec(
      $$0 -> $$0.group(det.d.fieldOf("biomes").forGetter($$0x -> $$0x.c), Codec.intRange(0, 62).fieldOf("scale").orElse(2).forGetter($$0x -> $$0x.e))
            .apply($$0, dfb::new)
   );
   private final jr<det> c;
   private final int d;
   private final int e;

   public dfb(jr<det> $$0, int $$1) {
      this.c = $$0;
      this.d = $$1 + 2;
      this.e = $$1;
   }

   @Override
   protected Stream<jn<det>> b() {
      return this.c.a();
   }

   @Override
   protected MapCodec<? extends dex> a() {
      return b;
   }

   @Override
   public jn<det> getNoiseBiome(int $$0, int $$1, int $$2, dfc.f $$3) {
      return this.c.a(Math.floorMod(($$0 >> this.d) + ($$2 >> this.d), this.c.b()));
   }
}
